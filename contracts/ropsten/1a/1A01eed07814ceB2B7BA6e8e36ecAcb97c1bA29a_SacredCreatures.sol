...
contract SacredCreatures is ERC721Enumerable, ReentrancyGuard, Ownable  {
    ...
    function _createCreature(uint256 _tokenId, string memory _name) internal {
        ...
        creatures[_tokenId].attack =  rangeValues.attackMax > rangeValues.attackMin ?randomFromString(string(abi.encodePacked("attack", toString(_tokenId), family)), rangeValues.attackMax - rangeValues.attackMin) + rangeValues.attackMin : rangeValues.attackMin;
        creatures[_tokenId].soul = rangeValues.soulMax > rangeValues.soulMin ? randomFromString(string(abi.encodePacked("soul", toString(_tokenId), family)), rangeValues.soulMax - rangeValues.soulMin) + rangeValues.soulMin : rangeValues.soulMin;
        creatures[_tokenId].wisdom =  rangeValues.wisdomMax > rangeValues.wisdomMin ?randomFromString(string(abi.encodePacked("wisdom", toString(_tokenId), family)), rangeValues.wisdomMax - rangeValues.wisdomMin) + rangeValues.wisdomMin : rangeValues.wisdomMin;
