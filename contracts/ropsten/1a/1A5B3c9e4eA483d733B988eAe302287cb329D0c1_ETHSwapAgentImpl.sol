...
contract ETHSwapAgentImpl is Context {
    ...
    mapping(bytes32 => bool) public filledKAITx;
    ...
    event SwapFilled(address indexed erc20Addr, bytes32 indexed kaiTxHash, address indexed toAddress, uint256 amount);
    ...
    function registerSwapPairToKAI(address erc20Addr) onlyOwner external returns (bool) {
    ...
    function fillKAI2ETHSwap(bytes32 kaiTxHash, address erc20Addr, address toAddress, uint256 amount) onlyOwner external returns (bool) {
        require(!filledKAITx[kaiTxHash], "kai tx filled already");
        require(registeredERC20[erc20Addr], "not registered token");

        filledKAITx[kaiTxHash] = true;
        IERC20(erc20Addr).safeTransfer(toAddress, amount);

        emit SwapFilled(erc20Addr, kaiTxHash, toAddress, amount);
        return true;
    }

    function swapETH2KAI(address erc20Addr, uint256 amount) payable external notContract returns (bool) {
        require(registeredERC20[erc20Addr], "not registered token");
