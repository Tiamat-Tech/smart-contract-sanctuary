...
contract LimelightTreasury is LimelightAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    /**
     * @notice mint new LMLT using excess reserves
     ...
     * or LMLT from the treasury, using sLMLT as collateral. This might allow an
     * sLMLT holder to provide LMLT liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(LMLT)) {
            require(permissions[STATUS.LMLTDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sLMLT.changeDebt(value, msg.sender, true);
        require(sLMLT.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(LMLT)) {
            LMLT.mint(msg.sender, value);
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sLMLT.changeDebt(value, msg.sender, false);
     ...
     * @param _amount uint256
     */
    function repayDebtWithLMLT(uint256 _amount) external {
        require(permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.LMLTDEBTOR][msg.sender], notApproved);
        LMLT.burnFrom(msg.sender, _amount);
        sLMLT.changeDebt(_amount, msg.sender, false);
    ...
    function enable(
        ...
        if (_status == STATUS.SLMLT) {
            sLMLT = IsLMLT(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SLMLT) {
            // 9
            sLMLT = IsLMLT(info.toPermit);
        } else {
    ...
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(LMLT)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return LMLT.totalSupply() - ohmDebt;
    }
}