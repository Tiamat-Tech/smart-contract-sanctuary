...
import "./SeaPunkFactory.sol";
import "./ERC721Metadata.sol";
import "./PaymentSplitter.sol";

contract SeaPunks is ERC721Metadata, PaymentSplitter {
  ...
  uint256 public constant TOKEN_LIMIT = 2048; // TODO update this
  ...
  function createSeaPunk(uint256 seed) external payable returns (string memory) {
    return _mint(_msgSender(), seed);
  }

  function calculatePrice() internal view returns (uint256) { // TODO update this
    uint256 price;
    if (numTokens < 256) {
  ...
  function _mint(address to, uint256 _seed) internal returns (string memory) {
    ...
    string memory seaPunk = SeaPunkFactory.draw(idToSeed[id]);
    emit Generated(id, to, seaPunk);
  ...
  {
    string memory uri =SeaPunkFactory.draw(idToSeed[tokenId]);
    return uri;
  }
