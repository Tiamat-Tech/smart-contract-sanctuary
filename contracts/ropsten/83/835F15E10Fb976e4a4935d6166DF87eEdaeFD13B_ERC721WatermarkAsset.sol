...
contract ERC721WatermarkAsset is ERC721Base {

    /// @notice Whitelist controller
    IWlController public wlController;

    event CreateERC721WatermarkAsset(address owner, string name, string symbol);
    event SetWlController(address indexed wlController);

    function __ERC721WatermarkAsset_init(
        string memory _name,
        ...
        emit CreateERC721WatermarkAsset(_msgSender(), _name, _symbol);
    }
        ...
        if (address(controller) != address(0)) {
            ...
            if (from != address(0)) {
                require(
                    ...
                    "ERC721WatermarkAsset: transfer permission denied"
                );
            }
