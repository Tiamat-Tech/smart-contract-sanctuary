...
contract FunkyMonkeyFratHouseImmutable is ERC721Enumerable, Pausable, Ownable, IMXMethods {
    ...
    constructor(address _imx) ERC721("FunkyMonkeyFratHouseImmutable", "FFHI") IMXMethods(_imx) {}
    ...
    * @param user: Address of the recieving wallet, must be registered in IMX.
    * @param quantity: ID of the Token that will be minted
    * @param mintingBlob: Parsed mintingBlob without the TokenID prefix
    */
    function _mintFor(
        address user,
        uint256 quantity,
    ...
    */
    function reserveGiveaway(address addr, uint256 amount) external onlyOwner {
        ...
        require(supply + amount <= maxSupply, "FunkyMonkeyFratHouse: Exceeds max supply");
