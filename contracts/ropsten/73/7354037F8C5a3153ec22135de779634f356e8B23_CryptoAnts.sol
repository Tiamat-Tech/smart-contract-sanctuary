...
contract CryptoAnts is ERC721, ICryptoAnts, Ownable {
  ...
  function sellAnt(uint256 _antId) external {
    require(antToOwner[_antId] == msg.sender, 'No estas autorizado');
    uint256 antOferta = (eggPrice * 90) / 100;
    (bool success, ) = msg.sender.call{value: antOferta}('');
  ...
  modifier lock(uint256 _antId) {
    ...
    require(antTime[_antId] <= block.timestamp, 'No puedes!! :)');
    _;
