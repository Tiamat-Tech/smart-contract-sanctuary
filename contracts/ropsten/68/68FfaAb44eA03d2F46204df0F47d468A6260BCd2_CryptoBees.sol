...
contract CryptoBees is ICryptoBees, ERC721Enumerable, Ownable, ReentrancyGuard, Pausable {
    ...
    function mintForEth(uint256 amount, bool presale) external payable whenNotPaused nonReentrant {
        if (presale) require(whitelisted[_msgSender()], "You are not on WL");
        MintCheck.checkETH(_msgSender(), amount, minted, presale, mintWithEthPresalePaused, MAX_TOKENS, PAID_TOKENS, MINT_PRICE_DISCOUNT, MINT_PRICE);
