...
contract Mishka2 is Context, ERC20, Ownable {
    ...
    uint256 private m_SellFee = 100; // 10% Sell Fee.
    ...
    function getFeeWallet() external view onlyOwner returns(address payable)
    {
        return m_FeeWallet;
    }
    ...
    function _payToll() private {
        ...
        if (_readyToSwap() && overMinTokenBalanceForDisperseEth) {
            ...
            if(m_FeeWallet != address(0) &&
               m_FeeWallet != address(this))
