...
contract Gachadass is ERC721Pausable, Ownable {
  ...
  function get_tokens(uint256 start, uint256 end) external view returns(Event_Token[] memory) {
      uint256 length = totalSupply();
      if (end > length){
          end = length;
      }
      Event_Token[] memory tokens = new Event_Token[](end - start);
      for (uint256 i = start; i < end; i++){
          tokens[i - start] = Event_Token(
            i,
            token_data[i],
            ownerOf(i)
          );
  ...
  function get_tokens(address user, uint256 start, uint256 end) external view returns(Event_Token[] memory) {
