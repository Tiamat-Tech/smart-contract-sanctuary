...
contract Sale{
    ...
    function _orderPlace(address from, uint256 tokenId, uint256 _price) public{
    ...
    function _cancelOrder(address from, uint256 tokenId) public{
    ...
    function _changePrice(uint256 value, uint256 tokenId) public{
        require( balances[tokenId][msg.sender] > 0, "Is Not a Owner");
    ...
    function _acceptBId(address token,address from, address admin, uint256 amount, uint256 tokenId) public{
    ...
    function _saleToken(address payable from, address payable admin,uint256 tokenId, uint256 amount) public payable{
...
contract ExchangeContract is Sale{
    ...
    function setApprovalForAll(address token, uint256 _type, address to, bool approved, uint256 tokenId) public {
     ...
     function setApprovalForAll(address from,uint256 tokenId, address token, uint256 _type, bool approved) public{
        if(_type == 721){
            MyERC721 tok= MyERC721(token);
            tok.setApprovalForAll(from, approved, tokenId);
        }
        else{
            DigitalERC1155 tok = DigitalERC1155(token);
            tok.setApprovalForAll(from, approved);
        }
