...
import './interfaces/IZetaRouter02.sol';
import './interfaces/IZetaFactory.sol';
import './interfaces/IERC20.sol';
import './interfaces/IWETH.sol';

contract ZetaRouter02 is IZetaRouter02 {
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            IZetaPair(ZetaLibrary.pairFor(factory, input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, ZetaLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
...
/*
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, ZetaLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
