...
contract VoxmonNFT is EIP712, ERC721URIStorage, AccessControl {
    ...
    function redeem(address redeemer, NFTVoucher calldata voucher, string memory signature) public payable returns (uint256) {
        // make sure signature is valid and get the address of the signer
        bytes memory bSignature = bytes(signature);
        address signer = _verify(voucher, bSignature);
