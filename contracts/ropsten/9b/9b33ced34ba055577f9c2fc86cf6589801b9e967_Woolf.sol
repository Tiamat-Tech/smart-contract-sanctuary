...
contract Woolf is IWoolf, ERC721Enumerable, Ownable, Pausable {
   ...
   */
  constructor(address _peach, address _traits, uint256 _maxTokens, address _superShibaBAMC, address _superShibaClub) ERC721("Wolf Game", 'WGAME') { 
    // 桃子币合约
  ...
  function _freeMint(uint256 amount, bool stake) private whenNotPaused {
    uint16[] memory tokenIds = stake ? new uint16[](amount) : new uint16[](0);
    uint256 seed;
    for (uint i = 0; i < amount; i++) {
      // 已被mint数量自增
