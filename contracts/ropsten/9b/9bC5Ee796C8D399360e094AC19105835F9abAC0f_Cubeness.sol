...
contract Cubeness is ERC721URIStorage, Ownable, RoyaltiesV2Impl{
    ...
    address payable _stash;
    uint256 public totalSupply;
    uint256 private totalCubeness;

    mapping(address => uint) private minterBalance;
    ...
    constructor(
        ...
        _stash = payable(stash);
        totalCubeness = 5000;
    }

    function Cubed() public {
        uint minted = minterBalance[_msgSender()];

        require((totalSupply + 1) <= totalCubeness);
        ...
        _setRoyalty(_stash, totalSupply + 1);
        minterBalance[_msgSender()]++;
        totalSupply++;
    }

    function setTotal(uint256 total) public onlyOwner{
        totalCubeness = total;
    }
    ...
    function _setRoyalty(address payable _splitter, uint256 _tokenId) internal {
        LibPart.Part[] memory _royalties = new LibPart.Part[](1);
        _royalties[0].value = 750;
        _royalties[0].account = _splitter;
