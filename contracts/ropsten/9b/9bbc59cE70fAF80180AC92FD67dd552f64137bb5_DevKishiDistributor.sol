...
contract DevKishiDistributor is Ownable {
    ...
    function distribute(uint256 _total) external onlyOwner {
        require(_total > 0, "No KISHI to distribute");
    ...
    // funtion to claim the locked tokens for DevKishiDistributor, which will transfer the locked tokens for dev to devAddr after the devLockingPeriod
    function claimLockedTokens(uint256 r) external onlyOwner {

        boneLocker.claimAll(r);
    }
    // Update kishiLocker address by the owner.
