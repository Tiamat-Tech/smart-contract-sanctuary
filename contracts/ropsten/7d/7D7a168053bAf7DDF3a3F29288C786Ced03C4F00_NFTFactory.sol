...
interface IGenericNFT {
    function makeUniqueNFT(string memory title, string memory nftDescription, string memory cid, address _user) external;
}
...
contract NFTFactory {
    ...
    function createNFT(address _genericNFT, string memory title, string memory nftDescription, string memory cid) public {
        IGenericNFT(_genericNFT).makeUniqueNFT(title, nftDescription, cid, msg.sender);
    }
