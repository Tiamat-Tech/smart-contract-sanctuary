...
contract Seller is ISeller, Ownable, WeekManaged {
    ...
    function getPendingBasket(address who_, uint8 category_, uint256 week_) public view returns(bytes memory) {
        BasketRequest storage request = basketRequestMap[who_][week_][category_];
        bytes memory result = new bytes(request.assetIndexes.length * 16);

        for (uint i = 0; i < request.assetIndexes.length; ++i) {
            result[i * 2] = byte(uint8(request.assetIndexes[i] / 256));
            result[i * 2 + 1] = byte(uint8(request.assetIndexes[i] % 256));
        }

        return result;
    }

    function getLatestPendingBasket(address who_, uint8 category_) external view returns(bytes memory) {
        uint256 week = getCurrentWeek();
