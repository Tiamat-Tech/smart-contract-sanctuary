...
contract TokenSale is Ownable {
    ...
    modifier saleIsOn() {
        require(block.timestamp > startTime && block.timestamp < endTime, "TokenSale: It is not allowed to buy tokens for now");
        require(_tokenIdTracker.current() != nonFungibleContract.totalSupply(), "TokenSale: Sale is over by reaching the total supply");
        _;
    }

    constructor(address _nftAddress, uint256 _startTime, uint256 _endTime) {
    ...
    */
    ...
    // function isEnded() public view virtual returns (bool) {
    //     return hasEnded;
    // }
}