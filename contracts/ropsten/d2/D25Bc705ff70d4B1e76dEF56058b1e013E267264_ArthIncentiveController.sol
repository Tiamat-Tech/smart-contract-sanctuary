...
contract ArthIncentiveController is IIncentiveController, Setters, Epoch {
    ...
    function estimateRewardToGive(
        ...
        // A number from 0-100%.
        uint256 rewardToGive = percentOfPool.mul(deviationFromTarget).div(10000);

        uint256 _rewardsThisEpoch = _getUpdatedRewardsPerEpoch();

        uint256 calculatedRewards =
            _rewardsThisEpoch.mul(rewardToGive).mul(rewardMultiplier).mul(arthToMahaRate).div(10000 * 100000 * 1e18);
