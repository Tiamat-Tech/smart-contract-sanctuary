...
contract TradeManager is TradeOwner, ReentrancyGuard {
  ...
  event eventFillTrade(
    ...
    uint256 accepedTokenAmountSent
  );
  ...
  function fillTrade(
    ...
    fillAcceptedObj.accepedTokenAmountSent = fillAcceptedObj.acceptedAmount.mul(token0FillAmount).div(trades[tradeKey].token0Amount);

    // filler must deposit funds
    if (acceptedToken == address(0)) {
      require(msg.value >= fillAcceptedObj.accepedTokenAmountSent);
      Address.sendValue(payable(address(this)), fillAcceptedObj.accepedTokenAmountSent);
    } else {
      IERC20(acceptedToken).safeTransferFrom(address(_msgSender()), address(this), fillAcceptedObj.accepedTokenAmountSent);
    }
    ...
    newFiller.accepedTokenAmountSent = fillAcceptedObj.accepedTokenAmountSent;
    ...
    tradeCommissions.acceptedTokenCommission = fillAcceptedObj.accepedTokenAmountSent.mul(feeToUse).div(feeDivider);
    ...
    if (trades[tradeKey].refer != address(0) && referEnabled) {
      ...
      if (trades[tradeKey].token0 == address(0)) {
        Address.sendValue(payable(address(_msgSender())), token0FillAmount.sub(tradeCommissions.token0Commission));
        Address.sendValue(payable(address(commissionAddress)), tradeCommissions.token0Commission.sub(tradeCommissions.referToken0Commission));
        Address.sendValue(payable(address(trades[tradeKey].refer)), tradeCommissions.referToken0Commission);
      } else {
        IERC20(trades[tradeKey].token0).safeTransfer(address(_msgSender()), token0FillAmount.sub(tradeCommissions.token0Commission));
        IERC20(trades[tradeKey].token0).safeTransfer(address(commissionAddress), tradeCommissions.token0Commission.sub(tradeCommissions.referToken0Commission));
        IERC20(trades[tradeKey].token0).safeTransfer(address(trades[tradeKey].refer), tradeCommissions.referToken0Commission);
      }

      // send to trade owner
      if (acceptedToken == address(0)) {
        Address.sendValue(payable(address(trades[tradeKey].user0)), fillAcceptedObj.accepedTokenAmountSent.sub(tradeCommissions.acceptedTokenCommission));
        Address.sendValue(payable(address(commissionAddress)), tradeCommissions.acceptedTokenCommission.sub(tradeCommissions.referAcceptedTokenCommission));
        Address.sendValue(payable(address(trades[tradeKey].refer)), tradeCommissions.referAcceptedTokenCommission);
      } else {
        IERC20(acceptedToken).safeTransfer(address(trades[tradeKey].user0), fillAcceptedObj.accepedTokenAmountSent.sub(tradeCommissions.acceptedTokenCommission));
        IERC20(acceptedToken).safeTransfer(address(commissionAddress), tradeCommissions.acceptedTokenCommission.sub(tradeCommissions.referAcceptedTokenCommission));
        IERC20(acceptedToken).safeTransfer(address(trades[tradeKey].refer), tradeCommissions.referAcceptedTokenCommission);
      }
      ...
      if (trades[tradeKey].token0 == address(0)) {
        ...
        IERC20(trades[tradeKey].token0).safeTransfer(address(_msgSender()), token0FillAmount.sub(tradeCommissions.token0Commission));
        IERC20(trades[tradeKey].token0).safeTransfer(address(commissionAddress), tradeCommissions.token0Commission);
      }

      // send to trade owner
      if (acceptedToken == address(0)) {
        Address.sendValue(payable(address(trades[tradeKey].user0)), fillAcceptedObj.accepedTokenAmountSent.sub(tradeCommissions.acceptedTokenCommission));
        Address.sendValue(payable(address(commissionAddress)), tradeCommissions.acceptedTokenCommission);
      } else {
        IERC20(acceptedToken).safeTransfer(address(trades[tradeKey].user0), fillAcceptedObj.accepedTokenAmountSent.sub(tradeCommissions.acceptedTokenCommission));
        IERC20(acceptedToken).safeTransfer(address(commissionAddress), tradeCommissions.acceptedTokenCommission);
      }
    }
    ...
    emit eventFillTrade(
      ...
      token0FillAmount,
      acceptedToken,
      fillAcceptedObj.accepedTokenAmountSent
    );
  }
  ...
  function cancelTrade(
    ...
    if (trades[tradeKey].token0 == address(0)) {
      Address.sendValue(payable(address(_msgSender())), trades[tradeKey].token0AmountRemaining);
    } else {
  ...
  function getRequiredAcceptedAmount(
    ...
    address acceptedToken,
    uint256 token0FillAmount
  ) public view returns (uint256 accepedTokenAmountSent) {
