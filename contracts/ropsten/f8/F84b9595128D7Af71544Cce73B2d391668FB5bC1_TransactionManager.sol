...
contract TransactionManager is TransactionOwner, ReentrancyGuard {
  ...
  function depositUser0(
    ...
    if (transaction.transactionType == TransactionType.ETHToToken) {
      ...
      Address.sendValue(payable(address(this)), transaction.token0Amount);
    }

    if (transaction.transactionType != TransactionType.ETHToToken) {
      IERC20(transaction.token0).safeTransferFrom(address(_msgSender()), address(this), transaction.token0Amount);
    }
  ...
  function depositUser1(
    ...
    if (transaction.transactionType == TransactionType.TokenToETH) {
      ...
      Address.sendValue(payable(address(this)), transaction.token1Amount);
    }

    if (transaction.transactionType != TransactionType.TokenToETH) {
      IERC20(transaction.token1).safeTransferFrom(address(_msgSender()), address(this), transaction.token1Amount);
    }
  ...
  function withdrawUser0(
    ...
    if (block.timestamp <= transaction.expiresAt) {
      ...
      if (transactions[transactionKey].refer != address(0) && referEnabled) {
        ...
        if (transaction.transactionType == TransactionType.TokenToETH) {
          Address.sendValue(payable(address(_msgSender())), transaction.token1Amount.sub(commissionAmount));
          Address.sendValue(payable(address(commissionAddress)), commissionAmount.sub(referCommission));
          Address.sendValue(payable(address(transactions[transactionKey].refer)), referCommission);
        }

        if (transaction.transactionType != TransactionType.TokenToETH) {
          IERC20(transaction.token1).safeTransfer(address(_msgSender()), transaction.token1Amount.sub(commissionAmount));
          IERC20(transaction.token1).safeTransfer(address(commissionAddress), commissionAmount.sub(referCommission));
          IERC20(transaction.token1).safeTransfer(address(transactions[transactionKey].refer), referCommission);
        }
        ...
        if (transaction.transactionType != TransactionType.TokenToETH) {
          IERC20(transaction.token1).safeTransfer(address(_msgSender()), transaction.token1Amount.sub(commissionAmount));
          IERC20(transaction.token1).safeTransfer(address(commissionAddress), commissionAmount);
        }
    ...
    if (block.timestamp > transaction.expiresAt) {
      ...
      if (transaction.transactionType == TransactionType.ETHToToken) {
        Address.sendValue(payable(address(_msgSender())), transaction.token0Amount);
      }

      if (transaction.transactionType != TransactionType.ETHToToken) {
        IERC20(transaction.token0).safeTransfer(address(_msgSender()), transaction.token0Amount);
      }
    ...
    if (block.timestamp <= transaction.expiresAt) {
      ...
      if (transactions[transactionKey].refer != address(0) && referEnabled) {
        ...
        if (transaction.transactionType != TransactionType.ETHToToken) {
          ...
          IERC20(transaction.token0).safeTransfer(address(commissionAddress), commissionAmount.sub(referCommission));
          IERC20(transaction.token0).safeTransfer(address(transactions[transactionKey].refer), referCommission);
        }
