...
contract FantomNFTFactoryPrivate is Ownable {
    ...
    */
    ...
    /// @notice handle whitelist
    function getWhitelistedAddresses() public view returns (address[] memory) {
        address[] memory whitelist = new address[](
            getWhitelistedAddressesCount()
        );
        for (uint256 i = 0; i < getWhitelistedAddressesCount(); i++) {
            whitelist[i] = whitelistedAddresses[i];
        }
        return whitelist;
    }

    function getWhitelistedAddressesCount()
        public
        view
    ...
    function setWhitelistedAddresses(address[] calldata _users)
        public
        onlyOwner
    {
