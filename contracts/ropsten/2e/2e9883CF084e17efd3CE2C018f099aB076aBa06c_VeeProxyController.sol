    ...
    function executeOrder(bytes32 orderId) external returns (bool){
        ...
        if(price >= orders[orderId].stopHighPairPrice || price <= orders[orderId].stopLowPairPrice){
            ...
            if(orders[orderId].autoRepay){                 
                  if (profit > 0) {
                     repayBorrow(orders[orderId].ctokenA, orders[orderId].orderOwner, orders[orderId].amountA);
...
//for test only
     ...
     * @param repayAmount The address of token A     
     *
    ...
    //     return result;
    // }
    function repayBorrow(address cToken, address borrower, uint repayAmount) public returns(uint)
    {
        ...
        erc20Inst.approve(cToken, repayAmount);
        //emit Approve(underlyingAddress, cToken, repayAmount, actionStatus);
