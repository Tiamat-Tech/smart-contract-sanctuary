...
contract AZNASFC2 is ERC721, ERC721Enumerable, Ownable {
        ...
        payable
    {
        ...
        upDateWinners(msg.sender, _mintPrice);
    }
    ...
    function winnerCanMint(address _addrs, uint256 _mintPrice) public view returns (bool) {
        // Get user from winners
        Winner memory user = winners[_addrs];
        if (user.mintPrice != 0 && user.mintCount < 2){
            // User could have won multiple promotions (e.g 0 or 0.015 ether / mint)
            if (user.mintPrice != _mintPrice){ 
                return true;
            }else{
                return false;
            }
