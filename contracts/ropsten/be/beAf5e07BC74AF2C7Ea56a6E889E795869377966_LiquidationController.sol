...
contract LiquidationController is Ownable {
    ...
    constructor() {
        ...
        swapRouter = Router(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506);
        // SUSHI Router on Ropsten
    }
    ...
    function executeLiquidations(address[] memory _borrowers, uint256[] memory _repayAmounts, address[] memory _bdCollaterals, uint256 _totalRepayAmount) external {
        ...
        modes[0] = 0;
        aaveLending.flashLoan(address(this), assets, amounts, modes, address(this), "", 0);
        ...
        aaveLending.repay(address(DAI), type(uint256).max, 0, address(this));
        // Don't think we should be using type(uint256).max here for the amount
    }
