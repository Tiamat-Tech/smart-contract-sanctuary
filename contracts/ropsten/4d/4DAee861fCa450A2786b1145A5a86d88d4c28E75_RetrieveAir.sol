...
contract RetrieveAir is Context, IERC20, Ownable {
    ...
    string private _name = "RetrieveAir";
    string private _symbol = "RTA";
...
// This make it secure from bots
    ...
    modifier launchRestrict(address sender, address recipient, uint256 amount) {
        if (_tradingTime > now) {
            require(sender == owner() || recipient == owner(), "RTA: transfers are disabled");
        } else if (_tradingTime <= now && _restrictionLiftTime > now) {
            require(amount <= _maxRestrictionAmount, "RTA: amount greater than max limit in restricted mode");
            if (!_isWhitelisted[sender] && !_isWhitelisted[recipient]) {
                require(_lastTx[sender].add(60) <= now && _lastTx[recipient].add(60) <= now, "RTA: only one tx/min in restricted mode");
                _lastTx[sender] = now;
                _lastTx[recipient] = now;
            } else if (!_isWhitelisted[recipient]){
                require(_lastTx[recipient].add(60) <= now, "RTA: only one tx/min in restricted mode");
                _lastTx[recipient] = now;
            } else if (!_isWhitelisted[sender]) {
                require(_lastTx[sender].add(60) <= now, "RTA: only one tx/min in restricted mode");
                _lastTx[sender] = now;
            }
        }
        _;
...
// Bot End here
    ...
    // RetrieveAir Fixed SSL- | Incorrect error message error
    ...
    function _transfer(
        ...
        require(!_paused || from == owner() || to == owner(), "RTA: Transfers are paused"); // pause check
    ...
    // RetrieveAir Fixed the condition
