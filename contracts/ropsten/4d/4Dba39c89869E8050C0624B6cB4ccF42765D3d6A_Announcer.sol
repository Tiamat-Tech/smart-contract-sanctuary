...
contract Announcer is Controllable, IAnnouncer {
  ...
  function timeLockInfo(uint256 idx) external view returns (TimeLockInfo memory) {
  ...
  ///                 7 - PsVault
  ///                 8 - Fund
  /// @param oldAddress Old address, only for information - will not be checked and can be zero
  ...
  function announceAddressChange(TimeLockOpCodes opCode, address oldAddress, address newAddress) external onlyGovernance {
    ...
    values[0] = newAddress;
    _timeLockInfos.push(TimeLockInfo(opCode, oldAddress, values, new uint256[](0)));
  ...
  function announceMint(uint256 totalAmount, address _distributor, address _otherNetworkFund) external onlyGovernance {
    ...
    _timeLockInfos.push(TimeLockInfo(opCode, IController(controller()).mintHelper(), adrValues, intValues));
    timeLockIndexes[opCode] = (_timeLockInfos.length - 1);
