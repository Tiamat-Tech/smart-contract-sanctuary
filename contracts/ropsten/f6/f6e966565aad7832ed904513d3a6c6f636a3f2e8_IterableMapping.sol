...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function getETHDividends(address holder) external view returns (uint256){
  ...
  function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
        if (amountForCharity > 0){
         ...
         // if no custom reward token or reward token is whitelisted, send ETH.
...
contract FindShibby is ERC20, Ownable {
    ...
    event FeesUpdated(uint256 indexed newETHRewardsFee, uint256 indexed newLiquidityFee, uint256 newOperationsFee, uint256 newBuyBackFee);
    ...
    // shows total amount in ETH gifted to charity.
    ...
    // shows % in ETH gifted to charity.
    ...
    function getETHDividends(address holder) public view returns (uint256){
        return dividendTracker.getETHDividends(holder);
    }
    
    function getETHAvailableForHolderBuyBack(address holder) external view returns (uint256){
        return getETHDividends(holder).sub(holderETHUsedForBuyBacks[msg.sender]);
    }
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        uint256 userETHDividends = getETHDividends(msg.sender);
        require(userETHDividends >= holderETHUsedForBuyBacks[msg.sender].add(msg.value), "Findshibby: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function swapBack() private {
        ...
        uint256 ethForRewards = ethBalance.mul(_rewardTokensToSwap).div(totalTokensToSwap);
        
        uint256 ethForLiquidity = ethBalance - ethForMarketing - ethForBuyBack - ethForCharity - ethForRewards;
        ...
        (bool success,) = address(operationsWallet).call{value: ethForMarketing}("");
        (success,) = address(charityWallet).call{value: ethForCharity}("");
        (success,) = address(dividendTracker).call{value: ethForRewards}("");
        (success,) = address(shibbyWallet).call{value: ethForBuyBack}("");
