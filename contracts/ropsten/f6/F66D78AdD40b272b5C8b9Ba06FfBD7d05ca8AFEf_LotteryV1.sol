...
contract LotteryV1 is AccessControl, Pausable {
    ...
    event BathConvertNFT(uint256[] indexed _tokenIds,uint256 indexed _amount,address indexed _from,string nonce ,string _convertType);
     ...
     * @param nonce 交易随机数
     */
    function convertNFT2ETH(uint256 _tokenId,uint256 _ETHAmount ,bytes32 hash, bytes memory signature,string memory nonce)public{
        //验证hash
     ...
     * @param nonce 交易随机数
     */
    function bathConvertNFT2ETH(uint256[] memory _tokenIdArray,uint256 _ETHAmount ,bytes32 hash, bytes memory signature,string memory nonce)public{
        //验证hash
        ...
        emit BathConvertNFT(_tokenIdArray,_ETHAmount,msg.sender,nonce,"ETH");
    }
...
//        require(_token1Number >= _token1Amount);
    ...
    function bathConvertNFT2Ticket(uint256[] memory _tokenIdArray,uint256 _ticketAmount ,bytes32 hash, bytes memory signature,string memory nonce)public{
        ...
        //销毁奖品
        require(_bathBurnNFT(_tokenIdArray),"burnNFT failed!");
        emit BathConvertNFT(_tokenIdArray,_ticketAmount,msg.sender,nonce,"Ticket");
    }
