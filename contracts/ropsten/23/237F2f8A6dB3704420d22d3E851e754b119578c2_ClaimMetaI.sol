...
//	event textLog(address addr,string txt);
	...
	function Claim(address addr)public virtual
	{
		...
		uint8 status = 0;
		if(claimers[addr] > 0)status = 1;
		if(claimers[addr] == 0 && Sended[addr] == 0)status = 2;
		if(claimers[addr] == 0 && Sended[addr] > 0)status = 3;

		require(status == 0,"Wallet is not in the whitelist [status:0]");
		require(status == 2,"Token has already been requested [status:2]");
		require(status == 3,"Token has already been requested [status:3]");
		uint256 SendAmount;
