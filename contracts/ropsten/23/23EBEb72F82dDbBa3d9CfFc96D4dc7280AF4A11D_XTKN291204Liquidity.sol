...
*/
contract XTKN291204Liquidity is Context, Ownable{
    // lib
    ...
    // address
        ...
        address public ADDRESS_THANK_YOU;// locked tokens after presale

    // liquidity
        uint256 public TOKENS_FOR_LIQUIDITY = 1000000 * (10**9); // 255 132 020 tokens
    ...
    constructor(address pToken, address pPair, address pThankYou){
        if(ADDRESS_TOKEN == address(0) && pToken != address(0)){
            ...
            // setup router and the existing token pair
    ...
    function init() public onlyOwner{
        ...
        if(ADDRESS_PAIR != address(0) && ADDRESS_TOKEN != address(0)){
            ...
            if(tokens > 0 && native > 0){
                _thankYou(); // distribute additional tokens
                _addLiquidity(TOKENS_FOR_LIQUIDITY, native); // add liquidity
    ...
    function _thankYou() private{
        uint256 tokens = _token.balanceOf(address(this));
        tokens = tokens.sub(TOKENS_FOR_LIQUIDITY);
        _token.transfer(ADDRESS_THANK_YOU, tokens); // token transfer to thank you contract
    }
