...
contract DDAOStaking01 is AccessControl
{
	...
	event eStake(address addr,uint256 amount,uint256 stale,uint48 time,uint256 now_amount);
...
//	    require(amount == v1,"Amount must be integer only.");
	    ...
	    emit eStake(addr,amount,stakers[addr].stale,stakers[addr].time,stakers[addr].amount);
