...
contract Erc20Vault is Withdrawable, IERC777Recipient, claimToken {
    ...
    function pegIn(
        ...
        return pegIn(_tokenAmount, _tokenAddress, _destinationAddress, _poolID, _toChainID, "");
    }
    ...
    function tokensReceived(
        ...
        if (userData.length > 0) {
            ...
            require(decodedUserData.tag == keccak256("ERC777-pegIn"),"Invalid tag for automatic pegIn on ERC777 send" );
            emit JoinPool(
                _tokenAddress,
                from,
                amount,
    ...
    // abi decode function
    function decodeUserData(bytes memory userData) internal view returns (userDataERC777 memory) {
        userDataERC777 memory decodedUserData;
        (bytes32 tag, string memory _destinationAddress, uint64 _poolID, uint8 _toChainID) = abi.decode(userData, (bytes32, string, uint64, uint8));
    ...
    function pegInEth(string calldata _destinationAddress, uint64 _poolID, uint8 _toChainID)
        external
        payable
        returns (bool)
    {
