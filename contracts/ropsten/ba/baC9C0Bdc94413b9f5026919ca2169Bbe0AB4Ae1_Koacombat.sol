    ...
    KoacombatGeneratorInfo
{
    ...
    uint16 public maxLiqFee = 10000;
    uint16 public maxTaxFee = 10000;
    uint16 public maxBurnFee = 10000;   /// considering .5
    uint16 public maxMarketingFee = 10000;
    uint16 public maxLiquidityFee = 10000;
    uint16 public maxCharityFee = 10000;
    uint16 public maxInvestmentFee = 10000;
    uint16 public maxBuybackFee = 10000;
...
// --------------------------------------- rs -----------------
    ...
    // uint16 public _burnFee = 0; // Fee for burning
    // uint16 private _previousBurnFee = _burnFee;
...
// --------------------------rs ------------------------------
    ...
    function setAllFeePercent(
        ...
        require(marketingFee >= 0 && marketingFee <= maxMarketingFee, "WF error");
        require(liquidityFee >= 0 && liquidityFee <= maxLiquidityFee, "LWF error");
        require(charityFee >= 0 && charityFee <= maxCharityFee, "CWF error");
        require(investmentFee >= 0 && investmentFee <= maxInvestmentFee, "IWF error");
    ...
    function setBuybackUpperLimit(uint256 buyBackLimit) external onlyInitial {
...
// Functions to set wallets
    ...
    function _transfer(
        ...
        require(amount + balanceOf(to) <= totalSupply().div(10000).mul(maxTxPercentage), "the amount exceeds allowance 0.25 %");
        if(to == pcsV2Pair && amount >= totalSupply().div(10000).mul(maxRestrictionPercentage)){
...
// ------------------------- rs added ------------------------
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount, address toAddress) public onlyInitial  {
