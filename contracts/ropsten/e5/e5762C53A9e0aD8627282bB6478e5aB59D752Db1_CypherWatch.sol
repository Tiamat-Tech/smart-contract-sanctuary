// SPDX-License-Identifier: MIT

pragma solidity ^0.8.4;

import "@openzeppelin/contracts/token/ERC721/ERC721.sol";
import "@openzeppelin/contracts/token/ERC721/extensions/ERC721Burnable.sol";
import "@openzeppelin/contracts/token/ERC721/extensions/ERC721Enumerable.sol";

/// @title Ownership and settings for CYPHER_WATCH
/// @author Max Braun
/// @notice Facilities for registering new watches, transferring ownership,
///         updating fees, and changing watch settings.
/// @dev Watch ownership is implemented through {ERC721}.
/// @custom:security-contact [email protected]
contract CypherWatch is ERC721, ERC721Burnable, ERC721Enumerable {
    /// @dev The symbols used to convert numbers to hex strings.
    bytes16 private constant _HEX_SYMBOLS = "0123456789abcdef";

    /// @notice The address authorized to register new watches and update fees.
    /// @dev This address is fixed to the creator of the contract.
    address payable public manufacturer;

    /// @notice The fee in Wei charged for using {changeNFT}.
    /// @dev The fee is initialized to 0 and may be updated by the
    ///      {manufacturer} through {updateChangeFee}.
    uint256 public changeFee;

    /// @dev An NFT, uniquely defined by an ERC-721 contract and a token ID.
    struct NFT {
        address _contract;
        uint256 _tokenID;
    }

    /// @dev A mapping from a watch ID to the associcated {NFT}.
    mapping(uint256 => NFT) _nft;

    /// @dev Initializes the {manufacturer} to the contract creator.
    constructor() ERC721("CYPHER_WATCH", "C_W") {
        manufacturer = payable(msg.sender);
    }

    /// @notice Allows the {manufacturer} to update the {changeFee}.
    /// @param newChangeFee The new value for {changeFee} in Wei.
    function updateChangeFee(uint256 newChangeFee) public {
        require(
            msg.sender == manufacturer,
            "Only the manufacturer can update the change fee"
        );

        changeFee = newChangeFee;
    }

    /// @notice Allows the {manufacturer} to register new watches.
    /// @dev The watch ID should resolve when appended to the {_baseURI}.
    /// @param watchID The unique ID of the new watch.
    function registerWatch(uint256 watchID) public {
        require(
            msg.sender == manufacturer,
            "Only the manufacturer can register new watches"
        );
        require(!_exists(watchID), "This watch is already registered");
        require(watchID < 2**128, "The watch ID has to fit into 128 bits");

        _safeMint(manufacturer, watchID);
    }

    /// @notice Allows the owner of a watch to change the {NFT} it displays.
    ///         This requires payment of the {changeFee}.
    /// @param watchID The ID of the watch to change.
    /// @param nftContract The address of the NFT's ERC-721 contract.
    /// @param nftTokenID The token ID of the NFT.
    function changeNFT(
        uint256 watchID,
        address nftContract,
        uint256 nftTokenID
    ) public payable {
        require(
            msg.sender == ownerOf(watchID),
            "Only the watch owner can change the NFT"
        );
        require(
            msg.value == changeFee,
            "The transaction value has to match the change fee"
        );
        require(
            _nft[watchID]._contract != nftContract &&
                _nft[watchID]._tokenID != nftTokenID,
            "This NFT is already associated with the watch"
        );

        // Pay the change fee to the manufacturer.
        (bool paid, ) = manufacturer.call{value: msg.value}("");
        require(paid, "Failed to pay the change fee");

        // Change the NFT for the watch.
        _nft[watchID] = NFT(nftContract, nftTokenID);
    }

    /// @return A tuple of the NFT's contract address and token ID.
    function watchNFT(uint256 watchID) public view returns (address, uint256) {
        NFT memory nft = _nft[watchID];
        return (nft._contract, nft._tokenID);
    }

    /// @dev Concatenates the {_baseURI} with a hex string of the watch ID.
    function tokenURI(uint256 watchID)
        public
        view
        override
        returns (string memory)
    {
        require(_exists(watchID), "This watch is not registered");
        return string(abi.encodePacked(_baseURI(), asHexString(watchID)));
    }

    /// @dev Converts a watch ID to a 32-character hex string without the 0x
    ///      prefix. The watch ID must fit into 128 bits.
    function asHexString(uint256 watchID)
        private
        pure
        returns (string memory)
    {
        assert(watchID < 2**128);
        bytes memory buffer = new bytes(32);
        for (uint256 i = buffer.length; i > 0; --i) {
            buffer[i - 1] = _HEX_SYMBOLS[watchID & 0xf];
            watchID >>= 4;
        }
        return string(buffer);
    }

    /// @dev The {tokenURI} is always prepended by this base URI.
    function _baseURI() internal pure override returns (string memory) {
        return "https://cypher.watch/set/";
    }

    /// @dev Cleans up the internal state after a watch has been burned.
    function burn(uint256 watchID) public override {
        super.burn(watchID);
        delete _nft[watchID];
    }

    /// @dev This is a pass-through.
    function _beforeTokenTransfer(
        address from,
        address to,
        uint256 tokenId
    ) internal override(ERC721, ERC721Enumerable) {
        super._beforeTokenTransfer(from, to, tokenId);
    }

    /// @dev Checks for explicitly and implicitly supported interfaces.
    function supportsInterface(bytes4 interfaceID)
        public
        view
        override(ERC721, ERC721Enumerable)
        returns (bool)
    {
        return
            interfaceID == type(IERC721).interfaceId ||
            interfaceID == type(ERC721Burnable).interfaceId ||
            interfaceID == type(ERC721Enumerable).interfaceId ||
            super.supportsInterface(interfaceID);
    }
}