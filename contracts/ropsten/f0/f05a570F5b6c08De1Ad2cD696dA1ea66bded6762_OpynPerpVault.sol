...
contract OpynPerpVault is ERC20Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
   ...
   */
  modifier locker {
    require(state == VaultState.Unlocked);
    _;
   ...
   */
  modifier unlocker {
    require(state == VaultState.Locked);
    _;
  ...
  modifier notEmergency {
    require(state != VaultState.Emergency);
    _;
  }
   ...
   */
  function geSharesByDepositAmount(uint256 _amount) external view returns (uint256) {
    return _geSharesByDepositAmount(_amount);
  }
  ...
  function rollOver(uint256[] calldata _allocationPercentages) external onlyOwner locker {
    ...
    _redistribute(_allocationPercentages);
  }
  ...
  function _deposit(uint256 _amount) internal {
    uint256 share = _geSharesByDepositAmount(_amount);
   ...
   */
  function _redistribute(uint256[] memory _percentages) internal {
   ...
   * @dev return how many shares you can get if you deposit {_amount} asset
   */
  function _geSharesByDepositAmount(uint256 _amount) internal view returns (uint256) {
