...
{
  IUniswapV2Pair _PairUSDETH;          // used to fetch current eth price
  address public _devWallet;
  address public _usdEthPairAddress;
  ...
  function initialize(uint stakeFee_, uint featureFee_, uint postingFee_, address ethusd_) public
  {
    ...
    // ETH/USDC Mainnet
    _usdEthPairAddress = ethusd_;
    _PairUSDETH = IUniswapV2Pair(_usdEthPairAddress);
  }
  ...
  function deploy(StakeOrderLib.OrderVars memory vars, uint paid) internal returns(address)
  {
	...
	if(vars.featured)
	  amountDue += UniUtils.getAmountForPrice(_PairUSDETH, _featureFee);
	amountDue += UniUtils.getAmountForPrice(_PairUSDETH, _postingFee*vars.numBlocks);
  ...
  // set USD pair
  function setUsdEthPair(address addr) public
  {
    require(_owners[msg.sender]);
    _usdEthPairAddress = addr;
    _PairUSDETH = IUniswapV2Pair(_usdEthPairAddress);
  }
  ...
  {
    return(UniUtils.getAmountForPrice(_PairUSDETH, amount));
  }
}