...
contract ETHLeverage is Ownable {
    ...
    function currentPosition() external onlyOwner returns (uint256) {
        uint256 cETHBalance = cETH.balanceOf(address(this));
        ...
        if (currentETHDeposited == 0) {
            console.log("Current Position: ", 0);
            return 0;
        }

        uint256 currentWETH = IWETH(WETH).balanceOf(address(this));
        uint256 position = ((currentWETH.add(currentETHDeposited)).mul(1000)).div(currentETHDeposited);
