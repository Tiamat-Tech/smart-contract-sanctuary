...
contract CampaignManager is AccessControl {
        ...
        checkRole(CAMPAIGN_ADMIN_ROLE, msg.sender, "Caller is not campaign admin")
    {
     ...
     function updateCampaign(uint _index, bytes32 _name, bytes32 _artistName,bytes32 _imageURL, address _artistAddr, uint8 _numNFTs, uint _rewardPerBlock) 
        external 
        checkRole(CAMPAIGN_ADMIN_ROLE, msg.sender, "Caller is not campaign admin")
    {
        campaigns[_index].name = _name;
        campaigns[_index].artistName = _artistName;
        campaigns[_index].artistAddress = _artistAddr;
        campaigns[_index].imageURL = _imageURL;
        campaigns[_index].numNFTs = _numNFTs;
        campaigns[_index].rewardPerBlock = _rewardPerBlock;
    }
