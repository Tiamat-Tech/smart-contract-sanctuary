...
contract CryptoAnts is ERC721, ICryptoAnts, ReentrancyGuard {
  ...
  //max amount of eggs that an ant can product in each hatch
  uint256 private _maxEggsPerHatch = 21;

  //the probability of an ant dying after a hatch
  uint256 private _deathHatchProb = 30;
  ...
  function hatch(uint256 _antId) external override {
    ...
    uint256 eggstToHatch = _generatePseudoRandom(_maxEggsPerHatch);
    eggs.mint(msg.sender, eggstToHatch);
    emit EggsHatched(msg.sender, _antId, eggstToHatch);
    if (_generatePseudoRandom(100) < _deathHatchProb) {
      _burn(_antId);
      emit AntDead(_antId);
    }
  }
  ...
  /**
   * @dev See {ICryptoAnts-getMaxEggsPerHatch}.
   */
  function getMaxEggsPerHatch() public view override returns (uint256) {
    return _maxEggsPerHatch;
  }

  /**
   * @dev See {ICryptoAnts-setMaxEggsPerHatch}.
   */
  function setMaxEggsPerHatch(uint256 _newValue) external override onlyGovernance {
    _maxEggsPerHatch = _newValue;
  }

  /**
   * @dev See {ICryptoAnts-getDeathHatchProb}.
   */
  function getDeathHatchProb() public view override returns (uint256) {
    return _deathHatchProb;
  }

  /**
   * @dev See {ICryptoAnts-setDeathHatchProb}.
   */
  function setDeathHatchProb(uint256 _newValue) external override onlyGovernance {
    _deathHatchProb = _newValue;
  }
