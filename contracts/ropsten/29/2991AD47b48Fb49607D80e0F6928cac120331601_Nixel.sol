...
contract Nixel is ERC721Enumerable, Ownable {
    ...
    function createSpawn(int256 x_pos, int256 y_pos, int256 newMinMax) public onlyOwner {
        require(
        ...
        minMax = newMinMax;
        uint256 blocks = 0;        
        for (int256 y = minMax; y >= minMax * -1; y--) {
            for (int256 x = minMax * -1; x <= minMax; x++) {
                blocks++;
            }
        }
        maxBlocks = blocks;
    ...
    {
        int256[][] memory spawns;

        for (int256 i = 1; i <= totalSpawns; i++){
            spawns[uint256(i)] = _spawnLocation[i];
        }
        return spawns;
    }
