...
contract NFTMarketplace is Ownable, Withdrawable {
    ...
    ///      There is no reason why the new owner of a token would need to revoke the prior owner's ineffectual offer.
    ///      But we provide this option anyway because we recognize the token market to be the prerogative of that
    ///      token's owner.
    /// @param tokenId Which token
    ...
    /// @param tokenId Which token
    function bid(uint256 tokenId) payable external {
    ...
    /// @param tokenId Which token
    function bidIncrease(uint256 tokenId) payable external {
    ...
    function setFeePortion(uint256 newFeePortion) onlyOwner external {
