...
contract sBombToken is ERC20, Ownable {
    ...
    function noFeeAddLiquidity(
        ...
        require(token1 != address(this) && token1 != address(0));
            ...
            if (_pairCheck(sender)) {
                ...
                if (
                    ...
                    if (_pairExisting(path)){
                        ...
                        payable(lotteryContract).transfer(address(this).balance);
                    }
                } else {
                ...
                path[1] = dexRouter.WETH();
                if (_pairExisting(path)) {
                    super._transfer(sender, address(this), toTeam + toLiquidity);
                    _approve(address(this), address(dexRouter), toTeam + toLiquidity);
                    _swapTokensForEth(toTeam, teamWallet, path);

                    IUniswapV2Pair pair = IUniswapV2Pair(IUniswapV2Factory(dexRouter.factory()).getPair(path[0], path[1]));
                    (uint256 reserve0, uint256 reserve1, ) = pair.getReserves();
                    uint256 half = getOptimalAmountToSell(int(address(this) == pair.token0() ? reserve0 : reserve1), int(toLiquidity)); //toLiquidity / 2;
