...
contract VeeProxyController is AccessControl, IVeeProxyController {    
    ...
    function executeOrder(bytes32 orderId) external returns (bool){
        ...
        if(price >= orders[orderId].stopHighPairPrice || price <= orders[orderId].stopLowPairPrice){
            ...
            if (newAmountA > amounts[1]) {
                profit = newAmountA.sub(amounts[1]);
            }
    ...
    function cancelOrder(bytes32 orderId) external returns(bool){
        ...
        require(hasRole(EXECUTOR_ROLE, msg.sender)||msg.sender == orders[orderId].orderOwner);

        IERC20 erc20TokenA = IERC20(orders[orderId].tokenA);
        erc20TokenA.transfer(orders[orderId].orderOwner, orders[orderId].amountA);
        delete orders[orderId];
        emit OnOrderCanceled(orderId, orders[orderId].amountA);
