...
// testing tool in order to log when testing with hardhat using console.log
...
contract Binaries is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getAllBits(uint256 tokenId) public view returns (string memory) {
        require(_exists(tokenId),"This token has not been minted yet!");
        require(_msgSender() == ownerOf(tokenId) || _msgSender() == owner(), "You do not own this token");
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        parts[0] = string(abi.encodePacked('<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: "', tokenFont, '"; font-size: 14px; font-weight: light}</style><rect width="100%" height="100%" fill="black" /><text x="10" y="20" class="base">'));
    ...
    function getFont() public view onlyOwner returns (string memory){
        return tokenFont;
    }

    function setFont(string memory _newFont) public onlyOwner() {
        tokenFont = _newFont;
    }
