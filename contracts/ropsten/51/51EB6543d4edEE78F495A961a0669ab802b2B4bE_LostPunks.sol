    ...
    function metadataAttributes(bytes memory punkAssets, uint16 index) internal view returns (string memory text) {
        ...
        for (uint8 j = 0; j < 8; j++) {
            ...
            if (asset > 0) {
                if (j > 0) {
                    ++accessoryCount;
                    text = string(abi.encodePacked(', { "trait_type": "Accessory", "value": "', getAssetName(asset), '" }'));
                } else {
                    text = string(abi.encodePacked('{ "trait_type": "Type", "value": "', getAssetName(asset), '" }'));
                }
            } else {
                break;
            }
        }
        text = string(abi.encodePacked(', { "trait_type": "# Traits", "value": ', toString(accessoryCount), ' }'));
        text = string(abi.encodePacked(', { "trait_type": "Generation", "value": ', toString(generationMapping[index]), ' }'));
        uint16 sibling = siblingMapping[index]; 
        if (sibling > 0) {
            text = string(abi.encodePacked(', { "trait_type": "Father", "value": ', toString(fatherMapping[index]), ' }'));
            text = string(abi.encodePacked(', { "trait_type": "Mother", "value": ', toString(motherMapping[index]), ' }'));
            text = string(abi.encodePacked(', { "trait_type": "Sibling", "value": ', toString(sibling), ' }'));
        }
        uint16 child1 = child1Mapping[index]; 
        if (child1 > 0) {
            text = string(abi.encodePacked(', { "trait_type": "Child 1", "value": ', toString(child1), ' }'));
        }
        uint16 child2 = child2Mapping[index]; 
        if (child2 > 0) {
            text = string(abi.encodePacked(', { "trait_type": "Child 2", "value": ', toString(child2), ' }'));
        }
    }
