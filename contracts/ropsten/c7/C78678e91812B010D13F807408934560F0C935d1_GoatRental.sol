...
contract GoatRental is GoatAuctionBase, ReentrancyGuard {
    ...
    event LogRentOut(uint256 indexed rentalId, RentalType rentalType, uint256 orderId);
        ...
        nonReentrant 
    {
        ...
        RentOutInfo memory rentOutInfo = RentOutInfo(
            order.seller, msg.sender, order.token, order.ids, order.amounts, order.currency, order.price, RentalType.limitedPriceRental, _orderId
        );
        uint256 actualPrice = _rentOut(rentOutInfo);
        ...
        nonReentrant 
    {
        ...
        if (order.highestPriceBuyer != address(0)) {
            RentOutInfo memory rentOutInfo = RentOutInfo(
                order.seller, order.highestPriceBuyer, order.token, order.ids, order.amounts, order.currency, order.highestPrice, RentalType.englishRenta, _orderId
            );
            uint256 actualPrice = _rentOut(rentOutInfo);
    ...
    function _rentOut(
        RentOutInfo memory rentOutInfo
    ) 
        internal
        returns (uint256 actualPrice)
    {
        ...
        uint256 _rentalTerm;
        if (rentOutInfo.rentalType == RentalType.limitedPriceRental) {
            _rentalTerm = limitedPriceOrderRentalTerm[rentOutInfo.orderId];
        } else {
            _rentalTerm = englishOrderRentalTerm[rentOutInfo.orderId];
        }

        IERC1155(rentOutInfo.token).setApprovalForAll(goatStatus.rentalWrapperAddress(), true);
        uint256[] memory wrappedIds = IGoatRentalWrapper(goatStatus.rentalWrapperAddress()).wrap(rentOutInfo.owner, rentOutInfo.token, rentOutInfo.ids, rentOutInfo.amounts, rentOutInfo.renter, _rentalTerm);

        uint256 repossessFee = rentOutInfo.price.mul(repossessFeeRate).div(repossessFeeRateBase);
        actualPrice = rentOutInfo.price.sub(repossessFee);
        ...
        for (uint256 i = 0; i < rentOutInfo.ids.length; i++) {
            if (rentOutInfo.token != goatStatus.rentalWrapperAddress()){
                originRentalOrder[wrappedIds[i]] = orderId;
            } else {
                uint256 originRentalOrderId = originRentalOrder[rentOutInfo.ids[i]];
        ...
        emit LogRentOut(orderId, rentOutInfo.rentalType, rentOutInfo.orderId);
    }
