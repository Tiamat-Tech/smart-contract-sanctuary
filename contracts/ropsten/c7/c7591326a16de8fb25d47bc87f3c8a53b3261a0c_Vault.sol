...
contract Vault is IVault, PausableUpgradeable, AccessControlEnumerableUpgradeable {
    ...
    function initialize(address _rhoAddr, address _underlyingAddr, uint256 _mintingFee, uint256 _redeemFee, uint256 _rLowerBound, uint256 _rUpperBound ) public initializer {
    ...
    function mint(uint256 amount) external override whenNotPaused {
        ...
        uint256 amountInRho = amount * 1e18 / _underlyingOne;
        // after mint, check reserve balance against upperbound
        // uint256 reserveBalance = _underlying.balanceOf(address(this));
        // uint256 totalSupplyToBe = _rho.totalSupply() + amountInRho;
        // if (reserveBalance < _calcUpperBoundReserve(totalSupplyToBe)) {
        _rho.mint(_msgSender(), amountInRho);
        return;
        // }
        // reserve balance exceeds upper bound, allocate such that the reserve drops back to lower bound
        // uint256 diff = reserveBalance - _calcLowerBoundReserve(totalSupplyToBe);
        // _allocate(diff);
        // _rho.mint(_msgSender(), amountInRho);
    }
    ...
    function rebase() external override onlyRole(REBASE_ROLE) whenNotPaused {
        ...
        uint256 underlyingTotal = underlyingUninvested + underlyingInvested;

        uint256 rhoRebasing = _rho.rebasingSupply();
        uint256 rhoNonRebasing = _rho.nonRebasingSupply();
        uint256 rhoTotalSupply = _rho.totalSupply();
        uint256 newM = ( ( underlyingTotal * _rhoOne / _underlyingOne ) - rhoNonRebasing) * 1e18 / rhoRebasing;
    ...
    function sweepRhoTokenContractERC20Token(address token, address to)external override onlyRole(SWEEPER_ROLE){
