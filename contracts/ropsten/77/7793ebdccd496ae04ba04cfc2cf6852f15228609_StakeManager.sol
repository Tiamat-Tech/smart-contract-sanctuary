...
contract StakeManager is IStakeManager, StakeBase {
    ...
    */
    function createStake(
        ...
        if (stakingDays >= config.bpdDayRange) {
    ...
    function createExistingStake(
        ...
        uint256 stakingDays = (end - start) / config.secondsInDay;
    ...
    function upgradeExistingStakeInternal(StakeUpgrade memory stakeUpgrade) internal {
        ...
        uint256 intendedEnd = stakeUpgrade.start + (config.secondsInDay * stakeUpgrade.stakingDays);

        // We use "Actual end" so that if a user tries to withdraw their BPD early they don't get the shares
        if (stakeUpgrade.stakingDays >= config.bpdDayRange) {
        ...
        uint256 newEnd = block.timestamp + (config.secondsInDay * 5555);

        addBpdMaxShares(
            stakeUpgrade.shares,
            stakeUpgrade.start,
            stakeUpgrade.start + (config.secondsInDay * stakeUpgrade.stakingDays),
            newShares,
            block.timestamp,
            newEnd
        );
    ...
    function unsetLegacyStake(
        ...
        (uint256 payout, uint256 penalty) =
            deleteStakeInternal(staker, id, shares, amount, start, firstInterestDay, stakingDays);
    ...
    function deleteStakeInternal(
        ...
        subtractFromGlobalTotals(amount, shares);

        uint256 interest = getStakeInterestInternal(firstInterestDay, stakingDays, shares);

        // We use "Actual end" so that if a user tries to withdraw their BPD early they don't get the shares
        if (stakingDays >= config.bpdDayRange) {
            uint256 intendedEnd = start + (config.secondsInDay * stakingDays);
    ...
    */
    function getPayoutAndPenaltyInternal(
        ...
        uint256 stakingSeconds = stakingDays * config.secondsInDay - start;
        uint256 secondsStaked = block.timestamp - start;
        uint256 daysStaked = secondsStaked / config.secondsInDay;
    ...
    function getStakeEnd(uint256 id) external view override returns (uint256) {
        return stakeData[id].start + (config.secondsInDay * stakeData[id].stakingDays);
    }
    ...
    */
    ...
    function setConfig(
        ...
        if (_shareRateScalingFactor != 0) config.shareRateScalingFactor = _shareRateScalingFactor;
        if (_secondsInDay != 0) config.secondsInDay = _secondsInDay;
        if (_contractStartTimestamp != 0) config.contractStartTimestamp = _contractStartTimestamp;
        if (_bpdDayRange != 0) config.bpdDayRange = _bpdDayRange;
    }
    ...
    */
    function addDailyInterest() public {
        ...
        if (interestPerShare.length != 0) {
            ...
            todaysSharePayout = (interest * (10**26)) / ((statFields.sharesTotalSupply * 1e12) + 1);
        }
    ...
    function getTodaysInterest() internal returns (uint256) {
        ...
        return todaysBalance + inflation;
    }
    ...
    */
    function updateShareRate(uint256 _payout) internal {
        ...
        if (config.shareRateScalingFactor == 0) {
            //use a shareRateScalingFactor which can be set in order to tune the speed of shareRate increase
            config.shareRateScalingFactor = 1e18;
        }

        interestFields.shareRate = (
            ((interestFields.shareRate *
                ((10**36) + (config.shareRateScalingFactor) * growthFactor)) / 10**36)
        )
    ...
    /** Sub from Global Totals
        @param amount {uint256}
        @param shares {uint256}
     */
    function subtractFromGlobalTotals(uint256 amount, uint256 shares) internal {
        /** Set Global Variables */
        statFields.sharesTotalSupply -= (shares / 1e12).toUint72();
    ...
    */
    ...
    function getDaysFromStart() external view returns (uint256) {
        return (block.timestamp - config.contractStartTimestamp) / config.secondsInDay;
    }
    ...
    function addBpdShares(
        ...
        uint256 end = start + (stakingDays * config.secondsInDay);
    ...
    function getBpdInterval(uint256 start, uint256 end) internal view returns (uint16[2] memory) {
        uint16[2] memory bpdInterval;
        uint256 denom = config.secondsInDay * config.bpdDayRange;

        bpdInterval[0] = uint16(
            MathUpgradeable.min(5, (start - config.contractStartTimestamp) / denom)
        ); // (start - t0) // 350
    ...
    function restore(
        ...
        config.shareRateScalingFactor = _shareRateScalingFactor;
        config.secondsInDay = _secondsInDay;
        config.contractStartTimestamp = _contractStartTimestamp;
        config.bpdDayRange = _bpdDayRange;
