...
contract VeeProxyController is IVeeProxyController, VeeSystemController, Initializable{    
   ...
   IUniswapV2Router02 private _router;
     ...
     * @return orderId 
     */
    function createOrder(address orderOwner, address ctokenA, address tokenA, address tokenB, uint256 amountA, uint256 stopHighPairPrice, uint256 stopLowPairPrice, uint256 expiryDate, bool autoRepay,uint256 maxSlippage) external veeLock(uint8(VeeLockState.LOCK_CREATE)) override returns (bytes32 orderId){
    ...
    function createOrder(address orderOwner, address cETH,  address tokenB, uint256 stopHighPairPrice, uint256 stopLowPairPrice, uint256 expiryDate, bool autoRepay,uint256 maxSlippage) external override veeLock(uint8(VeeLockState.LOCK_CREATE)) payable returns (bytes32 orderId){
        require(orderOwner != address(0), "createOrder: invalid order owner");
...
//for test only
   ...
   function getRandom() private returns (uint256) {
     ...
     *
     */
    function swap(address tokenA,uint256 amountA, address tokenB,uint256 maxSlippage) internal returns (uint256[] memory){
