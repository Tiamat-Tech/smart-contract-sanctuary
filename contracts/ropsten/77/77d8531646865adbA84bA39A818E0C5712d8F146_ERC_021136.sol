...
// main contract []
contract ERC_021136 is Context, IERC20, Ownable, ReentrancyGuard {
    // public properties
        // extend
        ...
        // addresses
            // contracts
                ERC_021136_LIQUIDITY private _contractLiquidity;
    ...
    // private properties
        // tokenomics
            string private _name = "ERC_021136";
            string private _symbol = "ERC021136";
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns(bool){
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "ERC_021136: decreased allowance below zero"));
        return(true);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(bool){
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "ERC_021136: transfer amount exceeds allowance"));
        return(true);
    }
    ...
    // setter
        function activateContract(address addressLiquidity) external onlyOwner{
            activateContractLiquidity(addressLiquidity, balanceOf(address(this)), address(this).balance); //send all tokens and native to liquidity contract
        }

        // all things liquidity
            function activateContractLiquidity(address _contract, uint256 tokens, uint256 native) public onlyOwner returns(bool){
                ADDRESS_LIQUIDITY = _contract;
                _contractLiquidity = ERC_021136_LIQUIDITY(ADDRESS_LIQUIDITY);
                ADDRESS_UNISWAP_PAIR = _contractLiquidity.getPair();
                
                if(tokens > 0){
                    _transferTokens(address(this), ADDRESS_LIQUIDITY, tokens);
                }
                if(native > 0){
                    _transferNative(address(this), ADDRESS_LIQUIDITY, native);
                }
                
                bool status = _contractLiquidity.activate();
                emit EventContractActivation(_contract, status);
                return(status);
            }
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "ERC_021136: approve from the zero address");
        require(spender != address(0), "ERC_021136: approve to the zero address");
    ...
    function _transfer(address from, address to, uint256 amount) private{
        ...
        if(from != ADDRESS_UNISWAP_PAIR){
            _contractLiquidity.addLiquidity();
        }

        /*
        if(!_lockSwapAndLiquifyEngaged && from != ADDRESS_UNISWAP_PAIR){
    ...
    function _transferNative(address from, address to, uint256 amount) private returns(bool){
