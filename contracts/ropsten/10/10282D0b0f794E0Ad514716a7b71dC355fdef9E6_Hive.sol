...
contract Hive is Ownable, IERC721Receiver, Pausable {
  ...
  mapping(uint256 => mapping(uint256 => Stake)) public hive;
  ...
  function addManyToHive(
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      ...
      _addBeeToHive(account, hiveId, tokenIds[i]);
    }
  }
  ...
  function _addBeeToHive(
    ...
    hive[hiveId][tokenId] = Stake({ owner: account, tokenId: uint16(tokenId), value: uint80(block.timestamp) });
    totalBeesStaked += 1;
  ...
  function _claimBeeFromHive(
    ...
    Stake memory stake = hive[hiveId][tokenId];
    ...
    if (newHiveId < 0) {
      ...
      delete hive[hiveId][tokenId];
      totalBeesStaked -= 1;
    } else {
      hive[hiveId][tokenId] = Stake({ owner: _msgSender(), tokenId: uint16(tokenId), value: uint80(block.timestamp) }); // reset stake
    }
