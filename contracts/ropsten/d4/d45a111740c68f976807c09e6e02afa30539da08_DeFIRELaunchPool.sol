...
contract DeFIRELaunchPool is Ownable, ReentrancyGuard {
    ...
    event pFAIRPurchased(address sender, uint256 investamount, uint256 pdefireamount);
    event dFAIRPurchased(address sender, uint256 investamount, uint256 ddefireamount);
    ...
    constructor(
        ...
        require(_pSaleTokenDegen != address(0), "pSaleTokenDegen address cannot be 0x");
        require(_treasuryPDeFIRE != address(0), "treasuryPDeFIRE address cannot be 0x");
    ...
    function PreSaleTokenDegenAvailable() view public onlyOwner returns ( uint ) {
      
            uint deposits = ERC20(pSaleTokenDegen).balanceOf(address(this));
            return deposits;
    ...
    function checkInvestorDetails(address _investor) public view returns(uint,uint,uint){     

        InvestorStats storage investor = investorMatrix[_investor];   

        return(investor.pSaleTokens, investor.pSaleDegenTokens, investor.amountInvested);        
    ...
    function buyPDeFIRE(uint256 _investAmount) external nonReentrant {
        ...
        uint pDeFIRELeft = ERC20(pSaleToken).balanceOf(address(this));

        uint currentBuyAllowance = investorCurrentBuyAllowance();

        InvestorStats storage investor = investorMatrix[msg.sender];

        require(msg.sender != treasuryPDeFIRE, "Treasury address cannot partake in presale");
        require(block.timestamp >= pStartTime , "Presale hasn't started yet, good things come to those who wait");
        require(block.timestamp < pEndTime, "Presale has ended! Thank you for your support!");
        ...
        require(_investAmount < currentBuyAllowance, "reached buy allowance limit for now");
        require(investor.amountInvested < currentBuyAllowance, "reached buy allowance limit for now");
    ...
    function buyDDeFIRE(uint256 _investAmount) external nonReentrant {
        ...
        uint dDeFIRELeft = ERC20(pSaleTokenDegen).balanceOf(address(this));
        ...
        require(investor.pSaleDegenTokens <= maxCapacityDegen  , "You have reached your max capacity for this token");
        ...
        assert(ERC20(pSaleTokenDegen).decimals() == ERC20(investToken).decimals());
    ...
    function withdrawUnclaimedToTreasury() external onlyOwner {
        ...
        require(pRemaining.add(dRemaining) > 0 , "No unclaimed tokens, nice job");
