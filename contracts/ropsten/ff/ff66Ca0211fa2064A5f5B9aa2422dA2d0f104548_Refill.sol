...
contract Refill is AccessControl, Pausable, Utils {
    ...
    function batchWithdraw(uint256[] memory _amountArray,address[] memory _contractAddressArray, uint256[] memory _tokenIdArray,  address[] memory _fromAddressArray, bytes32 hash, bytes memory signature, string memory nonce) public whenNotPaused{
        ...
        //验证签名
        require(matchAddressSigner(hash, signature), "Invalid signature!");
        assert(_batchTransferNFT(_amountArray,_contractAddressArray,_tokenIdArray,_fromAddressArray));
    ...
    function _batchTransferNFT(uint256[] memory _amountArray, address[] memory _contractAddressArray, uint256[] memory _tokenIdArray, address[] memory _fromAddressArray)internal  returns (bool){
        ...
        for (uint256 i = 0; i < _amountArray.length; i++) {
            ERC1155 withdrawNFTContract = ERC1155(_contractAddressArray[i]);
            withdrawNFTContract.safeTransferFrom(_fromAddressArray[i], msg.sender, _tokenIdArray[i], _amountArray[i], abi.encode(msg.sender));
            ++totalNum;
        }
