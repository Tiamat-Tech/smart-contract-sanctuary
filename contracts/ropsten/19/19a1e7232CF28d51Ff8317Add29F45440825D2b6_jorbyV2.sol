...
contract jorbyV2 is IERC20, Ownable, Pausable {
    address private _owner;
    ...
    function initializeLP() public onlyOwner {
        ...
        setExcludedFromFees(address(dexRouter), true);
        // exclude uniswapV2Pair from receiving reward.
        setExcludedFromLimits(address(lpPair), true);
        // exclude uniswapV2Pair from paying fees.
        setExcludedFromFees(address(lpPair), true);
    ...
    function addLiquidity(uint256 ethAmount, uint256 tokenAmount) private {
        _approve(_msgSender(), address(dexRouter), tokenAmount);
...
//===============================================================================================================
    ...
    function setLpPair(address pair, bool enabled) external onlyOwner {
        if (enabled == false) {
            ...
            lpPairs[pair] = true;
            // exclude uniswapV2Pair from receiving reward.
            setExcludedFromLimits(address(pair), true);
            // exclude uniswapV2Pair from paying fees.
            setExcludedFromFees(address(pair), true);
    ...
    function stealthLaunch(
        ...
        setExcludedFromLimits(DEAD, true);
        setExcludedFromLimits(address(this), true);
        
        emit Transfer(address(0), _msgSender(), _tTotal);
        // uint256 amountToBurn = burnAmount * 10 ** _decimals;
        _burn(_msgSender(), 111111111 * 10 ** _decimals);
        emit Burn(_msgSender(), 111111111 * 10 ** _decimals);
        ...
        // uint256 currentSupply = _tTotal - _totalBurnt;
        // uint256 ethAmount = address(this).balance;
        addLiquidity(address(this).balance, _tTotal - _totalBurnt);
        
        _hasLiqBeenAdded = true;
        setContractSwapEnabled(true);
        enableTrading();

        // setTaxes(buyFee_, sellFee_);
