...
contract LiquifiRouter is ILiquifiRouter02 {
    ...
    // ) internal {
    //     address from = (token == WETH && convertETH == ConvertETH.IN_ETH) ? address(this) : msg.sender;
    ...
    //     return (tokenA == WETH ? address(0) : tokenA) < (tokenB == WETH ? address(0) : tokenB);
    // }

    // function _deposit(
    //     address token1, uint amount1,
    //     address token2, uint amount2,
    //     address to,
    //     ConvertETH convertETH,
    //     uint timeout
    // ) private ensure(timeout) returns (
    //     uint liquidityOut,
    //     uint amountA,
    //     uint amountB
    // ) {
    ...
    //         pool = PoolFactory(factory).getPool(tokenA, tokenB);
    //     }
    ...
    function addLiquidity(
        ...
        emit Mint(tokenA, amountA, tokenB, amountB, liquidity, to, ConvertETH.NONE); // TODO: check order of tokens
    }
    ...
    returns (uint amountToken, uint amountETH, uint liquidity) {
        ...
        liquidity = DelayedExchangePool(pool).mint(to);
        emit Mint(token, amountToken, WETH, amountETH, liquidity, to, ConvertETH.IN_ETH); // TODO: check order of tokens
    ...
    function removeLiquidity(
        ...
        // TODO: Stack too deep, try removing local variables. For liquidity
        // emit Burn(token0, amount0, token1, amount1, liquidity, to, ConvertETH.NONE);
    ...
    function removeLiquidityETH(
        ...
        // emit Burn(token1, amount1, token2, amount2, liquidityIn, to, convertETH.OUT_ETH);
    ...
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        ...
        // emit Burn(token, amountTokenMin, WETH, amountETH, liquidity, to, convertETH.OUT_ETH);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            emit Swap(input, amount0Out, output, amount1Out, to, ConvertETH.NONE, 3); // TODO: check it
        }
    }
