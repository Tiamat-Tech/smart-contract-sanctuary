...
contract MttmMaToken is ERC20, AccessControl {
    ...
    bytes32 public constant TRANSFER_WHITELIST = keccak256("TRANSFER_WHITELIST");

    constructor() ERC20("MTTM Manager Association", "MTTM-MA") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(MINTER_ROLE, msg.sender);
        _grantRole(BURNER_ROLE, msg.sender);
        _grantRole(TRANSFER_WHITELIST, msg.sender);
    ...
    function grantTransferWhitelist(address grantTo) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _grantRole(TRANSFER_WHITELIST, grantTo);
    }

    function revokeTransferWhitelist(address revokeFrom) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _revokeRole(TRANSFER_WHITELIST, revokeFrom);
    }
    ...
    override(ERC20)
    {
        require(hasRole(TRANSFER_WHITELIST, from));
        require(hasRole(TRANSFER_WHITELIST, to));
