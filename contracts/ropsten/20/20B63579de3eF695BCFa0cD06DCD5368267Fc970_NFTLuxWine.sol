...
contract NFTLuxWine is  Ownable, ReentrancyGuard, ERC721{
    ...
    event WineId(uint256 indexed wineId);
    event WineMetadata(uint256 indexed wineId, uint256 indexed price, string indexed name, string uri, bool onSale );
    ...
    function mintWine( string memory _uri, string memory _name, uint256 _price, bool _onSale) public onlyOwner returns (uint256) {
        ...
        emit WineId(newWineId);

        WineMeta memory meta = WineMeta(newWineId, _price, _name, _uri, _onSale);
        _setWineMeta(newWineId, meta);
        emit WineMetadata(newWineId, _price, _name, _uri, _onSale);
