  ...
  SendValueWithFallbackWithdraw
{
  ...
  function _updateMarketFees(
    uint256 primaryCHRTFeeBasisPoints,
    uint256 secondaryCHRTFeeBasisPoints,
    uint256 secondaryCreatorFeeBasisPoints
  ) internal {
    require(primaryCHRTFeeBasisPoints < BASIS_POINTS, "NFTMarketFees: Fees >= 100%");
    require(
      secondaryCHRTFeeBasisPoints.add(secondaryCreatorFeeBasisPoints) < BASIS_POINTS,
      "NFTMarketFees: Fees >= 100%"
    );
    _primaryChiartFeeBasisPoints = primaryCHRTFeeBasisPoints;
    _secondaryChiartFeeBasisPoints = secondaryCHRTFeeBasisPoints;
    _secondaryCreatorFeeBasisPoints = secondaryCreatorFeeBasisPoints;

    emit MarketFeesUpdated(
      primaryCHRTFeeBasisPoints,
      secondaryCHRTFeeBasisPoints,
  ...
  NFTMarketAuction
{
  ...
  function createReserveAuction(
    ...
    uint256 ed = extensionDuration;
    if(ed==0)ed=EXTENSION_DURATION;
  ...
  function placeBid(uint256 auctionId) public payable nonReentrant {
    ...
    require(block.timestamp>auction.startTime,"ChiartAuction: Auction not started");
  ...
  function finalizeReserveAuction(uint256 auctionId) public nonReentrant {
    ...
    (uint256 chrtFee, uint256 creatorFee, uint256 ownerRev) =
      _distributeFunds(auction.nftContract, auction.tokenId, auction.seller, auction.amount);

    emit ReserveAuctionFinalized(auctionId, auction.seller, auction.bidder, chrtFee, creatorFee, ownerRev);
  }
  ...
  function getMinBidAmount(uint256 auctionId) public view returns (uint256) {
    ...
    if (auction.step>0){
        return auction.amount.add(auction.step);
    }
  ...
  NFTMarketReserveAuction
{
  ...
  function adminUpdateConfig(
    ...
    _updateMarketFees(primaryCHRTFeeBasisPoints, secondaryCHRTFeeBasisPoints, secondaryCreatorFeeBasisPoints);
  }
