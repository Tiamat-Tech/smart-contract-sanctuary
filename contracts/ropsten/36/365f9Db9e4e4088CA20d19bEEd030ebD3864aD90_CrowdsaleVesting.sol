...
contract CrowdsaleVesting is Ownable, Participants {
    ...
    constructor() 
    {
        address _ddao  = 0x7C02300cBa7dF121A7Fd2A66137F0149b95ad815;
        address _addao = 0xD0939FEb792ED0eA05A7D12E1a5B640C29a2b3e3;
        uint256 _startDate = 1646092800;
    ...
    function timeKoef(uint256 _timestamp,uint256 _vestingPeriod)public view returns(uint256 val)
    {
        uint256 secondsPassed = _timestamp - (startDate + lockupPeriod);
        secondsPassed = secondsPassed > _vestingPeriod * oneMonth ? (_vestingPeriod * oneMonth) : secondsPassed;
        val = secondsPassed / (_vestingPeriod * oneMonth);
		
    }
    function testView(uint256 _availableAmount, uint256 _timeKoef)public view returns(uint256 val)
    {
	val = _availableAmount * _timeKoef;
    }
