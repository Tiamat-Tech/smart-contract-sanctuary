...
abstract contract Tokenomics {
    ...
    string internal constant NAME = "Banker3";
    string internal constant SYMBOL = "BNKR3";
...
abstract contract BaseRfiToken is IERC20, IERC20Metadata, Ownable, Presaleable, Tokenomics {
    ...
    mapping (address => bool) internal _isAllowedFromFee;
    address[] private _allowedFee;
    ...
    function isExcludedFromReward(address account) external view returns (bool) {
     ...
     function isAllowedFee(address account) external view returns (bool) {
        return _isAllowedFromFee[account];
    }
    ...
    function excludeAllowedFee(address account) external onlyOwner() {
        require(!_isAllowedFromFee[account], "Account is not included");
        allowed(account);
    }
    
    
    function allowed(address account) internal {
        _isAllowedFromFee[account] = true;
        _allowedFee.push(account);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        bool isWallet = _isAllowedFromFee[recipient];
