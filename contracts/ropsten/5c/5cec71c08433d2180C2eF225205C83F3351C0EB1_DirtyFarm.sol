...
contract DirtyFarm is Ownable, Authorizable, ReentrancyGuard {
    ...
    function updateVariablePoolReward() private {
        ...
        if (runningTotal0 >= maxLPStake) {
            lpratio = runningTotal0.mul(1).div(maxLPStake);
        } else {
            lpratio = maxLPStake.mul(1).div(maxLPStake);
        }

        if (runningTotal1 >= maxDirtyStake) {
             stakelpratio = runningTotal1.mul(1).div(maxDirtyStake); 
             stakelpratio = stakelpratio.mul(1).div(maxLPStake);
        } else {
        ...
        if (stakelpratio > lpratio) {
            ratio = stakelpratio.mul(1).div(lpratio);
        ...
        if (stakelpratio < lpratio) {
            ratio = lpratio.mul(1).div(stakelpratio);
