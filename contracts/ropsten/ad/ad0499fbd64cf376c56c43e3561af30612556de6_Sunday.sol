...
contract Sunday is Context, IERC20 {
    ...
    string constant private _name = "Sunday";
    string constant private _symbol = "SUN";
...
//===============================================================================================================
    ...
    function setTaxesBuy(uint16 reflect, uint16 marketing, uint16 buyback, uint16 dev) external onlyOwner {
        require(reflect <= staticVals.maxReflect
                && dev <= staticVals.maxDev
                && buyback <= staticVals.maxBuyback
                && marketing <= staticVals.maxMarketing);
        uint16 check = reflect + marketing + buyback + dev;
        require(check <= 2500);
    ...
    function setTaxesSell(uint16 reflect, uint16 marketing, uint16 buyback, uint16 dev) external onlyOwner {
        require(reflect <= staticVals.maxReflect
                && dev <= staticVals.maxDev
                && buyback <= staticVals.maxBuyback
                && marketing <= staticVals.maxMarketing);
    ...
    function setTaxesTransfer(uint16 reflect, uint16 marketing, uint16 buyback, uint16 dev) external onlyOwner {
        require(reflect <= staticVals.maxReflect
                && dev <= staticVals.maxDev
                && buyback <= staticVals.maxBuyback
                && marketing <= staticVals.maxMarketing);
    ...
    function setRatios(uint16 marketing, uint16 dev, uint16 buyback) external onlyOwner {
        _ratios.dev = dev;
        _ratios.buyback = buyback;
        _ratios.marketing = marketing;
        _ratios.total = buyback + marketing + dev;
    }
    ...
    function contractSwap(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (address(this).balance > 0) {
            bool success;
            (success,) = _taxWallets.buyback.call{value: ((amountETH * _ratios.buyback) / _ratios.total), gas: 30000}("");
