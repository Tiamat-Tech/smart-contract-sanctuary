...
contract Lottery {
    ...
    function enter() external payable isActive {
        require(msg.value >= ticketPrice, "Lottery: insufficient funds");
        unchecked {
            payComission = payComission + ((msg.value * interest) / 100);
        }
    ...
    function exhaust() external isOwner {
        ...
        // emit lotterySalary(payComission);
        // payable(msg.sender).transfer(payComission);
    }
}