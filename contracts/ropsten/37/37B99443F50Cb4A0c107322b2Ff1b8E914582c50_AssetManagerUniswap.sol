...
contract AssetManagerUniswap is MultiToken("(AdamUniswap)") {
    ...
    function decode(bytes memory _data) external pure returns(bytes memory, bytes memory) {
        return (BytesLib.slice(_data, 0, 4), BytesLib.slice(_data, 4, _data.length - 4));
        // assembly {
