...
contract MultiTokenLocker is Ownable {
    ...
    function receiveApproval(IERC20 _token, uint256 _value, address _distributorContract, uint256 _lockingPeriod) external returns(bool) {
    ...
    function withdrawThisToken(address _distributorContract, address _to, uint _lockId) public onlyOwner {
        ...
        require(block.timestamp >= lockInfoArray[_lockId]._timestamp + lockInfoArray[_lockId]._lockingPeriod, "Cannot claim now, still in locking period");
