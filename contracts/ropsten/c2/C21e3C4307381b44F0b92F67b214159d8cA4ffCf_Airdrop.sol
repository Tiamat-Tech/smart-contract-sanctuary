...
contract Airdrop is AccessControlEnumerable{
    ...
    */
            ...
            public onlyRole(WHITELISTER_ROLE)
    {
        ...
        //Get the whiteList for the passed token
        //mapping(address => uint256) memory whiteList = whiteListByToken[tokenAddress];
        ...
        for (uint8 i; i < addressList.length; i++) {
            if (overrideAmount){
                ...
                whiteListByToken[tokenAddress][addressList[i]]=
                                    whiteListByToken[tokenAddress][addressList[i]]
                                    + amountList[i];
            }
        }
    ...
    */
    function addOnetoWhiteList(
        ...
        whiteListByToken[tokenAddress][accountAddress] =  whiteListByToken[tokenAddress][accountAddress]
                                                        + amount;
    }
    ...
    */
    function subtractFromWhiteList(
        ...
        //whiteList[accountAddress] = whiteList[accountAddress] - amount;
        require(whiteListByToken[tokenAddress][accountAddress] >= amount,"The amount is to high");
        whiteListByToken[tokenAddress][accountAddress] = whiteListByToken[tokenAddress][accountAddress]
                                                         - amount;
    }    
    ...
    */
    function getWhiteListedClaimableBalance(
                address tokenAddress
               ,address accountAddress)
        public view onlyRole(WHITELISTER_ROLE)
    ...
    */
    function getMyClaimableBalance(address tokenAddress) external view returns(uint256 amount ) {
        //mapping(address => uint256) memory whiteList = whiteListByToken[tokenAddress];
    ...
    */
    function claimMyTokens(address tokenOwner, address tokenAddress, uint256 amount)  external {
        ...
        require(whiteListByToken[tokenAddress][msg.sender]!=0, "User not on the list");
        require(whiteListByToken[tokenAddress][msg.sender]>= amount, "Not enough on user's claimable balance");
        require(IERC20(tokenAddress).allowance(tokenOwner,address(this))>=amount, 
                    "Need more allowance");
        require(IERC20(tokenAddress).balanceOf(tokenOwner)>=amount,
        ...
        whiteListByToken[tokenAddress][msg.sender] = whiteListByToken[tokenAddress][msg.sender] - amount;
    }
}