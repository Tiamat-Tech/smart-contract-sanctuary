...
contract ARTStaking is IERC1155Receiver {
  ...
  constructor(
    ...
    rewardsPerBlock = SafeMath.div(totalRewards , totalBlocks);
    rewardEndBlock = SafeMath.add(block.number , _totalBlocks);
    owner = msg.sender;
    ...
    for (uint256 i = 0; i < allNFTs.length; i++) {
      if (allNFTs[i].nftID == _nftID && allNFTs[i].nftAddress == _nftAddress) {
        ...
        if (totalUsageWithWeight > 0) {
          rewardPerShare = SafeMath.div(rewardsPerBlock , totalUsageWithWeight);
        } else {
          rewardPerShare = rewardsPerBlock;
        }

        uint256 weightOfNFT = weightOfRank[allNFTs[i].nftID];
        uint256 totalBlocksStaked = SafeMath.sub(Math.min(block.number, rewardEndBlock) , allNFTs[i].lastBlock);

        calculatedReward = SafeMath.add(allNFTs[i].reward , SafeMath.mul(weightOfNFT,rewardPerShare).mul(totalBlocksStaked))  ;
      }
    }
  ...
  function calculateRewards() internal {
    for (uint256 i = 0; i < allNFTs.length; i++) {
      ...
      allNFTs[i].reward = SafeMath.add(allNFTs[i].reward , SafeMath.mul(weightOfNFT,rewardPerShare).mul(totalBlocksStaked))  ;
