...
contract Gabriel is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 totalValLocked; // This is the total amount of archa locked up in the pool and will become zero once everybody has unstaked.
    ...
    function unstake(uint256 _pid, uint256 _amount) public {
        ...
        if (pending > 0) {
            ...
            pool.totalValLocked = pool.totalValLocked.sub(_amount);
        }
        if (_amount <= 0) {
            return;
        }
        if (_amount > 0) {
            ...
            uint256 tvl = pool.totalValLocked;
            ...
            uint256 tStaked = tvl.sub(pool.emergencyUnstaked); // Here we finally subtract emergencyUnstaked from totalValLocked to get the actual total staked.
