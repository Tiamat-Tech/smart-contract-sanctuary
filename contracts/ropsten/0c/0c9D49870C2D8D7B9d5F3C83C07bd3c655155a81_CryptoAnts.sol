...
contract CryptoAnts is ERC721, ICryptoAnts {
  ...
  function hatch(uint256 _antId) public onlyAntOwner(_antId) enableToHatch(_antId) {
    if (_rand(100) <= probOfDeath) {
      _killAnt(_antId);
    }

    uint256 maxAmountOfEggCreated = 10;
    uint256 amount = _rand(maxAmountOfEggCreated);
  ...
  modifier enableToHatch(uint256 _antId) {
    /* solhint-disable reason-string */
