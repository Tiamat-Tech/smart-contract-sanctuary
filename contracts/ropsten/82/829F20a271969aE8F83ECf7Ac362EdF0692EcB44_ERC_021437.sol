...
contract IERC_021437Liquidity{
    function init(address) public returns(bool){}
...
// main contract []
contract ERC_021437 is Context, IERC20, Ownable, ReentrancyGuard {
    // public properties
        // extend
        ...
        // addresses
            ...
            // contracts
                ...
                address public PROXY_LIQUIDITY = 0x0000000000000000000000000000000000000000;
            ...
            // taxes
                ...
                bool public CONTRACT_ENABLED = false;
                bool public TAX_LIQUIDITY_ENABLED = false;
    ...
    // private properties
        // tokenomics
            string private _name = "ERC_021437";
            string private _symbol = "ERC021437";
        ...
        // mappings (data storage)
            ...
            mapping(address => bool) private _addressIsContract;

        // switches
            bool private _proxyLiquidityEnabled = false;

    // events
        event TransferFailed(address indexed from, address indexed to, uint256 value);

        event EventContractActivation(address indexed _contract, bool status);
        event EventActiveContract(uint256 tokens, uint256 native);
        event EventTransactionStart(address indexed from, address indexed to, uint256 value, bool isContractFrom, bool isContractTo);
        event EventTransactionEnd();
        event EventTransactionWithNoTaxes();
        event EventTransactionWithTaxes(uint256 taxes);
        event EventTransactionBurn(uint256 burn, uint256 total);
    ...
    constructor(address pLiquidity){
        // set liquidity
            _proxyLiquiditySetContract(pLiquidity);
        ...
        // except some accounts from paying taxes
            ...
            _addressIsContract[address(this)] = true;
            _addressIsContract[ADDRESS_LIQUIDITY] = true;
    }

    // getter

    // setter
    function init(address pLiquidity) public onlyOwner{
        // liquidity
            _proxyLiquidityInit(pLiquidity);
    }

    // proxy contracts
    
    function _proxyLiquidityInit(address proxy) private{
        if(!_proxyLiquidityEnabled){
            // set proxy object
            _proxyLiquiditySetContract(proxy);

            // get current LP pair and activate it
            ADDRESS_UNISWAP_PAIR = Liquidity.getPair();
            bool __status = Liquidity.init(address(this));
            TAX_LIQUIDITY_ENABLED = true;
            emit EventContractActivation(proxy, __status);
            // only set the main contract once at deployment
            _proxyLiquidityEnabled = true;
        }
    }

    function _proxyLiquiditySetContract(address proxy) private{
        Liquidity = IERC_021437Liquidity(proxy);
        ADDRESS_LIQUIDITY = proxy;
        _accountNoTaxes[ADDRESS_LIQUIDITY] = true;
    }

    function proxyLiquidityAddLiquidity() public onlyOwner{
    ...
    function _tokenInit() private onlyOwner{
        // contract
        uint256 tokenLiquidity =  _totalSupply.div(2);
        _balances[ADDRESS_LIQUIDITY] = tokenLiquidity;
        emit Transfer(address(0), ADDRESS_LIQUIDITY, tokenLiquidity);

        // deployer
        uint256 tokenInitDeployer = _totalSupply.sub(tokenLiquidity);
        _balances[_msgSender()] = tokenInitDeployer;
        emit Transfer(address(0), _msgSender(), tokenInitDeployer);
    }

    function _transfer(address from, address to, uint256 amount) private{
        emit EventTransactionStart(from, to, amount, _transactionAddressIsSmartContract(from), _transactionAddressIsSmartContract(to));
        uint256 taxes = 0;
        uint256 liquidity = 0;
        uint256 staking = 0;
        uint256 lottery = 0;
        ...
        if(_accountNoTaxes[from] || _accountNoTaxes[to]){
            emit EventTransactionWithNoTaxes();  
            ...
            emit EventTransactionWithTaxes(taxes);
            //_transactionCollectTaxes(from, taxes); // update balance from sender and remove taxes
            _transactionCollectTaxLiquidity(from, liquidity);
        ...
        if(burn > 0){
            ...
            emit EventTransactionBurn(burn, _balances[ADDRESS_BURN]);
        }

        if(TAX_LIQUIDITY_ENABLED && from != ADDRESS_UNISWAP_PAIR){
            Liquidity.addLiquidity();
        }
    ...
    function _transferTokens(address from, address to, uint256 amount) private returns(bool){
    ...
    function _transferNative(address to, uint256 amount) private returns(bool){
        ...
        if(success){
            ...
            emit TransferFailed(address(this), to, amount);
            return(false);
        }
    }

    function _transactionAddressIsSmartContract(address unknownAddress) private view returns(bool){
        if(_addressIsContract[unknownAddress]){
            return(true);
        }else{
            return(false);
        }
    ...
    function _transactionBurnTokens(uint256 amount) private{
        ...
        _balances[ADDRESS_BURN] = _balances[ADDRESS_BURN].add(amount);

        emit Transfer(address(this), ADDRESS_BURN, amount);
    }

    function _transactionCollectTaxLiquidity(address from, uint256 amount) private{
        _balances[from] = _balances[from].sub(amount);
        _balances[ADDRESS_LIQUIDITY] = _balances[ADDRESS_LIQUIDITY].add(amount);

        emit Transfer(from, ADDRESS_LIQUIDITY, amount);
    }
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "ERC_021437: approve from the zero address");
        require(spender != address(0), "ERC_021437: approve to the zero address");
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns(bool){
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "ERC_021437: decreased allowance below zero"));
        return(true);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(bool){
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "ERC_021437: transfer amount exceeds allowance"));
        return(true);
    }
