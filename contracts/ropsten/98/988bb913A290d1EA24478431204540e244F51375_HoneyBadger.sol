...
contract HoneyBadger is ERC721Enumerable, Ownable, PaymentSplitter {
	...
	constructor() ERC721("HoneyBadger", "HBGR") PaymentSplitter(addressList, shareList) {
	...
	function whitelistBuy(
		...
		require(isTokenValid(msg.sender, tokenId, proof), "W1");
		require(usedAddresses[msg.sender] + qty <= 3, "W2");
		require(tokenPrice * qty == msg.value, "W3");
		require(block.timestamp >= presaleStartTime, "W4");
		require(_tokenIds.current() + qty <= maxSupplyPresale, "W5");
