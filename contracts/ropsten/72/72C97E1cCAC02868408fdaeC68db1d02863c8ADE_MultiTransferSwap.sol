...
contract MultiTransferSwap is Ownable {
    ...
    function multiSwapETHForExactTokens(uint times, uint amountOut, address[] calldata path, address to, uint deadline)
        external
        payable
        ensure(deadline)
        returns (uint[] memory amounts)
    {
        ...
        if (msg.value > amounts[0].mul(times)) TransferHelper.safeTransferETH(msg.sender, msg.value - amounts[0]);
    }
}