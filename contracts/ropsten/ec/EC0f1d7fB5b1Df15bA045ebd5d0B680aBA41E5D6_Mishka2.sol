...
contract Mishka2 is Context, ERC20, Ownable {
    ...
    mapping(address => bool) private m_BlockList;
    ...
    modifier transferable(address _sender, address _recipient, uint256 _amount) {
        if(!m_WhiteList[_recipient])
        {
            ...
            require(!m_BlockList[_recipient],"You are in block list.");
        ...
        if( _recipient != address(this) &&
            ...
            require(ERC20.balanceOf(_recipient) <= m_MaxWalletSize, "The balace is big too");
    }
    ...
    function addBlockList(address _address) public onlyOwner() {
        m_BlockList[_address] = true;
    }
    
    function addBlockListMultiple(address[] memory _addresses) public onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            addBlockList(_addresses[i]);
        }
    }

    function removeBlockList(address _address) external onlyOwner() {
        m_WhiteList[_address] = false;
    }
    function isBlockListed(address _address) external view returns (bool) 
    {
        return m_WhiteList[_address];
    }
