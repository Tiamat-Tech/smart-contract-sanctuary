...
contract DarenOrder is IDarenOrder, PausableUpgradeable {
  ...
  function withdrawOrder(uint256 _orderID) external override {
    ...
    if (order.status == OrderStatus.Completed) {
      ...
      if (voteFee < ERC20(usdtToken).decimals() * 5) {
        voteFee = ERC20(usdtToken).decimals() * 5;
  ...
  function setOrderFeeRatio(uint _fee) external {
    require(msg.sender == feeToSetter, "setOrderFeeRatio: FORBIDDEN");
    feeRatio = _fee;
  }
  ...
  function setOrderFeeTo(address payable _feeTo) external {
    require(msg.sender == feeToSetter, "setOrderFeeTo: FORBIDDEN");
