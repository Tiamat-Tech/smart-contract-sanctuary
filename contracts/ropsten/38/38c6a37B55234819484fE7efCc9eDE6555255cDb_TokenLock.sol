...
contract TokenLock is Ownable {
    ...
    function lockToken(
        ...
        if (msg.value > initFee) {
            ...
            payable(msg.sender).transfer(remainFee);
        }
    ...
    function extendLockDuration(uint256 _id, uint256 _unlockTime) external {
        require(
            ...
            "TokenLock: withdrawable is false"
        );
        require(
            msg.sender == lockedToken[_id].withdrawalAddress,
            "TokenLock: not accessible"
        );

        // set new unlock time
        lockedToken[_id].unlockTime = lockedToken[_id].unlockTime.add(
            _unlockTime.mul(1 days)
        );
    ...
    function withdrawToken(uint256 _id) external {
        ...
        require(
            !lockedToken[_id].withdrawn,
            "TokenLock: withdrawable not accepct"
        );

        // transfer tokens to wallet address
        IERC20(lockedToken[_id].tokenAddress).safeTransfer(
            msg.sender,
    ...
    function getTotalTokenBalanceByWithdrawAddress(
        address _tokenAddress,
        address _withdrawalAddress
    ...
    function getDepositsByWithdrawAddress(address _withdrawAddress)
        public
        view
