...
contract MSActivityCenter is Context, Pausable, AccessControlEnumerable {
    ...
    uint256 internal _breedActivityId;
    ...
    event NewBreedActivityId(
        uint256 oldBreedActivityId,
        uint256 newBreedActivityId,
        address setter
    );
    ...
    function setBreedActivityId(uint256 newBreedActivityId) public onlyCreator {
        uint256 oldBreedActivityId = _breedActivityId;
        _breedActivityId = newBreedActivityId;
        emit NewBreedActivityId(oldBreedActivityId, newBreedActivityId, _msgSender());
    }
    ...
    {
        require(
            activityId != _breedActivityId,
            "MSActivityCenter: Cannot use breed activity number"
        );
    ...
    function breed(uint256 matronId, uint256 sireId) public whenNotPaused {
        require(matronId != sireId, "MSActivityCenter: matronId is equal to sireId");

        Activity storage activity = activities[_breedActivityId];

        chargeTokens(_breedActivityId, 1);

        uint256 tokenId = NoahNFT(activity.nftContract).breed(
            _breedActivityId,
            _msgSender(),
            matronId,
            sireId
        );
        activity.circulation += 1;

        emit Breed(
            _breedActivityId,
            activityCharges[_breedActivityId],
            _msgSender(),
            activity.nftContract,
            tokenId,
            block.timestamp
