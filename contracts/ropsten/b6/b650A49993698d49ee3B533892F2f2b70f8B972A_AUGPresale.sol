...
contract AUGPresale {
    ...
    uint256 public GLOBAL_CAP_BASE; // in wei
    uint256 public INDIVIDUAL_CAP_BASE; // in wei
    ...
    /**
     * @dev      Update the global cap base
     * @param _cap the cap [no decimals allowed]
     */
    function updateGlobalCapBase(uint256 _cap) external protected isPending {
        require(_cap != uint256(0), ERROR_CAP);
        GLOBAL_CAP = uint256(_cap).mul(ETH_PRICE_BASE).div(ETH_PRICE);
    }
     ...
     * @param _cap the cap [no decimals allowed]
     */
    function updateIndividualCapBase(uint256 _cap) external protected isPending {
        require(_cap != uint256(0), ERROR_CAP);
        INDIVIDUAL_CAP = uint256(_cap).mul(ETH_PRICE_BASE).div(ETH_PRICE);
    }
    ...
    function _setPrice(uint256 _ETHPrice) private {
        ETH_PRICE = _ETHPrice;
        TOKEN_PRICE = _ETHPrice.mul(ETH_PRICE_BASE).mul(uint256(10)).div(
            uint256(4)
        );
    }
