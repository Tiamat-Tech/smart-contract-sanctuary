...
contract DT is Context, Ownable, IERC20, LockableSwap, FeeLibrary, EthReflectingToken {
    ...
    function calculateSell(address from, uint256 amount) private returns(uint256, uint256, uint256){
        ...
        (uint256 tTransferAmount,,, uint256 tOther) = _getTValues(amount, 0, 0, finalSaleFee);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(amount, 0, 0, tOther, _getRate());
