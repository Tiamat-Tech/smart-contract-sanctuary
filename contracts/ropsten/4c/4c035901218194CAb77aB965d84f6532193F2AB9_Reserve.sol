...
contract Reserve is IReserve, Ownable, Pausable {
    ...
    function canWithdraw() external view override returns (bool) {
        ...
        uint256 toWithdraw = _getWithdrawAmount(details.proportion);

        if (toWithdraw >= details.amountWithdrawn) return true;
        return false;
    }

    function _getWithdrawAmount(uint256 _proportion)
        internal
        view
        returns (uint256 toWithdraw)
    {
        toWithdraw = (totalMoneyCollected.mul(_proportion)).div(
            totalProportions
        );
    }
        ...
        returns (uint256 toWithdraw)
    {
        ...
        toWithdraw = _getWithdrawAmount(details.proportion);
