...
contract SuperFuel is IBEP20, Manageable, AntiLPSniper, LockableFunction, LPSwapSupport {
    ...
    TokenTracker private feeDistributionTracker;
...
//        emit Transfer(address(0), address(this), _presaleReserve);
    ...
    function _calculateFees(uint256 amount, bool isTransfer) private returns(uint256 liquidityFee, uint256 marketingFee, uint256 buybackFee, uint256 reflectionFee, uint256 lotteryFee) {
        ...
        feeDistributionTracker.liquidity = feeDistributionTracker.liquidity.add(fees.liquidity);
        feeDistributionTracker.marketingTokens = feeDistributionTracker.marketingTokens.add(fees.marketing);
        feeDistributionTracker.buyback = feeDistributionTracker.buyback.add(fees.buyback);
        feeDistributionTracker.reward = feeDistributionTracker.reward.add(fees.tokenReflection);
        feeDistributionTracker.lottery = feeDistributionTracker.lottery.add(fees.lottery);
    }
    ////////////////////////////////////////////////////// - TODO Rewrite
    function _takeFees(uint256 amount, bool isTransfer) private returns(uint256 transferAmount){
    ...
    function _transfer( address from, address to, uint256 amount) private neitherBlacklisted(from, to){
        ...
        if(from != owner() && to != owner() && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            transferAmount = _takeFees(amount, false);
    ...
    function selectSwapEvent() private lockTheSwap {
        ...
        if(tokenContractBalance >= tokenSwapThreshold){
            ...
            if(tokenContractBalance > tokenTracker.liquidity){
                ...
                sendValue = swappedCurrency.mul(feeDistributionTracker.buyback).div(relativeDistributions);
