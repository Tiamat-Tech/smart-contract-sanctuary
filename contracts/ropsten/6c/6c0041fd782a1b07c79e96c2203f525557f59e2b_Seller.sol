...
contract Seller is ISeller, Ownable, WeekManaged {
    ...
    function getLatestPendingBasket(address who_, uint8 category_) external view returns(bytes memory) {
        ...
        for (uint i = 2; i >= 0; --i) {
            BasketRequest storage request = basketRequestMap[who_][week + 2][category_];
            if (request.time > 0) {
                bytes memory result = new bytes(request.assetIndexes.length * 16);
                for (uint j = 0; j < request.assetIndexes.length; ++j) {
                    result[j * 2] = byte(uint8(request.assetIndexes[j] / 256));
                    result[j * 2 + 1] = byte(uint8(request.assetIndexes[j] % 256));
                }
