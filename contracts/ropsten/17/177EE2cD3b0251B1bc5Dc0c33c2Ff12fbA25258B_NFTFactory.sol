...
contract NFTStore is Ownable, SignerRole, StoreBase {
    ...
    function initialize(string memory _name,string memory _symbol,address signer) external {
        name = _name;
        symbol = _symbol;
...
contract NFTFactory is Ownable {
    event NewSmartChefContract(address indexed smartChef, uint256 indexed NFTstoreCount);
    ...
    function deployNFT(string memory _name,string memory _symbol,address signer) external onlyOwner {
        NFTdeployCount++;
        ...
        emit NewSmartChefContract(NFTAddress, NFTdeployCount);
    }
}