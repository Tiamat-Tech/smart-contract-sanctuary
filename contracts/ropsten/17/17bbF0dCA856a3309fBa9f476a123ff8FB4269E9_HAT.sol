...
contract HAT is ERC721, Ownable, Pausable
{
	...
	mapping(uint256 => ticket_data) Tickets;
	...
	function Buy(address to, uint8 param_id,uint256 time)public
	{
		...
		Tickets[TicketCount].start = time;
		else
		Tickets[TicketCount].start = block.timestamp;

		Tickets[TicketCount].end = Tickets[TicketCount].start + Params[param_id].interval;
