  ...
  function mint(address _to, uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
      ...
      if(whitelisted[msg.sender].isWhitelisted == true) {        
        // if the mintedTokens the whitelisted user has is greater then the maxAirdropAmount then charge them
        // or is the amount they want to mint plus the amount they have already minted is greater then the maxAirdropAmount they are charged
        if(whitelisted[msg.sender].mintedTokens > maxAirDropAmount || _mintAmount + whitelisted[msg.sender].mintedTokens > maxAirDropAmount) {
        ...
        whitelisted[msg.sender].mintedTokens += _mintAmount;
      }
