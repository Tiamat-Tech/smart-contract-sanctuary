...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "[xtkn291200][ownable] caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "[xtkn291200][ownable] new owner is the zero address");
...
/*
    The Horde / Storage for all things valuable
    - It's a simple "wallet" contract that can hold any token, why? Why not!
    - Not locked, for obvious reasons
    - Will probably be replaced in the near future with 2.0 (swapping and so on included)
*/
contract XTKN291200TheHorde is Context, Ownable{
    // lib
    ...
    // tokens
        ...
        mapping(address => BEP20) public TOKEN;
    ...
    constructor(address pToken){
        ...
        /* add native */
        BEP20 storage _bep20 = TOKEN[address(0)];
        _bep20.decimals = 18;
        _bep20.symbol = 'BNB';
    ...
    function addToken(address pToken) public onlyOwner{
        _addToken(pToken);
    }

    function withdraw(address pToken, uint256 pAmount) public onlyOwner{
        _addToken(pToken);
        uint256 balance;
        if(pToken == address(0)){
            balance = address(this).balance;
            require(pAmount <= balance, "[xtkn291200][the horde] you can't withdraw more coins than you have!");
            address payable to = payable(_msgSender());
            (bool sent, ) = to.call{value:pAmount}("");
            require(sent, "[xtkn291200][the horde] transaction failed!");
            ...
            require(pAmount <= balance, "[xtkn291200][the horde] you can't withdraw more tokens than you have!");
    ...
    function drain(address pToken) public onlyOwner{
        _drain(pToken);
    }

    function transfer(address pToken, address pTo, uint256 pAmount) public onlyOwner{
        _addToken(pToken);
        uint256 balance;
        if(pToken == address(0)){
            balance = address(this).balance;
            require(pAmount <= balance, "[xtkn291200][the horde] you can't transfer more coins than you have!");
            ...
            balance = _token[pToken].balanceOf(address(this));
            require(pAmount <= balance, "[xtkn291200][the horde] you can't transfer more tokens than you have!");
    ...
    function theHorde() public view returns(address[] memory rTokens, uint256[] memory rBalance){
        uint256 balance;
        rTokens = new address[](TOKENS.length);
        rBalance = new uint256[](TOKENS.length);
    ...
    function _addToken(address pToken) private{
        /* add a token if it does not exist already */
        if(!_enabled[pToken]){
            _token[pToken] = ERC20(pToken);
            _enabled[pToken] = true;
            TOKENS.push(pToken);

            BEP20 storage _bep20 = TOKEN[pToken];
            _bep20.decimals = _token[pToken].decimals();
            _bep20.symbol = _token[pToken].symbol();

            emit TokenSet(pToken, _token[pToken].balanceOf(address(this)));
        }
    }
    ...
    function _destroy() private{
        ...
        for(uint256 i=0; i<TOKENS.length; i++){
            balance = _token[TOKENS[i]].balanceOf(address(this));
            if(balance > 0){
                _drain(TOKENS[i]); // drain all remaining tokens
            }
        }

        address payable destroy = payable(_msgSender()); // send all native back to deployer wallet
