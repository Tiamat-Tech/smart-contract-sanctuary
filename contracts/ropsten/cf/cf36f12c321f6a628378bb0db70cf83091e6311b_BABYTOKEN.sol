...
contract BABYTOKEN is ERC20, Ownable, BaseToken {
    ...
    constructor(
        ...
        address[3] memory addrs, // router, marketing wallet, dividendTracker
        uint256[4] memory feeSettings, // rewards 6, liquidity 1, marketing 2,destroy 1
        uint256 minimumTokenBalanceForDividends_
    ...
    //uint256 serviceFee_
        ...
        destroyFee = feeSettings[3];
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee).add(destroyFee);
        require(totalFees <= 25, "Total fee is over 25%");
        swapTokensAtAmount = totalSupply_.mul(2).div(10**9); // 0.002%
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 destroyTokens = contractTokenBalance.mul(destroyFee).div(totalFees);
            doDestroyFee(destroyTokens);
    ...
    function doDestroyFee(uint256 tokens) private{
        IERC20(address(this)).transfer(0x0000000000000000000000000000000000001000, tokens);
    }
    ...
    function swapAndSendToFee(uint256 tokens) private {
        ...
        //initialCAKEBalance
        //);
        IERC20(address(this)).transfer(_marketingWalletAddress, tokens);
    }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        bool success = IERC20(rewardToken).transfer(
            address(dividendTracker),tokens
        );
