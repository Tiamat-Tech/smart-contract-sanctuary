...
contract Bridge is Pausable, AccessControl, SafeMath {
    ...
    /**
        @notice Returns a proposal status.
        @param originDomainID Chain ID deposit originated from.
        @param depositNonce ID of proposal generated by proposal's origin Bridge contract.
        @param amount Amount of deposited.
        @param recipient recipient address of deposited.
        @return Proposal which consists of:
        - _proposedBlock Block number of the proposal created.
        - _yesVotes Number of votes in favor of proposal.
        - _noVotes Number of votes against proposal.
        - _status Current status of proposal.
     */
    function getProposalStatus(uint8 originDomainID, uint64 depositNonce, bytes32 resourceID, uint amount, address recipient) external view returns (ProposalStatus) {
        uint72 nonceAndID = (uint72(depositNonce) << 8) | uint72(originDomainID);
        bytes32 dataHash = keccak256(abi.encodePacked(resourceID, amount, recipient));
        return _proposals[nonceAndID][dataHash]._status;
    }
     ...
     */
    function deposit(uint8 destinationDomainID, bytes32 resourceID, uint amount) external payable whenNotPaused {
        ...
        depositHandler.deposit(resourceID, msg.sender, amount);

        emit Deposit(destinationDomainID, resourceID, depositNonce, msg.sender, amount);
    }
     ...
     */
    function voteProposal(uint8 domainID, uint64 depositNonce, bytes32 resourceID, uint amount, address recipient) external onlyRelayers whenNotPaused {
        address handler = _resourceIDToHandlerAddress[resourceID];
        ...
        if (proposal._status == ProposalStatus.Passed) {
            executeProposal(domainID, depositNonce, amount, recipient, resourceID, true);
            return;
        }
        ...
        if (proposal._status == ProposalStatus.Passed) {
            executeProposal(domainID, depositNonce, amount, recipient, resourceID, false);
        }
    }
     ...
     */
    function cancelProposal(uint8 domainID, uint64 depositNonce, bytes32 resourceID, uint amount, address recipient) public onlyAdminOrRelayer {
        uint72 nonceAndID = (uint72(depositNonce) << 8) | uint72(domainID);
        bytes32 dataHash = keccak256(abi.encodePacked(resourceID, amount, recipient));
        Proposal memory proposal = _proposals[nonceAndID][dataHash];
     ...
     */
    function executeProposal(uint8 domainID, uint64 depositNonce, uint amount, address recipient, bytes32 resourceID, bool revertOnFail) public onlyRelayers whenNotPaused {
        address handler = _resourceIDToHandlerAddress[resourceID];
        uint72 nonceAndID = (uint72(depositNonce) << 8) | uint72(domainID);
        bytes32 dataHash = keccak256(abi.encodePacked(resourceID, amount, recipient));
        ...
        if (revertOnFail) {
            depositHandler.executeProposal(resourceID, amount, recipient);
        } else {
            try depositHandler.executeProposal(resourceID, amount, recipient) {
