...
contract RewardPool is Ownable, Constants, IRewardPool {
    ...
    function transferAllBUSD (uint256 _amount) public onlyOwner {
    ...
    function distributeRewardPool () public onlyOwner {
        if(hasSwappedToLotlThisCycle)
        {
            transferAllBUSD(IERC20(busdAddr).balanceOf(address(this)));
        }
        else
        {
    ...
    // Burns LOTL.
    function burnLotl (uint256 _amount) public onlyOwner {
