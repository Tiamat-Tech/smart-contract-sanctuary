...
contract Gabriel is Ownable {
        ...
        returns (uint256)
    {
        if (block.timestamp < endTime) {
            ...
            return time.mul(user.amount).div(lpSupply.mul(tSecs)).mul(totalRewards);
        }
        if (block.timestamp > endTime) {
            PoolInfo storage pool = poolInfo[_pid];
            UserInfo storage user = userInfo[_pid][_user];
            uint256 lpSupply = pool.totalStaked;
            return tSecs.mul(user.amount).div(lpSupply.mul(tSecs)).mul(totalRewards);
        }
        return 0;
    ...
    function unstake(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        uint256 pending = tSecs.mul(user.amount).div(pool.totalStaked.mul(tSecs)).mul(totalRewards);
