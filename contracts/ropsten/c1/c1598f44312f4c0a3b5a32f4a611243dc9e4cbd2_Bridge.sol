...
contract Bridge is Pausable, AccessControl, SafeMath {
     ...
     */
    function getProposalStatus(uint8 originDomainID, uint64 depositNonce, bytes32 dataHash) external view returns (ProposalStatus) {
    ...
    /**
        ...
        @param amount Amount data to be passed to specified handler.
    ...
    function deposit(uint8 destinationDomainID, bytes32 resourceID, uint amount) external payable whenNotPaused {
        ...
        bytes memory handlerResponse = depositHandler.deposit(resourceID, msg.sender, amount);

        emit Deposit(destinationDomainID, resourceID, depositNonce, msg.sender, amount, handlerResponse);
    }
