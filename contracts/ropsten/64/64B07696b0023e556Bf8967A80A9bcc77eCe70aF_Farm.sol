...
contract Farm is Ownable {
    ...
    // Percentage of entry fee in the pool
    uint256 public poolEntryFees;
    ...
    constructor(IERC20 _erc20, uint256 _rewardPerBlock, uint256 _poolEntryFees, address _feeDepositAddress, uint256 _startBlock) public {
        erc20 = _erc20;
        rewardPerBlock = _rewardPerBlock;
        poolEntryFees = _poolEntryFees;
    ...
    function updatePoolEntryFees(uint256 _poolEntryFees) public onlyOwner {
        poolEntryFees = _poolEntryFees;
    }
    ...
    function add(uint256 _allocPoint, IERC20 _erc20Token, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        uint256 fees = 0;
        if (poolEntryFees > 0) {
            fees = _amount.div(100).mul(poolEntryFees);
            pool.erc20Token.safeTransferFrom(address(msg.sender), feeDepositAddress, fees);
        }

        pool.erc20Token.safeTransferFrom(address(msg.sender), address(this), _amount.sub(fees));
