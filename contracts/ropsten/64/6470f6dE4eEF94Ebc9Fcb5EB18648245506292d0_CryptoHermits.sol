...
contract CryptoHermits is ERC721Enumerable, Ownable {
  ...
  function mint(address _to, uint256 _mintAmount) public payable {
    ...
    // require(_mintAmount <= maxMintAmount, "Mint amount has to be less then or equal to 50.");
    // the balance of the sender plus the amount they want to mint has to be less then the max mint amount
    require(balanceOf(msg.sender) + _mintAmount <= maxMintAmount, "You can only purchase 20 tokens");
