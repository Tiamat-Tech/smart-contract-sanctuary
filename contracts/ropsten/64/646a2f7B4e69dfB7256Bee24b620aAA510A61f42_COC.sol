...
contract COC is Context, IBEP20, Ownable {
    ...
    uint256 public _MAX_TX_AMOUNT = 5000000 * 10**6 * 10**9;
    ...
    constructor(string memory name_, string memory symbol_, uint256 decimals_, uint256 totalSupply_, uint256 taxFee_, uint256 burnFee_) public {
        _name = name_;
        _symbol = symbol_;
    ...
    */
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(sender != owner() && recipient != owner())
            require(amount <= _MAX_TX_AMOUNT, "Transfer amount exceeds the maxTxAmount.");
...
//            _tOwned[account] = tokenFromReflection(_rOwned[account]);
    ...
    */
    ...
    /**
    * @dev Set new max transfer amount for every transaction
    * Can only be called by the current owner.
    */
    function setMaxTxPercent(uint256 maxTxPercent_) external onlyOwner() {
        _MAX_TX_AMOUNT = _tTotal.mul(maxTxPercent_).div(10**2);
    }
