...
// main contract []
contract ERC_020901 is Context, IERC20, Ownable, ReentrancyGuard {
    // public properties
        // extend
        ...
        // addresses
            // addresses
                ...
                address public ADDRESS_WALLET_LIQUIDITY;
    ...
    // private properties
        // tokenomics
            string private _name = "ERC_020901";
            string private _symbol = "ERC020901";
    ...
    constructor () {
        ...
        // set smart contract address list
            _addressIsContract[address(this)] = true;
            _addressIsContract[PROXY_UNISWAP_ROUTER] = true;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns(bool){
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "ERC_020901: decreased allowance below zero"));
        return(true);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(bool){
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "ERC_020901: transfer amount exceeds allowance"));
        return(true);
    }
    ...
    // setter
        function activateContract() external onlyOwner{
            require(!_initialLiquidity, "ERC_020901: Initial liquidity already set, cant active again!");
            _approve(address(this), address(uniswapV2Router), 2**256 - 1);
            _addressIsContract[address(uniswapV2Router)] = true;
            uint256 tokens = _balances[address(this)];
            uint256 native = address(this).balance;
            _contractAddLiquidityNative(tokens, native);
            TAX_ENABLED = true;

            emit EventActiveContract(tokens, native);
        }

        function setTaxesEnabled() external onlyOwner returns(bool enabled){
            TAX_ENABLED = true;
            return(TAX_ENABLED);
        }

        function setTaxesDisabled() external onlyOwner returns(bool enabled){
            TAX_ENABLED = false;
            return(TAX_ENABLED);
        }
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "ERC_020901: approve from the zero address");
        require(spender != address(0), "ERC_020901: approve to the zero address");
    ...
    function _transactionAddressIsSmartContract(address unknown) private view returns(bool){
        if(_addressIsContract[unknown]){
            return(true);
        }
        return(false);
    }
