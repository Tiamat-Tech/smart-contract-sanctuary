...
contract Vortex is ERC721Enumerable, Ownable {
    ...
    modifier alreadyRevealed() {
        require(revealed); // dev: Wait for reveal!
        _;
    }
     ...
     */
    modifier notLocked() {
        require(!locked); // dev: Methods are locked
        _;
    }
    ...
    {
        require(_exists(tokenId)); // dev: Cannot query non-existent token
    ...
    function setName(uint256 _tokenId, string memory _name)
        ...
        alreadyRevealed
    {
        require(_exists(_tokenId)); // dev: Cannot update non-existent token
        require(ownerOf(_tokenId) == msg.sender); // dev: You don't own this General!
    ...
    function gift(address[] memory _recipients) external onlyOwner {
        require(_giftCount + _recipients.length <= MAX_GIFT_COUNT); // dev: Max gift limit Reached!
        require(totalSupply().add(_recipients.length) <= _maxSupply); // dev: All Generals are sold out. Sorry!
    ...
    function presale(uint8 _num) external payable {
        require(presaleLive); // dev: Presale has not yet started.
        require(whitelist[msg.sender]); // dev: You're not on the whitelist.
        require(_num <= presaleNumPerAddress[msg.sender]); // dev: Can't purchase more than allowed presale Generals
        require(msg.value == uint256(_num).mul(PRICE)); // dev: You need to pay the required price.
        _mintTokens(_num);
    ...
    function mint(uint8 _num) external payable {
        require(bigBang); // dev: Wait for BigBang!
        require(totalSupply().add(uint256(_num)) <= _maxSupply); // dev: All Generals are sold out. Sorry!
        require(_num <= MAX_MINT); // dev: Max mint limit breached!
        require(msg.value == uint256(_num).mul(PRICE)); // dev: You need to pay the required price.
        _mintTokens(_num);
    }
