...
contract SoundsOfTheBlockchain is ERC721("Sounds of The Blockchain", "SoTB"), HasSecondarySaleFees{
	...
	function createSoundMould(string memory _name, uint256  _price, uint128 _mintLimit, address[] memory members, uint256[] memory shares, string memory _ipfsHash) external onlyTeam {
		...
		SoundMould memory mould = SoundMould(false, _name, _price, _mintLimit, 0, members, shares, _ipfsHash);
		mouldCount += 1;
	...
	(uint256 soundId, uint256 soundEdition, uint256 mintLimit, string memory name, string memory ipfsHash) {
       SoundByte memory sound = soundBytes[_id];
       SoundMould memory mould = soundMoulds[sound.mouldId];
       return (sound.mouldId, sound.edition, mould.mintLimit, mould.name, mould.ipfsHash);
   }
