...
contract HWS is ERC721Enumerable, PaymentSplitter, Ownable, ReentrancyGuard {
    ...
    uint256 public constant MAX_SUPPLY_PRESALE = 1000;
    ...
    address[] private team_ = [0xDC66B0631f6677d25344142938809B526eb202eC];
    uint256[] private teamShares_ = [10000];

    constructor(
        string memory _initBaseURI,
        string memory _initNotRevealedUri
    ) ERC721("Hustlers Of Wall Street", "HWS") PaymentSplitter(team_, teamShares_) {
    ...
    function presaleMint(bytes32[] calldata _merkleProof) external payable nonReentrant
    {
        ...
        uint256 price = 0.10 ether;

        require(supply + 1 <= MAX_SUPPLY_PRESALE, "HWS: Presale is SOLD OUT!");

        require(workflow == WorkflowStatus.Presale, "HWS: Presale is not started yet!");
        require(tokensPerWallet[msg.sender] < 1, "HWS: Presale mint is one token only.");
    ...
    function publicSaleMint(uint256 ammount, bytes32[] calldata _merkleProof) public payable nonReentrant {
        ...
        require(workflow == WorkflowStatus.Sale, "HWS: public sale is not started yet");
        require(msg.value >= price * ammount, "HWS: Insuficient funds");
