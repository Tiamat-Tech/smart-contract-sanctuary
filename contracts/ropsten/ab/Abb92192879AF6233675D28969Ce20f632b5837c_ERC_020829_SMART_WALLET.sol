...
contract ERC_020829_SMART_WALLET is Ownable{
    event DelegateContractOwner(address indexed newOwner);

    constructor(){
        
    }

    function delegate(address parent) external onlyOwner{
        _transferOwnership(parent);
        emit DelegateContractOwner(parent);
    }
}