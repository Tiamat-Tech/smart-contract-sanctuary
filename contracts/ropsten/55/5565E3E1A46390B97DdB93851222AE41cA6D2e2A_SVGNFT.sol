...
contract SVGNFT is ERC721URIStorage, Ownable {
    ...
    event CreatedPNGNFT(uint256 indexed tokenId, bytes tokenURI);
    ...
    function createPNG(bytes memory png) public {
        _safeMint(msg.sender, tokenCounter);
        string memory imageURI = pngToImageURI(png);
        _setTokenURI(tokenCounter, formatTokenURI(imageURI));
        tokenCounter = tokenCounter + 1;
        emit CreatedPNGNFT(tokenCounter, png);
    }

    function pngToImageURI(bytes memory png) public pure returns (string memory) {
        string memory baseURL = "data:image/png;base64,";
        string memory pngBase64Encoded = Base64.encode(bytes(abi.encodePacked(png)));
