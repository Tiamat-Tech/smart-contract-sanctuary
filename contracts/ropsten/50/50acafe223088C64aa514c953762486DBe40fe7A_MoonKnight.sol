...
contract MoonKnight is IMoonKnight, ERC721, AcceptedToken, ReentrancyGuard {
    ...
    function offer(uint knightId) external override nonReentrant payable {
        address buyer = msg.sender;
        uint offeredValue = msg.value;
        uint currentOfferValue = knightsWithOffers[knightId][buyer];
        ...
        require(offeredValue != currentOfferValue, "MoonKnight: offer with same value");

        knightsWithOffers[knightId][buyer] = offeredValue;

        if (currentOfferValue > 0) {
            (bool success,) = buyer.call{value: currentOfferValue}("");
