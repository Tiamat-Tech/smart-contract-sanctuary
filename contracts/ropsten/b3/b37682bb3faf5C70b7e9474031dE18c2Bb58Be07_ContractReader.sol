...
contract ContractReader is IGovernable, Initializable, Controllable {
  ...
  public view returns (UserInfoV0[] memory) {
    UserInfoV0[] memory result = new UserInfoV0[](vaults().length);
    for (uint256 i; i < vaults().length; i++) {
      result[i] = userInfoV0(_user, vaults()[i]);
    }
    return result;
  }

  function userInfoV0(address _user, address _vault) public view returns (UserInfoV0 memory) {
  ...
  function userInfo(address _user, address _vault) public view
  ...
  public view returns (VaultWithUserInfo[] memory){
    ...
    uint256 start = Math.min(page * (size % pageSize), size - 1);
  ...
  internal view returns (uint256) {
    ...
    console.log("ppfsChange", ppfsChange);
