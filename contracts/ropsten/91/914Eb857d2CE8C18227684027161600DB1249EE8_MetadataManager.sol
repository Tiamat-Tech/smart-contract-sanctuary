...
//TODO: give the ability to add new 1/1s
contract MetadataManager is Ownable {
    ...
    */
    ...
    function generateImage(uint[] memory _traits, uint _tokenId, uint _collectionId, uint _numTraits) public view returns(string memory) {
        ...
        if (mythicId > 0) {
            ...
            if(bytes(mythicInfo.name).length > 0) {
                ...
                //return string(abi.encodePacked(header, wrapTag(mythicInfo.image), getAdditionalImage(_collectionId, _tokenId), footer));
            } else {
     ...
     * @param _traitRarities List of traits for the property.
     */
    ...
    //TODO: consider that if we use 10000 as the divisor and a uint128, we can only fit 9 properties in a uint128
   ...
   */
    ...
    /**
     *@dev reveals the collection, by randomly mixing up the traits, and randomly destributing the 1/1's. Can only be called by the owner
     * and can only be called once per collection
     *@param _collectionId the unique id for the collection
     *@param _totalSupply the total number of tokens minted in the collection, used to allocate 1/1's
     *@param _numMythic the number of special 1/1s to distrubute
    */
    function revealCollection(uint _collectionId, uint _totalSupply, uint _numMythic, uint _numProperties) external onlyOwner {

        //make sure this hasnt already been called
    ...
    function mixUpTraits(uint _collectionId, uint _propertyIndex) internal onlyOwner {
        ...
        uint total = 0;
        //revealStruct.traitRarities = PropertyRarityRanges[_collectionId][i]; 
        ...
        for(uint j = 0; j < traitRarities.length; j++) {
            ...
            //require(index < tempLength, "index is not less than temp length");

            total += traitRarities[index];

            // require(total <= 10000, "total is greator than 10k");

            // require(index < traitRarities.length, "index greator than length");

            //finalRarities[i] = TraitRarity(uint16(revealStruct.total), uint16(revealStruct.index));
            ...
            // require(tempLength - 1 < traitRarities.length, "temp index out of range");

            // require(tempLength > 0, "tempLength == 0");

            // uint currentTempLength = tempLength;
            ...
            // require(currentTempLength == tempLength, "these lengths should be the same");

            // require(tempLength > 0, "temp length about to go negative");
            tempLength -= 1;

            // require(j < traitRarities.length, "j is on final pass");
    ...
    function setAddonContractAddress(address _addr, uint _index) external onlyOwner {
        ...
        if(_index == addonAddresses.length) {
            ...
            //we want to add an address
            addonAddresses.push(_addr);

        } else {
            //we want to update an address
    ...
    //TODO: remove these
    function getLastMythic(uint _collectionId) public view returns(uint) {
        return mythicsAdded[_collectionId];
    }

    function getTotalMythics(uint _collectionId) public view returns(uint) {
        return totalMythics[_collectionId];
    }

    function isRevealed(uint _collectionId) public view returns(bool) {
        return collectionRevealed[_collectionId];
    }
