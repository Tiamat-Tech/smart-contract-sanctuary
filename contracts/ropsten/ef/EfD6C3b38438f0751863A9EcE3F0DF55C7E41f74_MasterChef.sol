...
contract MasterChef is Ownable {
    ...
    // Bonus muliplier for early longdrink makers.
    ...
    function pendingLong(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 longReward = multiplier.mul(longPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 devAlloc = longReward.mul(devFundRate).div(divRate);
        uint256 treasuryAlloc = longReward.mul(treasuryRate).div(divRate);

        uint256 longWithoutDevAndTreasury = longReward.sub(devAlloc).sub(treasuryAlloc);

        ERC20Mintable(address(long)).mint(devaddr, devAlloc);

        

        ERC20Mintable(address(long)).mint(treasuryaddr, treasuryAlloc);
        ERC20Mintable(address(long)).mint(address(this), longWithoutDevAndTreasury);

        pool.accLongPerShare = pool.accLongPerShare.add(longWithoutDevAndTreasury.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for LONG allocation.
    ...
    function setLongPerBlock(uint256 _longPerBlock) public onlyOwner {
        require(_longPerBlock > 0, "!longPerBlock-0");
        longPerBlock = _longPerBlock;
    }
