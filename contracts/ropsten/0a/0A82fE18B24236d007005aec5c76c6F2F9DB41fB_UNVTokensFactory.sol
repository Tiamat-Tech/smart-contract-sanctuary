...
import "./UNVTokensFactory.sol";

contract UNVToken is ERC20Upgradeable, OwnableUpgradeable {
  ...
  function addMerkleRoot(
    ...
    require(
      recoveredAddress != address(0) &&
        recoveredAddress == UNVTokensFactory(factory).merkleRootSigner(),
      "Invalid signature"
    );
  ...
  function claimProjectTokensByFeeCollector() external {
    ...
    require(
      _msgSender() == UNVTokensFactory(factory).feeCollector(),
      "Cammot claim project tokens if caller is not fee collector"
    );

    uint256 amount = redeemToken.balanceOf(address(this));
    redeemToken.transfer(UNVTokensFactory(factory).feeCollector(), amount);
  }
    ...
    returns (bool)
  {
    ...
    if (overridenFee == uint256(-1)) {
      feeFromAmount = amount.mul(UNVTokensFactory(factory).fee()).div(100000);
    } else {
      feeFromAmount = amount.mul(overridenFee).div(100000);
    }

    super.transfer(UNVTokensFactory(factory).feeCollector(), feeFromAmount);
    ...
    returns (bool)
  {
    ...
    super.transferFrom(sender, UNVTokensFactory(factory).feeCollector(), feeFromAmount);
