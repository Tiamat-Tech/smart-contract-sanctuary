 ...
 */
contract NameYourGratia is ERC20, Ownable{
    ...
    mapping(address => uint256) public emissionStarts;                  // ex: 1623751121
    mapping(address => uint256) public emissionEnds;                    // ex: 1623751121 + (86400 * 365 * 5); // 5 Years
    ...
    function lastClaim(address token, uint256 tokenIndex) public view returns (uint256) {
        ...
        require(IGratiaERC721(token).ownerOf(tokenIndex) != address(0), "Owner cannot be 0 address");
        require(tokenIndex < IGratiaERC721(token).totalSupply(), "NFT at index has not been minted yet");
    ...
    function accumulated(address token, uint256 tokenIndex) public view returns (uint256) {
        ...
        if (lastClaimed == emissionStarts[token]) {
            uint256 initialAllotment = IGratiaERC721(token).isMintedBeforeReveal(tokenIndex) == true ? emissionInitialAllotments[token].mul(emissionPreRevealMultipliers[token]) : emissionInitialAllotments[token];
    ...
    function claim(address token, uint256[] memory tokenIndices) public returns (uint256) {
        ...
        for (uint i = 0; i < tokenIndices.length; i++) {
            ...
            require(tokenIndices[i] < IGratiaERC721(token).totalSupply(), "NFT at index has not been minted yet");
            ...
            require(IGratiaERC721(token).ownerOf(tokenIndex) == _msgSender(), "Sender is not the owner");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        // Approval check is skipped if the caller of transferFrom is the Gratia contract. For better UX.
