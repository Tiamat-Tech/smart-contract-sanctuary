...
contract LostPunks is ERC721Enumerable, Ownable {
    ...
    function mintLostPunk(uint16 fatherIndex, uint16 motherIndex) external {
        ...
        uint16 fathersFather = fatherMapping[fatherIndex];
        uint16 fathersMother = motherMapping[fatherIndex];
        uint16 mothersFather = fatherMapping[motherIndex];
        uint16 mothersMother = motherMapping[motherIndex];
        if (fathersMother != 0) {
            require((fathersMother != mothersMother) && (fathersMother != motherIndex), "Cannot mint children for close family");
        } 
        if (mothersFather != 0) {
            require((mothersFather != fathersFather) && (mothersFather != fatherIndex), "Cannot mint children for close family");
        } 
