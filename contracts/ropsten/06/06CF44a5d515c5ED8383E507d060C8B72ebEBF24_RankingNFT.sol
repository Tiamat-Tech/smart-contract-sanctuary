...
contract RankingNFT is ERC721, Ownable {
    ...
    function mintNFT(string memory _tokenURI, uint256 _initERC20Price) public returns (uint256) {
        ...
        _mint(owner(), newItemId); //Mint sents token to company at first time
        _setTokenURI(newItemId, _tokenURI);

        PurchaseInfo memory initPurchase = PurchaseInfo(msg.sender, msg.sender, 0, _initERC20Price, block.timestamp, block.timestamp);
        ...
        emit OnMintNFT(owner(), _tokenURI, newItemId);
