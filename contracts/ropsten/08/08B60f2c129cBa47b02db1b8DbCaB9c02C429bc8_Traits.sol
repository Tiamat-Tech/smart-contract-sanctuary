...
contract Traits is Ownable, ITraits {
   ...
   * @return a valid SVG of the Merchant / Pirate
   */
  function drawSVG(uint256 tokenId) public view returns (string memory) {
    IPirate.MerchantPirate memory s = pirate.getTokenTraits(tokenId);
    uint8 shift = s.isMerchant ? 0 : 9;

    string memory svgString = string(abi.encodePacked(
      drawTrait(traitData[0 + shift][s.fur]),
      s.isMerchant ? drawTrait(traitData[1 + shift][s.head]) : drawTrait(traitData[1 + shift][s.alphaIndex]),
  ...
  function compileAttributes(uint256 tokenId) public view returns (string memory) {
    ...
    return string(abi.encodePacked(
      ...
      tokenId <= pirate.getPaidTokens() ? '"Gen 0"' : '"Gen 1"',
      '},{"trait_type":"Type","value":',
      s.isMerchant ? '"Merchant"' : '"Pirate"',
      '}]'
    ));
  }
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    string memory metadata = string(abi.encodePacked(
      ...
      s.isMerchant ? 'Merchant #' : 'Pirate #',
      tokenId.toString(),
      '", "description": "Thousands of Merchants and Pirates compete in the oceans of the metaverse. A tempting prize of Resources awaits, with deadly high stakes. All the metadata and images are generated and stored 100% on-chain. No IPFS. NO API. Just the Ethereum blockchain.", "image": "data:image/svg+xml;base64,',
