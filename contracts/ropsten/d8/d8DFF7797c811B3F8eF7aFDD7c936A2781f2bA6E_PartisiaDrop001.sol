...
contract PartisiaDrop001 is ERC721URIStorage, Ownable {
  ...
  function setWhitelistAndTime(
      ...
      for (uint16 i = 0; i < addresses.length; i++) {
        _whitelists.push(addresses[i]);
        _addressExists[addresses[i]] = true;
      }
  ...
  function addToWhitelist(address[] memory addresses) external onlyOwner{
    for (uint16 i = 0; i < addresses.length; i++) {
      // TODO: Skip over no existent addresses
      _whitelists.push(addresses[i]);
      _addressExists[addresses[i]] = true;
    }
  }
  ...
  function getWhitelist() public view virtual returns (address[] memory) {
      return _whitelists;
  }
  ...
  function mint() external payable returns (uint256) {
      ...
      require(_addressExists[msg.sender], "Address not whitelisted");
