...
contract Vesting is Ownable {
    ...
    */
    function grantToken(
        ...
        require( keccakSeed == keccak256(abi.encodePacked(_round)) || keccakPrivate == keccak256(abi.encodePacked(_round)) || keccakPublic == keccak256(abi.encodePacked(_round)) , "Such round does not exist");
    ...
    */
    function claimed(address user) public view TGEisStarted returns (uint256 amount) {
        uint256 total = claimedInCategory(user, 'seed') + claimedInCategory(user, 'private') + claimedInCategory(user, 'public') - BalanceClaimed[user];
        return total;
    }
    ...
    */
    function claimedInCategory(address user, string memory categoryName) public view TGEisStarted returns (uint256 amount){
        Round memory round = round_[categoryName];

        // get round ID
        uint256 _roundID = round.roundID;
        if(_roundID == 0){
            ...
            uint256 timePassed = vestingTime - round.lockPeriod;
            uint256 totalUnits = (round.period - round.lockPeriod) / round.timeUnit;
            uint256 amountOfUnits = timePassed / round.timeUnit;
            ...
            uint256 bank = TotalTokenAllocations[categoryName][user];
            uint256 unlockAtTGE = round.onTGE;
            uint256 denom  = round.afterUnlockDenominator;
            uint256 reward = bank * unlockAtTGE/denom;
            // before lock period
    ...
    */
    function totalTokensAllocated(address _address) view public returns(uint256) {
