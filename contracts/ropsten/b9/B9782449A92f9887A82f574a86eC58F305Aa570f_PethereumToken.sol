...
//
//
// Pethereum ERC20 Smart Contract
...
contract Owned is Context {
    ...
    modifier onlyOwner {
        require(_msgSender() == _owner, "Pethereum Token: Only Owner can perform this task");
        _;
    }
    ...
    function acceptOwnership() public {
        require(_msgSender() == _newOwner, "Pethereum Token: Token Contract Ownership has not been set for the address");
 ...
 *
 */
contract PethereumToken is IERC20, Owned {
    ...
    function approve(address spender, uint256 amount) public override returns (bool) {
        ...
        require(spender != address(0), "Pethereum Token: approve to the zero address");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        require(amount <= _allowances[sender][_msgSender()], "Pethereum Token: Check for approved token count failed");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(recipient != address(0), "Pethereum Token: Cannot have recipient as zero address");
        require(sender != address(0), "Pethereum Token: Cannot have sender as zero address");
        require(_balances[sender] >= amount, "Pethereum Token: Insufficient Balance");
        require(_balances[recipient] + amount >= _balances[recipient], "Pethereum Token: Balance check failed");
    ...
    * @dev Destroys `amount` tokens from the `account`.
    ...
    function burn(address account, uint256 amount) public onlyOwner {
        ...
        require(account != address(0), "Pethereum Token: burn from the zero address");

        if (_balances[account] == _unlockedTokens[account]) {
            _unlockedTokens[account] = _unlockedTokens[account].sub(amount, "Pethereum Token: burn amount exceeds balance");
        }

        _balances[account] = _balances[account].sub(amount, "Pethereum Token: burn amount exceeds balance");
        ...
        if (account != _msgSender()) {
            ...
            require(amount <= _allowances[account][_msgSender()], "Pethereum Token: Check for approved token count failed");
    ...
    function _updateUnLockedTokens(address _from, uint tokens) private returns (bool success) {
        ...
        // if _unlockedTokens are less than "tokens" of "to", update _unlockedTokens by checking record with "now" time
            ...
            // otherwise revert
                ...
                revert("Pethereum Token: Insufficient unlocked tokens");
            }
        }
    }
