...
contract Booster is Initializable, Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            lockedFor = lockedFor.mul(amount_old).add(blocks_locked.mul(_amount)).div(userAmount);
            // set new locked at 
    ...
    function _burnStakedTokens() internal {
        ...
        uint256 burnRatio = (block.number - lastBurnBlock).div(burnBlockQuota).mul(burnMultiplier);
    ...
    function burningRate() external view returns (uint256 burnAmount) {
        ...
        uint256 burnRatio = (block.number - lastBurnBlock).div(burnBlockQuota).mul(burnMultiplier);
            //1000).mul(2); // burn 2% of every 1000th block; 0.02 * 100 = 2% 
