...
contract CrocsLeague is ERC721, Ownable {
    ...
    function mintCroc(uint256 numberOfTokens) public payable {
        ...
        require(
            numberOfTokens > 0 && numberOfTokens <= crocsPerWallet,
            "Each wallet can only mint 7 crocs"
        );
        require(
            totalSupply().add(numberOfTokens) <= CROCS_SUPPLY,
            "Not enough crocs left to mint"
        );
        require(
            msg.value >= crocBasePrice.mul(numberOfTokens),
            "Incorrect ETH sent"
        );
