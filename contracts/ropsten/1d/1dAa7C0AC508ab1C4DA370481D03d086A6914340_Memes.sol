...
contract Memes is Ownable {
  ...
  function withdrawFromMeme(bytes32 hashOfMemeString) public returns (bool) {
    ...
    uint256 balanceAvailable = (mimemeData.shares * memeData.totalFunds) / memeData.totalShares;
    ...
    // n.b., if exists == false and updatedAt is non-zero, mimeme was withdrawn
    mimemeData.updatedAtTimestamp = block.timestamp;
  ...
  function getMimemeExistence(bytes32 mimemeHash) public view returns (bool) {
    return mimemes[mimemeHash].exists;
  }

  function getMimemeShares(bytes32 mimemeHash) public view returns (uint256) {
    return mimemes[mimemeHash].shares;
  }
  
  function getMimemeUpdatedAtTimestamp(bytes32 mimemeHash) public view returns (uint256) {
    return mimemes[mimemeHash].updatedAtTimestamp;
  }
