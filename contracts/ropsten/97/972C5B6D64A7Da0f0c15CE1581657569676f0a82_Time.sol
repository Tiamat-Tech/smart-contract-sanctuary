...
contract Time is ERC20 {
    ...
    function unlockTime() public {
        ...
        _mint(address(this), (block.timestamp - lastTimeClaim - 5)* 10 ** uint(decimals()) );  // Time Contract recieves a day - 5 sec ideally
        _mint(msg.sender, (5)* 10 ** uint(decimals()) ); // Time Distributor recieves 5 seconds
        lastTimeClaim = block.timestamp;
    }
