...
contract TronWars is ERC721, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721("TronWars", "TRON") {
        setSale(
            "PRESALES",
            Sale(1644004800, 1644091199, 2, 2, 0.15 ether, false)
        );
        setSale(
            "COMMUNITY",
            Sale(1644004800, 1644091199, 100, 100, 0.15 ether, false)
        );
        setSale(
            "PUBLIC",
            Sale(1644004800, 1991332800, 3, 3, 0.15 ether, false)
        );
    }
    ...
    function saleIsOpen(string memory _name) public view returns (bool) {
        return
            sales[_name].start > 0 &&
            block.timestamp >= sales[_name].start &&
            block.timestamp <= sales[_name].end &&
            !sales[_name].paused;
    }
    ...
    ) public payable canMint("PRESALES", _count) nonReentrant {
        address wallet = _msgSender();

        if (!_isOG) {
            require(
                balanceSale["PRESALES"][wallet] <= 1,
                "Max per wallet limit (PRESALES)"
            );
        }

        require(signatureIds[_signatureId] == false, "Signature already used");
        require(
            checkSignature(wallet, _count, _isOG, _signatureId, _signature) ==
                signAddress,
    ...
    function communityMint(uint16 _count)
        public
        payable
        canMint("COMMUNITY", _count)
        nonReentrant
    {
        require(
            _msgSender() == address(0xDfd143aE8592e8E3C13aa3E401f72E1ca7deAED0),
            "Bad community wallet"
        );

        _mintTokens(_count);
    }

    function publicSalesMint(uint16 _count)
        public
        payable
        canMint("PUBLIC", _count)
        nonReentrant
    {
        _mintTokens(_count);
    }
    ...
    function price(string memory _name, uint256 _count)
        public
        view
        returns (uint256)
    {
    ...
    function minted(string memory _name, address _wallet)
        public
        view
        returns (uint16)
    {
