    ...
    Ownable
{
    ...
    function mintbusinesstwinNFT(
        ...
        tokenIsAtIndex[currentTokenId] = int(userOwnedTokens[msg.sender].length);
    ...
    function buyToken(uint256 _tokenId) public payable {
        ...
        int256 index = tokenIsAtIndex[_tokenId];
    ...
    function removeFromArray(int256 _index, address _address) internal {
        uint256 arrayLength = userOwnedTokens[_address].length;
        if (_index >= int(arrayLength)) revert();
        ...
        tokenIsAtIndex[userOwnedTokens[_address][ uint(_index)]] = -1;

        userOwnedTokens[_address][ uint(_index)] = lastElem;
