...
contract ARTStaking is IERC1155Receiver {
    ...
    for (uint256 i = 0; i < allNFTs.length; i++) {
      ...
      if (totalUsageWithWeight > 0) {
        ...
        rewardPerShare = rewardsPerBlock;
            ...
            calculatedReward = allNFTs[i].reward +(weightOfRank[allNFTs[i].nftID] * rewardPerShare * (Math.min(block.number, rewardEndBlock) - allNFTs[i].lastBlock));
  ...
  function deleteNFTFromBank(
    ...
    for (uint256 i = 0; i < nftBank[_owner][_nftAddress].length; i++) {
      ...
      nftBank[_owner][_nftAddress][indexToDelete] = nftBank[_owner][_nftAddress][nftBank[_owner][_nftAddress].length - 1];
  ...
  function calculateRewards() internal {
    for (uint256 i = 0; i < allNFTs.length; i++) {
      ...
      allNFTs[i].reward += (weightOfRank[allNFTs[i].nftID] * rewardPerShare * (Math.min(block.number, rewardEndBlock) - allNFTs[i].lastBlock));
