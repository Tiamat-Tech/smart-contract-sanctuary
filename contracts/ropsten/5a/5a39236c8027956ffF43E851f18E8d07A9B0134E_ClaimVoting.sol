...
contract ClaimVoting is IClaimVoting, Initializable, AbstractDependant {
    ...
    function initializeVoting(
        ...
        uint256 claimIndex = claimingRegistry.submitClaim(
            claimer,
            policyBookAddress,
            evidenceURI,
            coverTokens,
        ...
        uint256 onePercentInBMIToLock = priceFeed.howManyBMIsInUSDT(coverTokens.div(100));
        ...
        returns (MyClaimInfo[] memory _myClaimsInfo)
    {
        ...
        .min(claimingRegistry.countPolicyClaimerClaims(msg.sender))
        .max(offset);
    ...
    function _calculateMinorityVote(
        ...
        uint256 minorityPercentageWithPrecision = Math.min(
