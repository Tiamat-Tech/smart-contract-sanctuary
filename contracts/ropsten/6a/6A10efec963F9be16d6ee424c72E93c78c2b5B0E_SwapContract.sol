...
contract SwapContract is AccessControlEnumerable, Pausable, ECDSAOffsetRecovery
{
    ...
    mapping(address => uint128) private numsTokens;
    ...
    /**
      ...
      * @param num uint128 index of a token to add
      */
    function addToken(address token, uint128 num) external onlyOwner {
        validTokens.add(token);
        numsTokens[token] = num;
    }
    ...
    function removeToken(address token) external onlyOwner {
        ...
        delete numsTokens[token];
    }

    /**
      * @dev Retrieves token num from list of valid tokens
      * @param token address of token contract to retrieve from list
      */
    function tokenNum(address token) public view returns (uint128) {
        return numsTokens[token];
    }
