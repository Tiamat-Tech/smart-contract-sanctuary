    ...
    ReentrancyGuardUpgradeable
{
    ...
    // nft
    uint256 public maxSupply; // maximum supply of token
    ...
    /**
     * @dev returns the tokenURI using baseURI and the token Id and the nft's metaId if set
     */
        ...
        returns (string memory)
    {
        ...
        // If metaId is set, concatenate tokenURI and metaId with a slash between
        if (data.metaId != 0) {
            return
                string(
                    abi.encodePacked(_tokenURI, "/", data.metaId.toString())
                );
        }

        // Otherwise, return tokenURI
        return _tokenURI;
    }
    ...
    function mintNftForAirdropUser(
        ...
        uint256 _lockAmount,
        uint256 _metaId,
        bytes memory _airdropKey
    ) external payable {
        uint256 qstkAmount =
            airdrop.withdrawLockedQStk(msg.sender, _airdropKey);
    ...
    {
        require(
            _nftId != uint256(0) && totalSupply() >= _nftId,
            "QNFT: invalid nft id"
        );
        ...
        uint256 mintPrice =
            (nftSettings.favCoinPrices(_favCoinId) *
                nftSettings.upgradePriceMultiplier()) / PERCENT_MAX;
        require(
            msg.value >= mintPrice,
