...
contract ServiceV1 {
  ...
  function claimAll() public payable {
    ...
    for (uint16 i = 1; i <= entityNodeCount[msg.sender]; i++) {
      ...
      if (reward > 0) {
        require(this.claim{value : fee}(i, block.number, msg.sender), "claim failed");
      }
      value = value - fee;
    }
  }
  ...
  function claim(uint128 nodeId, uint256 blockNumber, address entity) public payable returns (bool) {
    ...
    uint256 reward = getRewardByBlock(entity, nodeId);
    ...
    payable(feeCollectorAddress).transfer(msg.value);
    nodeifyToken.transfer(entity, reward);
  
    emit Claimed(entity, reward);
  ...
  function payAll(uint256 nodeCount) public payable {
    ...
    for (uint16 nodeId = 1; nodeId <= entityNodeCount[msg.sender]; nodeId++) {
      ...
      this.payFee{value : recurringFeeInWei}(nodeId, msg.sender);
  ...
  function payFee(uint128 nodeId, address entity) public payable {
    bytes memory id = getNodeId(entity, nodeId);

    require(doesNodeExist(entity, nodeId), "doesnt exist");
    require(hasNodeExpired(entity, nodeId) == false, "too late");
    require(hasMaxPayments(entity, nodeId) == false, "too soon");
    ...
    emit Paid(entity, nodeId, entityNodePaidOnBlock[id]);
  }
