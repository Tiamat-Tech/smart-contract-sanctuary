...
contract Governance is Ownable, IGovernance {
  ...
  function createProposal(
    ProposalCreating memory proposal
  ) external override returns (uint256) {
    require(proposal.targets.length != 0, 'INVALID_EMPTY_TARGETS');
    require(
      proposal.targets.length == proposal.values.length &&
        ...
        proposal.targets.length == proposal.withDelegatecalls.length,
      'INCONSISTENT_PARAMS_LENGTH'
    );

    require(isExecutorAuthorized(address(proposal.executor)), 'EXECUTOR_NOT_AUTHORIZED');

    require(
      IProposalValidator(address(proposal.executor)).validateCreatorOfProposal(
        this,
        msg.sender,
        block.number - 1
      ),
    ...
    vars.endBlock = vars.startBlock + IProposalValidator(address(proposal.executor)).VOTING_DURATION();
    ...
    newProposal.executor = proposal.executor;
    newProposal.targets = proposal.targets;
    newProposal.values = proposal.values;
    newProposal.signatures = proposal.signatures;
    newProposal.calldatas = proposal.calldatas;
    newProposal.withDelegatecalls = proposal.withDelegatecalls;
    newProposal.startBlock =  vars.startBlock;
    newProposal.endBlock =  vars.endBlock;
    //block.number.add(_votingDelay).add(IProposalValidator(address(proposal.executor)).VOTING_DURATION());
    newProposal.strategy = _governanceStrategy;
    newProposal.ipfsHash = proposal.ipfsHash;
    _proposalsCount++;
  ...
  function getChainId() view internal returns (uint256) {
  uint256 chainIdVal;
  assembly {
    chainIdVal := chainid()
  }
  ...
  function _submitVote(
    ...
    uint256 votingPower = IGovernanceStrategy(proposal.strategy).getVotingPower(
      voter
      // proposal.startBlock
    );
