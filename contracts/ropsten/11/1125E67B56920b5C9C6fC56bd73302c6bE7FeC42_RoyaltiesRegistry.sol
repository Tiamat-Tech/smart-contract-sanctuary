...
import './@rarible/contracts/royalties/LibRoyaltiesV1.sol';
import './@rarible/contracts/royalties/RoyaltiesV2.sol';
import './@rarible/contracts/royalties/RoyaltiesV1.sol';
import './@rarible/contracts/royalties/IERC2981.sol';
...
contract RoyaltiesRegistry is IRoyaltiesProvider, Ownable {
    /// @dev deprecated
   ...
   constructor(){
    ...
    function setRoyaltiesType(address token, uint royaltiesType, address royaltiesProvider) internal {
        ...
        royaltiesProviders[token] = uint160(royaltiesProvider) + 2**(256 - royaltiesType);
    }
    ...
    function clearRoyaltiesType(address token) external {
        checkOwner(token);
        royaltiesProviders[token] = uint160(getProvider(token));
    }
    ...
    function calculateRoyaltiesType(address token, address royaltiesProvider ) public view returns(uint) {   
