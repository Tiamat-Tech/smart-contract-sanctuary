...
contract Elementals is ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    uint256 private maxEarthIndex;
    uint256 private maxWaterIndex;
    uint256 private maxFireIndex;
    uint256 private maxWindIndex;
    uint256 private maxLightningIndex;
    ...
    constructor(
        uint256 _reserved, 
        uint256 _supply, 
        uint256 _maxEarthIndex, 
        uint256 _maxWaterIndex,
        uint256 _maxFireIndex,
        uint256 _maxWindIndex,
        uint256 _maxLightningIndex) 
        ERC721("Elementals Vol. 1", "ELMTS") {
        ...
        maxSupply = _supply;
        maxEarthIndex = _maxEarthIndex;
        maxWaterIndex = _maxWaterIndex;
        maxFireIndex = _maxFireIndex;
        maxWindIndex = _maxWindIndex;
        maxLightningIndex = _maxLightningIndex;
    }
    ...
    function getRemainingElementals() external view returns (uint256[] memory) {
        ...
        counts[0] = maxEarthIndex - earthReserves - earthTokenId;
        counts[1] = maxWaterIndex - waterReserves - waterTokenId;
        counts[2] = maxFireIndex - fireReserves - fireTokenId;
        counts[3] = maxWindIndex - windReserves - windTokenId;
        counts[4] = maxLightningIndex - lightningReserves - lightningTokenId;
        return counts;
    }
