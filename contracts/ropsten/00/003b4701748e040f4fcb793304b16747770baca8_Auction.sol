...
contract Auction is IAuction, ExternallyCallable, AuctionBase {
     ...
     */
    function bid(address bidder, address ref)
        external
        payable
        override
        onlyExternalCaller
        returns (uint256)
    {
        ...
        if (optionsOf[currentAuctionId].referralsOn == true) {
            auctionBidOf[currentAuctionId][msg.sender].ref = ref;
        }

        /** Set auctionBid for bidder */
        auctionBidOf[currentAuctionId][bidder].eth += msg.value.toUint96();

        auctionsOf[bidder].add(currentAuctionId);

        reservesOf[currentAuctionId].eth += (msg.value / 1e12).toUint48();
    ...
    */
    ...
    function withdraw(uint256 auctionId, uint256 stakeDays) external {
        ...
        if (auctionMode == 0) {
            require(
                stakeDays >= optionsOf[currentAuctionId].autoStakeDays,
                'AUCTION: stakeDays < minimum days'
            );
        } else if (auctionMode == 1) {
            require(
                stakeDays >= optionsOf[currentAuctionId].ventureAutoStakeDays,
                'AUCTION: stakeDays < minimum days'
            );
        }
        ...
        withdrawInternal(
            ...
            uint256(auctionBidOf[auctionId][msg.sender].eth) * 1e12,
            auctionId,
            stakeDays
        );
    ...
    function withdrawLegacy(uint256 auctionId, uint256 stakeDays) external {
        ...
        require(auctionsOf[msg.sender].contains(auctionId) == false, 'AUCTION: Already withdrawn.');
        ...
        (uint256 eth, address ref, bool withdrawn) =
            contracts.auctionV2.auctionBetOf(auctionId, msg.sender);
    ...
    function withdrawInternal(
        ...
        uint256 uniswapPayoutWithPercent =
            _calculatePayoutWithUniswap(auctionId, eth, _calculatePayout(auctionId, eth));

        /** If referrer is empty simple task */
        if (address(ref) == address(0)) {
            contracts.stakeMinter.externalMintStake(
            ...
            if (address(ref) != address(0x000000000000000000000000000000000000dEaD)) {
                contracts.stakeMinter.externalMintStake(toRefMintAmount, 14, ref);
            }
    ...
    */
    function addTokensToNextAuction(uint256 amount) external override onlyExternalCaller {
        ...
        reservesOf[nextAuctionId].token += (amount / 1e12).toUint64();
    }
    ...
    function _calculatePayoutWithUniswap(
        ...
        uint256 uniswapPayout =
            ((uint256(reservesOf[auctionId].uniswapMiddlePrice) * 1e12) * amount) / 1e18;

        // Get payout with percentage based on discount, premium
        uint256 uniswapPayoutWithPercent =
            uniswapPayout +
                ((uniswapPayout * uint256(optionsOf[auctionId].discountPercent)) / 100) - // I dont think this is necessary
                ((uniswapPayout * uint256(optionsOf[auctionId].premiumPercent)) / 100);
    ...
    function _calculatePayout(uint256 auctionId, uint256 amount) internal view returns (uint256) {
        return
            (amount * (uint256(reservesOf[auctionId].token) * 1e12)) /
    ...
    */
    function _saveAuctionData(uint256 currentAuctionId) internal {
        ...
        if (settings.lastAuctionId < currentAuctionId) {
            ...
            reservesOf[currentAuctionId].token += uint256(
                uint256(auctions[currentDay].amountToFillBy) * 1e18
            )
                .toUint64();
            ...
            uint256 tokensSold =
                uint256(reservesOf[currentAuctionId].eth) *
                    uint256(reservesOf[currentAuctionId].uniswapMiddlePrice);

            tokensSold +=
                ((tokensSold * uint256(optionsOf[currentAuctionId].discountPercent)) / 100) -
                ((tokensSold * uint256(optionsOf[currentAuctionId].premiumPercent)) / 100);

            uint256 tokensForSale = reservesOf[currentAuctionId].token;

            if (tokensSold < tokensForSale) {
                uint256 nextWeeklyAuction = calculateNearestWeeklyAuction();

                reservesOf[nextWeeklyAuction].token += (tokensForSale - tokensSold).toUint64();
            }
    ...
    function withdrawBalance() external onlyExternalCaller {
    ...
    function setVentureAutoStakeDays(uint8 day, uint8 _autoStakeDays) external onlyManager {
        auctions[day].options.ventureAutoStakeDays = _autoStakeDays;
    }
    ...
    function init(
        ...
        // Contracts
        address _stakeMinter, // also externally callable
        address _auctionV2,
        address _vcAuction,
        address _uniswap
        ...
        _setupRole(EXTERNAL_CALLER_ROLE, _recipient);
    ...
    function restore(
        ...
        for (uint256 i = 0; i < auctions.length; i++) {
            ...
            auctions[i].options.ventureAutoStakeDays = _ventureAutoStakeDays;
