...
contract Fruitles is ERC721Enumerable, ERC721Burnable, ERC721Pausable, Ownable {
    ...
    string public baseTokenURI = "ipfs://QmNqgfoAsuHbPp4BqCz48mpDqSvYu7UhYaaEzQ8mM7ZWz8/";
    ...
    modifier mintRequirementsSale(uint256 count) {
        ...
        if (_private_sale_open) {
            require(_private_sale_open, "");
            price = PRESALES_PRICE;
        } else {
            require(_public_sale_open, "");
        }
        ...
        if (count == 1) {
            ...
            require(msg.value >= mul(price, 2), "");
        } else if (count == 3) {
            // You pay 0.18
            require(msg.value >= PUBLIC_PRICE_OF_3, "");
        } else if (count == 4) {
            ...
            require(msg.value >= (price + PUBLIC_PRICE_OF_3), "");
        } else if (count == 5) {
            // You pay 0.25; the discounted price for 5
            require(msg.value >= PUBLIC_PRICE_OF_5, "");
        } else if (count > 5) {
            require(msg.value >= mul(PUBLIC_PRICE_OVER_5_PER_NFT, count), "");
        }
        _;
    }
    ...
    //     NUMBER_OF_RESERVED_FRUITLES = NUMBER_OF_RESERVED_FRUITLES + numberToReserve;
    // }

    /**
     * @notice After reveal and all is ok, contract should be frozen by using the freeze command.
     */
     ...
     * @notice Minting for whitelisted people only
       ...
       *** Beam me up Scotty! ***
     */
    function mintFruitlesWhitelisted(uint256 count, bytes32[] calldata _merkleProof)
        external
        payable
        unfrozen
        saleIsOpen
    ...
    function mintFruitles(uint256 count) external payable unfrozen mintRequirementsSale(count) {
        _internalMintMultiple(msg.sender, count);
    }
