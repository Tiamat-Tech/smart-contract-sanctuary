...
contract Networks is AccessControl  {
	...
	function NetworkModify(uint id, bool disabled, bytes32 chain_id,string memory name,bytes32 url)public virtual onlyAdmin
	{
		network_list[id] = Network(id,disabled,chain_id,name,url);
	}

	function NetworkView(uint id)public view returns(uint,bool,bytes32,string memory,bytes32)
	{
