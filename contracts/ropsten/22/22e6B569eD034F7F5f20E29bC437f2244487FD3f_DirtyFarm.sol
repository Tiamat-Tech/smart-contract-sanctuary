...
contract DirtyFarm is Ownable, Authorizable, ReentrancyGuard {
    ...
    function redeem(uint256 _id) public nonReentrant {
        ...
        require(creator.redeemable, "This NFT is not redeemable with DirtyCash");
    ...
    function purchase(uint256 _id) public nonReentrant {
        ...
        if (creator.exists) { //re-arrange so no taxes are incurred
            ...
            if (creatorShare > 0) {
                IERC20(token).transferFrom(address(this), address(creator.creatorAddress), creatorShare);
            }
           /* if (poolShare > 0) {
    ...
    function setCreatorInfo(address _creator, uint256 _nftid, uint256 _splitPercent, bool _redeemable, bool _all) public onlyAuthorized {
