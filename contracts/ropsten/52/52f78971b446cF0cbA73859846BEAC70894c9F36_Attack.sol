...
contract Attack is IAttack, Ownable, Pausable {
    event BearsAttacked(address indexed owner, bytes32 indexed revealHassh, uint256 successes, uint256 value, uint256 errors, uint256 h, uint256 mod);
    ...
    function manyBearsAttack(
        ...
        uint256 seed = randomizerContract.commitRevealSeed(_msgSender(), revealHash);
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            uint256 ts = hiveContract.getLastStolenHoneyTimestamp(hiveIds[i]);
            if (ts == 0 || (ts + hiveCooldown) > block.timestamp) {
                errors += 1;
                continue;
            }
            ...
            (owed, successes, h) = _attack(seed, tokenIds[i], hiveIds[i]);
        ...
        emit BearsAttacked(_msgSender(), revealHash, successes, owed, errors, h, h % 100);
        ...
        )
    {
        ...
        uint256 h = (random(seed + tokenId + 0) & 0xFFFF);
