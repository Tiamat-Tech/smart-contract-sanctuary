...
contract TraitSelector {
    ...
    */
    function selectTraits(uint256 seed) internal view returns (IGnP.GalleonPirate memory t) {    
...
contract GnP is IGnP, ERC721Enumerable, Accessable, Pausable, TraitSelector {
    ...
    constructor(uint256 _maxTokens) ERC721("Pirate Game", "GnP") {  //TODO
    ...
    */
    function generate(uint256 tokenId, uint256 seed, LastWrite memory lw) internal returns (GalleonPirate memory t) {
        t = selectTraits(seed);
        ...
        if (seed == MAX_INT)
            return generate(tokenId, 1, lw);
        else
            return generate(tokenId, seed+1, lw);
    }
    ...
    */
    ...
    function isGalleon(uint256 tokenId) external view override 
        ...
        returns (bool) 
    {
        IGnP.GalleonPirate memory s = tokenTraits[tokenId];
        return s.isGalleon;
    }
