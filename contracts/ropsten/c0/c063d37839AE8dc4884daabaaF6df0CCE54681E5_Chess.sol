...
contract Chess {
  ...
  event GameCanceled(uint gameId, address canceledBy, uint8 gameState);
  event TurnTaken(uint gameId, address player, string board, string lastMove, uint8 gameState);
  ...
  function cancelGame(uint _id) external {
    ...
    emit GameCanceled(_id, g.canceledBy, uint8(g.gameState));
  }
  ...
  function makeMove(uint _id, string calldata _lastMove, string calldata _newBoard,
    ...
    emit TurnTaken(_id, msg.sender, oldBoard, _lastMove, uint8(_gameState));
