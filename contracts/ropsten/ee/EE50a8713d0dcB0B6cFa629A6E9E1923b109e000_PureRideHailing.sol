...
contract PureRideHailing {
    ...
    modifier onlyDriver {
        require(drivers[msg.sender].exists, "Unknown Driver");
        _;
    }

    modifier onlyPassenger {
        require(passengers[msg.sender].exists, "Unknown Passenger");
        _;
    }
    ...
    function p_register(string memory _name, string memory _number) public {
        require(!passengers[msg.sender].exists, "Passenger Exists");
    ...
    function d_register(string memory _name, string memory _number) public {
        require(!drivers[msg.sender].exists, "Driver Exists");
    ...
    */
    function request_trip(string memory _origin, string memory _destination, uint256 _deadline) public onlyPassenger returns(uint256){
        //Passenger should not be in trip
        require(passengers[msg.sender].inTrip == 0, "Passenger is in trip");
    ...
    */
    function bid_trip(uint256 _tripId, uint256 _amount, uint256 _deadline) public onlyDriver {
        ...
        require(drivers[msg.sender].inTrip == 0, "Driver in trip");
        require(_tripId <= trip_count, "404");
        //trip state should be pending for bids
        require(trips[_tripId].state == 0, "Trip already started");
        //passenger's deadline
        require(block.timestamp < trips[_tripId].deadline, "Expired");
    ...
    */
    function approve_bid(uint256 _tripId, uint256 _bidIndex) onlyPassenger public{
        //Passenger should not be in trip
        require(passengers[msg.sender].inTrip == 0, "Passenger in trip");
        require(_tripId <= trip_count, "404:trip");
        require(trips[_tripId].passenger == msg.sender, "403");
        //trip state should be pending for bids
        require(trips[_tripId].state == 0, "Trip Already Started");
        require(_bidIndex < trips[_tripId].bids.length, "404:bid");


        Bid memory _driverBid = trips[_tripId].bids[_bidIndex];
        //Driver should not be in trip
        require(drivers[_driverBid.driver].inTrip == 0, "Driver took off");

        //driver's deadline
        require(block.timestamp < _driverBid.deadline, "Expired Bid");
    ...
    */
    function end_trip_p(uint256 _tripId, uint8 _rate) public onlyPassenger {
        require(passengers[msg.sender].inTrip == _tripId, "403");

        require(trips[_tripId].state == 1, "trip should be open!");
        require(!trips[_tripId].p_rated, "already rated");
        require(0 < _rate && _rate < 6, "0<rate<6");
    ...
    */
    function end_trip_d(uint256 _tripId, uint8 _rate) public onlyDriver {
        require(drivers[msg.sender].inTrip == _tripId, "403");

        require(trips[_tripId].state == 1, "trip should be open");
        require(!trips[_tripId].d_rated, "already rated");
        require(0 < _rate && _rate < 6, "0<rate<6");
    ...
    */
    function cancel_trip_p(uint256 _tripId) public onlyPassenger {
        require(trips[_tripId].passenger == msg.sender, "403");
        require(!trips[_tripId].p_rated, "already ended");
    ...
    function cancel_trip_d(uint256 _tripId) public onlyDriver {
        require(trips[_tripId].driver == msg.sender,"403");
        require(!trips[_tripId].d_rated,"Already ended");
