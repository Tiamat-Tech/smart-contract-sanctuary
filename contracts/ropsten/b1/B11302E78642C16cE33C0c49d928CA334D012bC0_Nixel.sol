...
contract Nixel is ERC721Enumerable, Ownable {
    ...
    function setForSale(
        ...
        require(getRenter(x_pos,y_pos) == address(0),"NFT is currently being rented");
    ...
    function getRenterByTokenId(uint256 tokenId)
        public
        view
        returns (address)
    {
        if (block.timestamp <= _rentStop[blockLocation(tokenId)[0]][blockLocation(tokenId)[1]]) {
            return _renterAddress[blockLocation(tokenId)[0]][blockLocation(tokenId)[1]];
        } else {
            return address(0);
        }
    }
