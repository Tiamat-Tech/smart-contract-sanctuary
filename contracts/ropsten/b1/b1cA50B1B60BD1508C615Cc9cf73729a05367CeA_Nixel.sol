...
contract Nixel is ERC721Enumerable, Ownable {
    ...
    function transferSpawn() public onlyOwner {
        ...
        _BlockName[0][0] = "Nixel Spawn";
        createSpawn(0, 0, totalBlocks + 1);
        emit MintNixel(_msgSender(), totalBlocks + 1, 0, 0);
        _mint(_msgSender(), 1 + totalBlocks++);
    }

    function createSpawn(int256 x_pos, int256 y_pos, uint256 tokenId) public onlyOwner {
        _spawnBlockId[x_pos][y_pos] = tokenId;
    }
    ...
    function setPriceLuxury(uint256 price) public onlyOwner {
        require(totalBlocks == maxBlocks, "Cannot change prices until all current blocks have been bought");
        priceLuxury = price;
    }
    ...
    function setLuxuryBounds(int256 bounds) public onlyOwner {
        require(totalBlocks == maxBlocks, "Cannot increase blocks until all current blocks have been bought");
        luxuryBounds = bounds;
    }
