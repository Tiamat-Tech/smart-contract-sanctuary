...
contract Traits is Accessable, ITraits {
    ...
    function drawSVG(uint256 tokenId) internal view returns (string memory) {
        IGnP.GalleonPirate memory s = nftContract.getTokenTraits(tokenId);
        ...
        return string(abi.encodePacked(
            '<svg id="GalletonPirateNFT" width="100%" height="100%" version="1.1" viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
            svgString,
            "</svg>"
        ));
    }
