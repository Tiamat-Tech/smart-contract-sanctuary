...
contract CerealClub is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint256 private _auctionMaxSupply = 9000;
  ...
  function auctionMint(uint256 amount) external payable nonReentrant onlyEOA {
    ...
    if (totalSupply() + amount > _auctionMaxSupply) revert AmountExceedsSupply();
    if (amount > MAX_MINTS_PER_TX)                  revert AmountExceedsTransactionLimit();
  ...
  function publicSaleMint(uint256 amount) external payable nonReentrant onlyEOA {
    ...
    if (totalSupply() + amount > MAX_SUPPLY)  revert AmountExceedsSupply();
  ...
  function marketingMint(uint256 amount) external onlyOwner {
    if (amount > _marketingSupply) revert AmountExceedsSupply();
    if (totalSupply() + amount > MAX_SUPPLY) revert AmountExceedsSupply();

    _marketingSupply -= amount;
