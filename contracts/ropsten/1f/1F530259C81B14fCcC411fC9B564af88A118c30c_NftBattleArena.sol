...
contract NftBattleArena is Ownable, ERC721
{
	...
	struct VotingPosition
	{
		...
		// uint256 startDate;                                           // Date of creating voting position.
		// uint256 endDate;                                             // Date of liquidating voting position.
	...
	// event ZooFunctionsUpdated(uint256 date);
	...
	event StakedNft(address staker, address indexed token, uint256 indexed id, uint256 positionId, uint256 currentEpoch);

	/// @notice Event records info about withdrawed nft from this pool.
	event UnstakedNft(address staker, address indexed token, uint256 indexed id, uint256 positionId, uint256 currentEpoch);
	...
	// event recomputedDaiVotes(address voter, uint256 votingPositionId, uint256 newVotes, uint256 oldVotes);
	...
	// event recomputedZooVotes(address voter, uint256 votingPositionId, uint256 newVotes, uint256 oldVotes);
	...
	function updateZooFunctions() external onlyOwner
	{
		...
		// emit ZooFunctionsUpdated(block.timestamp);                              // Emits event that functions were updated.
	}
	...
	// /// @notice Function to staker position id by index in array.
	// /// @param i - index of id in array.
	// /// @return id - id of zoo battles nft position.
	// function getStakerPositions(uint256 i) public view returns (uint256 id)
	// {
	// 	return StakerPositions[i];
	// }
	...
	/// @notice Function to get id of voting position from index.
	/// @param staker - address of staker.
	/// @param i - index of staker nft position in array.
	/// @return id - id of nft voting position of staker.
	function getSpecificStakerPositions(address staker, uint256 i) public view returns (uint256 id) 
	{
		return positionsOfStaker[staker][i];
	}

	/// @notice Function to get amount of staker nft voting positions.
	/// @param staker - address of staker.
	/// @return amount - amount of staking positions.
	function getSpecificStakerPositionsLength(address staker) public view returns(uint256 amount)
	{
		return positionsOfStaker[staker].length;
	}

	// /// @notice Function to get id of voting position from index.
	// /// @param voter - address of voter.
	// /// @param i - index of voter nft position in array.
	// /// @return id - id of nft voting position of voter.
	// function getVotingPositions(address voter, uint256 i) public view returns (uint256 id) 
	// {
	// 	return positionsOfVoter[voter][i];
	// }

	/// @notice Function to get amount of voter nft voting positions.
	/// @param voter - address of voter.
	/// @return amount - amount of voting positions.
	function getVotingPositionsLength(address voter) public view returns(uint256 amount)
	{
	...
	/// @return NftPair - struct with info about nft pair.
	...
	function stakeNft(address token, uint256 id) public
	{
		...
		numberOfPositions++;                                                       // Increments amount and id of all positions.
	...
	function unstakeNft(uint256 stakingPositionId) public
	{
		...
		for(uint256 i = 0; i < positionsOfStaker[msg.sender].length; i++)
		{
			if (positionsOfStaker[msg.sender][i] == stakingPositionId)
			{
				positionsOfStaker[msg.sender][i] = positionsOfStaker[msg.sender][positionsOfStaker[msg.sender].length - 1];
				positionsOfStaker[msg.sender].pop();                               // Removes position of staker from his positions array.
				break;
			}
		}
	...
	{
		require(typeOfPositions[positionId] == PositionType.StakerPosition, "Wrong position type"); // Requires to be staker position type.
		require(getCurrentStage() == Stage.FirstStage, "Wrong stage!");             // Requires to be at first stage in battle epoch.
		require(ownerOf(positionId) == msg.sender, "Not the owner!");               // Requires to be owner of position.

		uint256 endEpoch = stakingPositions[positionId].endEpoch;                   // Gets endEpoch from position.
		...
		{
			int256 saldo = stakingPositions[positionId].rewards[i].yTokensSaldo;    // Get saldo from staker position.
		...
		stakingPositions[positionId].lastRewardedEpoch = end;                       // Records epoch of last reward claim.
		// vault.transfer(uint256(yTokensReward), beneficiary);
		uint256 reward = vault.withdraw(sharesToTokens(uint256(yTokensReward)), beneficiary); // Gets reward from yearn.
		dai.transfer(beneficiary, reward);                                          // Transfers dai to beneficiary.
	...
	function createNewVotingPosition(uint256 positionId, uint256 amount) public returns (uint256 votes)
	{
		...
		require(_exists(positionId), "wrong position id");                          // Requires for id to exist.
		...
		votingPositions[numberOfPositions].stakingPositionId = positionId;          // Records staker position Id voted for.
		...
		// votingPositions[numberOfPositions].startDate = block.timestamp;          // Records date when position created.
		...
		if (stakingPositions[positionId].rewards[currentEpoch].votes == 0)          // If staker position had zero votes before,
		{
			for(uint i = 0; i < StakerPositions.length; i++)                        // adds this staker position to eligible for pairing.
			{
				if (StakerPositions[i] == positionId)
				{
		...
		stakingPositions[positionId].rewards[currentEpoch].votes += votes;          // Adds votes for staker position for this epoch.
		stakingPositions[positionId].rewards[currentEpoch].yTokens += yTokensNumber;// Adds yTokens for this staker position for this epoch.

		positionsOfVoter[msg.sender].push(numberOfPositions);                       // Records id of this voter position into array for voter address.
		numberOfPositions++;
	...
	function pairNft(uint256 stakingPositionId) external
	{
		...
		require(_exists(stakingPositionId), "wrong position id");                   // checks if this id exist in battles nft.
		require(typeOfPositions[stakingPositionId] == PositionType.StakerPosition, "Wrong position type"); // Requires to be staker position type.
		...
		(nftsInGame, index1) = (index1, nftsInGame);                                // Swaps nftsInGame with index.
		...
		(nftsInGame, index2) = (index2, nftsInGame);                                // Swaps nftsInGame with index of opponent.
		nftsInGame++;                                                               // Increases amount of paired nft.
	...
	function voteWithZoo(uint256 stakingPositionId, uint256 votingPositionId, uint256 amount) public returns (uint256 votes)
	{
		...
		require(votes <= votingPositions[votingPositionId].daiVotes, "votes amount more than invested!"); // Requires for votes from zoo to be less than votes from dai.
		...
		// emit VotedWithZoo(msg.sender, stakingPositionId, votingPositionId, amount); // Emits VotedWithZoo event.
	...
	{
		// require(getNftPairLength(currentEpoch) >= i, "Wrong index"); // Todo: invalid opcode is acceptable?
		...
		uint256 token1 = pairsInEpoch[currentEpoch][i].token1;                      // Get id of 1st candidate.
		updateInfo(token1);
		uint256 votesForA = stakingPositions[token1].rewards[currentEpoch].votes;   // Get votes for 1st candidate.
		
		uint256 token2 = pairsInEpoch[currentEpoch][i].token2;                      // Get id of 2nd candidate.
		updateInfo(token2);
	...
	function updateEpoch() public {
		require(getCurrentStage() == Stage.FourthStage, "Wrong stage!");            // Requires to be at fourth stage.
	...
	function liquidateVotingPosition(uint256 votingPositionId, address beneficiary) public
	{
		...
		// votingPositions[votingPositionId].endDate = block.timestamp;                   // Sets endDate to now.
		...
		{
			if (positionsOfVoter[msg.sender][i] == votingPositionId)
			{
				positionsOfVoter[msg.sender][i] = positionsOfVoter[msg.sender][positionsOfVoter[msg.sender].length - 1];
				positionsOfVoter[msg.sender].pop();                                       // Removes position of voter from array.
				break;
			}
		}
		...
		// numberOfPositions--; todo: fix.

		emit VotingPositionLiquidated (msg.sender, beneficiary, votingPositionId, votingPositions[votingPositionId].zooInvested, currentEpoch);
	}
	...
	function claimRewardFromVoting(uint256 positionId, address beneficiary) public
	{
		require(getCurrentStage() == Stage.FirstStage, "Wrong stage!");             // Requires to be at first stage.
		require(ownerOf(positionId) == msg.sender, "Not the owner!");               // Requires to be owner of position.
		require(typeOfPositions[positionId] == PositionType.VoterPosition, "Wrong position type"); // Requires to be voter position type.

		uint256 stakingPositionId = votingPositions[positionId].stakingPositionId;  // Gets staker position id from voter position.
		(uint256 yTokens, uint256 yTokensIns, uint256 yTokensGas, uint256 yTokensTeam, uint256 lastEpochNumber) = getPendingVoterReward(positionId); // Calculates amount of reward.
		...
		votingPositions[positionId].lastRewardedEpoch = lastEpochNumber;            // Records epoch of last reward claimed.
	...
	function recomputeDaiVotes(uint256 votingPositionId) external
	{
		...
		require(newVotes > votes, "recomputed to lower value");// Requires for new votes amount to be bigger than before.
		...
		// emit recomputedDaiVotes(msg.sender, votingPositionId, newVotes, votes);
	}
	...
	function recomputeZooVotes(uint256 votingPositionId) external
	{
		...
		require(newZooVotes > oldZooVotes, "recomputed to lower value");// Requires for new votes amount to be bigger than before.
		require(newZooVotes <= votingPositions[votingPositionId].daiVotes, "votes amount more than invested!"); // Requires for votes from zoo to be less than votes from dai.
		...
		// emit recomputedZooVotes(msg.sender, votingPositionId, newZooVotes, oldZooVotes);
	}
	...
	function addDaiToPosition(uint256 votingPositionId, uint256 amount) external returns (uint256 votes)
	{
		...
		uint256 positionId = votingPositions[votingPositionId].stakingPositionId;   // Gets id of staker position.
		stakingPositions[positionId].rewards[currentEpoch].votes += votes;          // Adds votes to staker position for current epoch.
		stakingPositions[positionId].rewards[currentEpoch].yTokens += yTokensNumber;// Adds yTokens to rewards from staker position for current epoch.
	}
	...
	function addZooToPosition(uint256 votingPositionId, uint256 amount) external returns (uint256 votes)
	{
		...
		require(zooVotes + votes <= votingPositions[votingPositionId].daiVotes, "votes amount more than invested!");// Requires for votes from zoo to be less than votes from dai.
