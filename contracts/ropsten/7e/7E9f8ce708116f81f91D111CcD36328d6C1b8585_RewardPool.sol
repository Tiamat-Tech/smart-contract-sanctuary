...
contract RewardPool is Ownable, Constants, IRewardPool {
    ...
    function swapTokens(IERC20 _inputToken) private {
    ...
    function swapToLotl(uint256 _amount) private {
        IERC20(lotlToken).safeApprove(routerAddr, _amount);
        IPancakeRouter02(routerAddr).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            _amount,
            0,
    ...
    function removeLiquidity(IERC20 _lpToken) private {
    ...
    function processFees() public{
    	for(uint8 i; i < lptoken.length; i++){
    		...
    		if(lpAmount > 0){
                lptoken[i].safeApprove(routerAddr, lpAmount);
    	...
    	for(uint8 i; i < tokens.length; i++){
    		...
    		if(tokenAmount > 0 && tokens[i] != IERC20(busdAddr)){
                tokens[i].safeApprove(routerAddr, tokenAmount);
    			swapTokens(tokens[i]);
    		}
    	}
        ...
        IERC20(busdAddr).safeApprove(address(chef), amount);
    ...
    function addLpToken(IERC20 _lpToken, IERC20 _tokenA, IERC20 _tokenB) public override nonDuplicated(_lpToken) {
