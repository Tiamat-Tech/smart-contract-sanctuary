...
contract Reserve is IReserve, Ownable, Pausable {
    ...
    {
        require(
            IERC20(money).balanceOf(address(this)) >= _deposit + moneyBalance,
            "Reserve:deposit:: ERR_MONEY_NOT_DEPOSITED"
        );

        moneyBalance = moneyBalance.add(_deposit);
    }

    function getMoney() external override whenNotPaused {
        require(
            IERC20(money).balanceOf(address(buyback)) != 0,
            "Reserve:updateWithdrawer:: ERR_NO_MONEY"
        );
        buyback.transferMoneyToReserve();
    }
...
//will we update proportion in future?