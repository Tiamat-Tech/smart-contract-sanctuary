...
contract Cypher is ERC721 {
	...
	event NewTokenMinted(string uri,address to,uint256 newItemId);
		...
		returns (uint256) 
	{
		...
		emit NewTokenMinted(uri,to,newItemId);
		...
		returns (uint256) 
	{
		...
		emit NewTokenMinted(uriTwo,to,newItemId);

		_tokenIds.increment();
		newItemId = _tokenIds.current();
		_mint(to,newItemId);
		_setTokenURI(newItemId, uriThree);
		emit NewTokenMinted(uriThree,to,newItemId);

		_tokenIds.increment();
		newItemId = _tokenIds.current();
		_mint(to,newItemId);
		_setTokenURI(newItemId, uriFour);
		emit NewTokenMinted(uriFour,to,newItemId);

		_tokenIds.increment();
		newItemId = _tokenIds.current();
		_mint(to,newItemId);
		_setTokenURI(newItemId, uriFive);
		emit NewTokenMinted(uriFive,to,newItemId);
		...
		returns (uint256) 
	{
		...
		emit NewTokenMinted(uriSix,to,newItemId);

		_tokenIds.increment();
		newItemId = _tokenIds.current();
		_mint(to,newItemId);
		_setTokenURI(newItemId, uriSeven);
		emit NewTokenMinted(uriSeven,to,newItemId);

		_tokenIds.increment();
		newItemId = _tokenIds.current();
		_mint(to,newItemId);
		_setTokenURI(newItemId, uriEight);
		emit NewTokenMinted(uriEight,to,newItemId);

		_tokenIds.increment();
		newItemId = _tokenIds.current();
		_mint(to,newItemId);
		_setTokenURI(newItemId, uriNine);
		emit NewTokenMinted(uriNine,to,newItemId);

		_tokenIds.increment();
		newItemId = _tokenIds.current();
		_mint(to,newItemId);
		_setTokenURI(newItemId, uriTen);
		emit NewTokenMinted(uriTen,to,newItemId);
