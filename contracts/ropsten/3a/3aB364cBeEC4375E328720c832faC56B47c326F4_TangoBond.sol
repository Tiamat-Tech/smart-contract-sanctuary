...
import "./interfaces/ITangoTreasury.sol";
...
contract TangoBond is Ownable {
    ...
    ITangoTreasury immutable customTreasury; // pays for and receives principal
    address immutable tangoDAO;
    address tangoTreasury; // receives fee
    ...
    constructor(
        ...
        customTreasury = ITangoTreasury( _customTreasury );

        payoutToken = IERC20( _payoutToken );
      
        principalToken = IERC20( _principalToken );

        tangoTreasury = _tangoTreasury;
     ...
     *  @param _tangoTreasury uint
     */
    function changeOlympusTreasury(address _tangoTreasury) external {
        require( msg.sender == tangoDAO, "Only Olympus DAO" );
        tangoTreasury = _tangoTreasury;
    }
    ...
    function deposit(uint _amount, uint _maxPrice, address _depositor) external returns (uint) {
        ...
        if ( fee != 0 ) { // fee is transferred to dao
            payoutToken.transfer(tangoTreasury, fee);
        }
