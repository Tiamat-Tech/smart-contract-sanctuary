...
contract TradePairs is Ownable, ITradePairs , ReentrancyGuard {
    ...
    function setDecimals(bytes32 _tradePairId, uint256 _decimals, bool _isBase) public override onlyOwner {
        uint oldValue = tradePairMap[_tradePairId].baseDecimals;
        if (_isBase) {
            tradePairMap[_tradePairId].baseDisplayDecimals = _decimals;
        } else {
            oldValue = tradePairMap[_tradePairId].quoteDecimals;
            tradePairMap[_tradePairId].quoteDisplayDecimals = _decimals;
        }
        emit ParameterUpdated(_tradePairId, "T-DEC", oldValue, _decimals);
    }
    ...
    function doOrderCancel(bytes32 _tradePairId, bytes32 _orderId) private {
        ...
        emitStatusUpdate(_tradePairId, _orderId);
    }
    ...
    function cancelOrder(bytes32 _tradePairId, bytes32 _orderId) public override nonReentrant {
        ...
        doOrderCancel(_tradePairId, _orderId);
    }
    ...
    function cancelAllOrders(bytes32 _tradePairId, bytes32[] memory _orderIds) public override nonReentrant {
        ...
        for (uint i = 0; i < _orderIds.length; i++) {
            ...
            if (_order.oId != '' && _order.quantityFilled < _order.quantity && (_order.status == Status.PARTIAL || _order.status == Status.NEW)) {
                doOrderCancel(_tradePairId, _order.oId);
            }
        }
    }
