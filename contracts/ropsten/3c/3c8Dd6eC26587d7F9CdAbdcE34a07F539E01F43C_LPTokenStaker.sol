...
contract LPTokenStaker is Ownable, StakerStructs {
    ...
    function getUserPwrd(address user) public view returns (uint256) {
        ...
        if (baseAmount == 0) {
            ...
            return _prepPwrd(baseAmount, false);
        }
    }
    ...
    /// @param deposit if the logic is for a deposit or a withdrawal
    function _prepPwrd(uint256 amount, bool deposit) internal view returns (uint256) {
        ...
        uint256 diff;
        if (deposit) {
            diff = amount * factor % BASE;
            result = amount * factor / BASE;
        } else {
            diff = amount * BASE % factor;
            result = amount * BASE / factor;
        }
        if (diff >= 5E17) {
            result = result + 1;
        }
    ...
    function deposit(uint256 pid, uint256 amount) public {
        ...
        pool.lpToken.safeTransferFrom(msg.sender, address(this), uAmount);
    ...
    function withdraw(uint256 pid, uint256 amount) public {
        ...
        uint256 uAmount = amount;
        if (address(poolInfo[pid].lpToken) == PWRD) {
            uAmount = _prepPwrd(amount, false);
        ...
        pool.lpToken.safeTransfer(msg.sender, uAmount);
    ...
    function emergencyWithdraw(uint256 pid) public {
        ...
        pool.lpToken.safeTransfer(msg.sender, uAmount);
        emit LogEmergencyWithdraw(msg.sender, pid, uAmount);
    }
