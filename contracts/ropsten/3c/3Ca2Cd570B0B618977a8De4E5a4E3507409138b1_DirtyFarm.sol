...
contract DirtyFarm is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accDirtyPerShare; // Accumulated DIRTYCASH tokens per share, times 1e2. See below.
    }
    ...
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accDirtyPerShare = accDirtyPerShare.add(dirtyReward.mul(1e2).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public updateDirtyPerBlock {
        ...
        pool.accDirtyPerShare = pool.accDirtyPerShare.add(dirtyReward.mul(1e2).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accDirtyPerShare).div(1e2).sub(user.rewardDebt);
            if (pending > 0) {
