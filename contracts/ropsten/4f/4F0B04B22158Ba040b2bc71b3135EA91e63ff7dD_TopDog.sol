...
import "./KishiToken.sol";
import "./KishiLocker.sol";
...
// TopDog is the master of Kishi. He can make Kishi and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once KISHI is sufficiently
...
contract TopDog is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of KISHIs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accKishiPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accKishiPerShare` (and `lastRewardBlock`) gets updated.
    ...
    // The BONE TOKEN!
    KishiToken public bone;
    // The Bone Token Locker contract
    KishiLocker public boneLocker;
    // Dev address.
    ...
    function boneLockerUpdate(address _boneLocker) public onlyOwner {
        boneLocker = KishiLocker(_boneLocker);
    }
