...
contract AxesRedeem is ReentrancyGuard, ERC721Holder, AccessControl {
    ...
    function redeemIdInfo(uint256 _id) external view returns (address burnedOwner, uint256[] memory burnedIds, uint256[] memory newIds) {
        require(redeems[_id].burnedIds.length > 0, "Nonexistent redeem ID.");
        return (redeems[_id].owner, redeems[_id].burnedIds, redeems[_id].newIds);
    }
