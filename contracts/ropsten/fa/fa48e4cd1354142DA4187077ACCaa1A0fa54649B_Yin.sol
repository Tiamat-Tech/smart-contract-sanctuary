...
contract ERC20Fee is Context, IERC20, Ownable {
  ...
  uint256 public minSelfVoteAmount;
  ...
  function makeProposal(ProposalType _type, uint256 value, uint256 vote) public {
    ...
    if (_type == ProposalType.FeePercentage) {
      require(value < 10000, "makeProposal: fee can't be >=100%");
    }
    ...
    proposals[proposals.length - 1]._proVotesAddress[_msgSender()] = vote;
    _transferExcluded(_msgSender(), address(this), vote);
  }
  ...
  function voteProposal(uint256 index, uint256 amount, bool pro) public {
    ...
    _transferExcluded(_msgSender(), address(this), amount);
  }
  ...
  function getFeePercentage() public view returns (uint256 fee) {
    ...
    for (uint256 i = proposals.length - 1; i >= 0; i--) {
      if (proposals[i]._type == ProposalType.FeePercentage && proposals[i].proVotes > proposals[i].conVotes) {
  ...
  function getLPHolderFee() public view returns (uint256 fee) {
    if (proposals.length == 0) {
      return lpHolderFee;
    }

    for (uint256 i = proposals.length - 1; i >= 0; i--) {
      if (proposals[i]._type == ProposalType.LPFeePercentage && proposals[i].proVotes > proposals[i].conVotes) {
        return proposals[i].value;
      }
    }
  ...
  function getLPHolderAmount() public view returns (uint256 amount) {
    if (proposals.length == 0) {
      return lpHolderAmount;
    }

    for (uint256 i = proposals.length - 1; i >= 0; i--) {
      if (proposals[i]._type == ProposalType.LPAmount && proposals[i].proVotes > proposals[i].conVotes) {
        return proposals[i].value;
      }
    }
  ...
  function getBurnPercentage() public view returns (uint256 percent) {
    if (proposals.length == 0) {
      return burnPercentage;
    }

    for (uint256 i = proposals.length - 1; i >= 0; i--) {
      if (proposals[i]._type == ProposalType.BurnPercentage && proposals[i].proVotes > proposals[i].conVotes) {
        return proposals[i].value;
      }
    }
