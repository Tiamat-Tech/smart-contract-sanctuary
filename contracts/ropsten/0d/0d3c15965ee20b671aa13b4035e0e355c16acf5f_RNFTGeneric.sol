...
contract RNFTGeneric is Initializable, ERC1155Upgradeable{
    ...
    function getRNFTSalePrice(uint256 RNFTId) public view returns(uint256){
       return  RNFTProjectDetails[RNFTId].price;
    }
    ...
    require(RNFTProjectDetails[tokenId].status != RNFTStatus.DENIED, 'osa');
    RNFTProjectDetails[tokenId].status = RNFTStatus.ONSALE;
    }

    function unListRNFT(uint256 tokenId) external OnlyOwner{
    require(tokenExists[tokenId] == true, 'It');
    RNFTProjectDetails[tokenId].status = RNFTStatus.REMOVED;
    }
