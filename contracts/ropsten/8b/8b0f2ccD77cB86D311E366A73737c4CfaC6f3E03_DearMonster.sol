...
//    mapping(uint256 => Attribute) public attributes;
    ...
    function mintDearMonster(string [] memory element, uint256 amount) public{
        ...
        _mintSingleNFT(msg.sender, element);
    }

    function _mintSingleNFT(address to, string [] memory element) private {
       // require(totalSupply() < MAX_SUPPLY, "All NFTs are minted already");
        ...
        for (uint i = 0 ; i < element.length; i++ ){
            ...
            attributes.push(Attribute({ owner: to, name: "", element: element[i],  level : 1, exp: 0, star: 1, energy: 0 }));
