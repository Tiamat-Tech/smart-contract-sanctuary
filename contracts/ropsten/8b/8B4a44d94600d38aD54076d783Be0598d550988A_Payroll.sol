...
contract Payroll is Ownable, IERC721Receiver, Pausable, IPayroll {
  ...
  function claimManyFromPayrollAndPack(uint16[] calldata tokenIds, bool unstake) external whenNotPaused _updateEarnings {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      if (isBull(tokenIds[i]))
        owed += _claimBullsFromBarn(tokenIds[i], unstake);
      else
   ...
   */
  function _claimBullsFromBarn(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    Stake memory stake = payroll[tokenId];
    require(stake.owner == _msgSender(), "That's why you're in prison");
    require(!(unstake && block.timestamp - stake.value < MINIMUM_TO_EXIT), "GONNA BE COLD WITHOUT TWO DAY'S BULLIES");
    if (totalBulliesEarned < MAXIMUM_GLOBAL_BULLIES) {
      owed = (block.timestamp - stake.value) * DAILY_BULLIES_RATE / 1 days;
    } else if (stake.value > lastClaimTimestamp) {
      owed = 0; // $BULLIES production stopped already
    } else {
      owed = (lastClaimTimestamp - stake.value) * DAILY_BULLIES_RATE / 1 days; // stop earning additional $BULLIES if it's all been earned
    }
    if (unstake) {
      ...
      uint256 taxPercentage = 100 - BULLIES_CLAIM_TAX_PERCENTAGE - _gangForBull(tokenId);
      _payBearTax(owed * taxPercentage / 100); // percentage tax to staked wolves
      owed = owed * (100 - taxPercentage) / 100; // remainder goes to Sheep owner
      payroll[tokenId] = Stake({
        owner: _msgSender(),
        tokenId: uint16(tokenId),
        value: uint80(block.timestamp)
  ...
  function _claimBearFromPack(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    owed = (medals) * (bulliesPerMedal - stake.value); // Calculate portion of tokens based on Medals
    if (unstake) {
