...
contract ETHPool is AccessControl {
    ...
    mapping(address => uint256) public usersRewardRate;
    ...
    /**
     * @dev Deposit ETH to the pool, setting the active stake and the
     * user reward rate.
     *
     * msg.value must be greater than 0.
     */
    function deposit() public payable {
        ...
        usersRewardRate[msg.sender] += distributionRate * msg.value;
     ...
     * multiply by the difference between the distribution rate,
     * and the user reward rate.
     *
     * @param user The user whose current reward is calculated.
     */
    function getCurrentReward(address user) public view returns (uint256) {
        if (distributionRate <= usersRewardRate[user]) {
            return 0;
        }
        return
            (activeStakes[user] * (distributionRate - usersRewardRate[user])) /
            1e18;
    }
}