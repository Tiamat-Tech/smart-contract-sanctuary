...
contract MARKETPLACE {
    ...
    function removeTokenFromSale(uint256 tokenId) public returns (bool) {
        require(
            nft.balanceOf(msg.sender, tokenId) != 0,
            "You donot own the token"
        );
    ...
    function removeTokenFromBid(uint256 tokenId) public returns (bool) {
        require(
            nft.balanceOf(msg.sender, tokenId) != 0,
            "You donot own the token"
        );
    ...
    function putTokenOnSale(uint256 tokenId, uint256 price)
        ...
        returns (bool)
    {
        require(saleItems[tokenId].onSale == false, "Token is already on sale");
        require(
            bidItems[tokenId].onBid == false,
            "Token is aleady on bid so remove from sale and then put it on sale"
        );
        require(
            nft.balanceOf(msg.sender, tokenId) != 0,
        ...
        returns (bool)
    {
        ...
        require(
            saleItems[tokenId].onSale == false,
            "Token is aleady on sale so remove from sale and then put it on bid"
        );
        require(
            nft.balanceOf(msg.sender, tokenId) != 0,
            "You donot own this token"
