...
contract DopexNFT is ERC721, ERC721Enumerable, Ownable {
    ...
    bool ownerNotReserverdMint = true;
        ...
        returns (string memory)
    {
        ...
        return
            string(abi.encodePacked(base, "/", _tokenURI.toString(), ".png"));
    }
    ...
    function safeMint(address to, uint256 _mintNumber) public payable {
        ...
        require(
            _mintNumber <= MAX_MINT,
            "Attempting to mint more than mint limit"
        );
        require(
            _mintNumber + _tokenIdCounter.current() <= MAX_SUPPLY,
            "Can not mint more than total maximum supply"
        );
