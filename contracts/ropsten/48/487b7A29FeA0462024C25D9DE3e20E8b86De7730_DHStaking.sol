...
contract DHStaking is Ownable {
    ...
    function unclaimedRewards(uint tokenId) public view returns (uint) {
        ...
        uint rewardLimit = (stakedToken.endsAt - stakedToken.stakedAt) * stakeReward * stakedToken.rewardMultiplier;
        uint rewardsUntilNow = (block.timestamp - stakedToken.stakedAt) * stakeReward * stakedToken.rewardMultiplier;
        return (rewardLimit > rewardsUntilNow ? rewardLimit : rewardsUntilNow) - tokenIdToClaimedRewards[tokenId];
    }
