...
contract BryanContract is Ownable {
    ...
    function _validAddresses(address payable[] memory _accounts, uint256[] memory _amounts, uint256 _balance) internal pure returns(bool) {
        uint256 _sum = 0;
        for (uint256 k = 0; k < _accounts.length; k++) {
            if (_accounts[k] == address(0)) return false;
    ...
    function multiTransferFrom(address payable[] memory receivers, uint256[] memory amounts) public {
        ...
        for (uint256 i = 0; i < receivers.length; i++) {
            ...
            require(payableToken.transferFrom(_msgSender(), receiver, amount), "Failed to send to account");
        }
    }
}