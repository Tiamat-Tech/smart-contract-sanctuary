...
contract RankingNFT is ERC721, Ownable {
    ...
    function mintNFT(address _recipient, string memory _tokenURI) public returns (uint256) {
        ...
        PurchaseInfo memory initPurchase = PurchaseInfo(msg.sender, msg.sender, 0, 0, block.timestamp, block.timestamp);
        ...
        newHistory.currentPrice = 0; //0 or last buyer's price
