...
contract Fund is IFund, ERC20 {
    ...
    function redeemUniFee() external ready() lock() {
        IConfig config = IFundManager(controller).getConfig();
        address ceo = config.ceo();
        require(msg.sender == ceo, "only ceo");
        require(fundStatus == Status.Liquidation, "status!=Liquidation");
        IUniswapV2Router02 router = IUniswapV2Router02(IFundManager(controller).uniswapV2Router());
        address uniFeeAddress = IUniswapV2Factory(router.factory()).feeTo();
        uint256 balance = balanceOf(uniFeeAddress);
        require(balance > 0, "INSUFFICIENT_BALANCE");
        for (uint256 i = 0; i < getToken.length; i++) {
            address token = getToken[i];
            uint256 total = IERC20(token).balanceOf(address(this));
            if (total > 0) {
                _redeemToken(token, config.feeTo(), (total * balance) / _totalSupply);
            }
        }
        _burn(uniFeeAddress, balance);
    }
