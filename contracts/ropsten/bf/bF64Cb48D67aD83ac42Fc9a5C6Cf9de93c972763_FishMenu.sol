...
contract FishMenu is ERC721Enumerable, Ownable, ERC721Burnable {
    ...
    mapping(uint256 => MenuSection) public menuSections;
    uint8 public currentMenuSection = 0;
    bool public menuSectionsLocked = false;

    uint16 public constant HARD_CAP = 8888;
    uint8 private constant MAX_MINT_PER_TRANSACTION = 5;
    ...
    function setMenuSection(uint8 _sectionKey) public onlyOwner {
    ...
    function maxSupply() public view returns (uint256) {
        ...
        for (uint256 i = 0; i <= currentMenuSection; i++) {
    ...
    function reserve(address _to, uint8 _count) public onlyOwner {
        MenuSection storage menuSection = menuSections[currentMenuSection];
        // uint256 total = _totalSupply();
        uint256 total = totalSupply();
        require(total + _count <= maxSupply(), "Exceeds max supply");
