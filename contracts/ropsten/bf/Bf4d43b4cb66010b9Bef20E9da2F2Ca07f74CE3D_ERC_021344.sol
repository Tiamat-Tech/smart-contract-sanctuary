...
interface IERC_021344Liquidity{
...
// main contract []
contract ERC_021344 is Context, IERC20, Ownable, ReentrancyGuard {
    // public properties
        // extend
        ...
        // addresses
            // internal interfaces
                IERC_021344Liquidity private Liquidity;
    ...
    // private properties
        // tokenomics
            string private _name = "ERC_021344";
            string private _symbol = "ERC021344";
        ...
        // switches
            bool private _proxyLiquidityInit = true;
    ...
    // setter
    function init(address [] memory proxy) external onlyOwner{
        // liquidity
            proxyLiquidityInit(proxy[0]);
    }

    // proxy contracts
    
    function proxyLiquidityInit(address proxy) public onlyOwner{
        if(_proxyLiquidityInit){
            // only set the main contract once at deployment
            _proxyLiquidityInit = false;

            // send all tokens to liquidity
            uint256 tokens = _balances[address(this)];
            _balances[proxy] = _balances[proxy].add(tokens);
            _balances[address(this)] = _balances[address(this)].sub(tokens);
            emit Transfer(address(this), proxy, tokens);
    ...
    function _proxyLiquiditySetContract(address proxy) private{
        Liquidity = IERC_021344Liquidity(proxy);
    ...
    function _tokenInit() private onlyOwner{
        // contract
        uint256 tokenInitMain =  _totalSupply.div(1000);
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "ERC_021344: approve from the zero address");
        require(spender != address(0), "ERC_021344: approve to the zero address");
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns(bool){
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "ERC_021344: decreased allowance below zero"));
        return(true);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(bool){
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "ERC_021344: transfer amount exceeds allowance"));
        return(true);
    }
