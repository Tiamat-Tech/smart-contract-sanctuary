...
contract CarRacing is ERC721, ERC721URIStorage, Ownable{
    ...
    function createRandomCar(string memory _name, uint packType) external payable{
        ...
        if(_carLevel == 1){
            ...
            payable(token_contract_address).transfer(packonefee);
            _createCar(
            _name, 
            randDna, 
            1, 
            ...
            payable(token_contract_address).transfer(packtwofee);  
            _createCar(
            _name, 
            randDna, 
            2, 
            ...
            payable(token_contract_address).transfer(packthreefee);  
            _createCar(
            _name, 
            randDna, 
            3, 
    ...
    function levelUp(address _owner, uint _carId) external payable {
         if (ownerOf(_carId) == _owner){
            ...
            payable(token_contract_address).transfer(_updatefee);
