...
contract JoystickPresale is Context, Ownable, Authorizable {
    ...
    function withdraw() external whenVesting {
        ...
        if (block.timestamp >= VESTING_START.add(VESTING_DURATION)) {
            withdrawalAmount = _userInfo.purchasedAmount;
        } else {
            withdrawalAmount = _userInfo.purchasedAmount.mul(block.timestamp.sub(VESTING_START).div(VESTING_DURATION));
        }

        uint256 withdrawAmount = withdrawalAmount - _userInfo.withdrawnAmount;
       
        require(withdrawAmount > 0, "No withdrawal amount!");
        require(govToken.balanceOf(address(this)) >= withdrawAmount, "$Joy token amount to withdraw is lack!");
        
        govToken.transfer(_msgSender(), withdrawAmount);
        _userInfo.withdrawnAmount = withdrawalAmount;
