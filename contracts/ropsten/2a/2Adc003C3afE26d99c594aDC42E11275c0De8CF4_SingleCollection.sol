...
contract Help {
    ...
    function prepareMessage(uint256[] calldata _id, address _contAddr, string[] calldata tokenURI) public pure returns (string memory) {
        return keccak256(abi.encode(_id, _contAddr, tokenURI)).toString();
    }

    function checkSignature(string calldata messageHash, uint8 v, bytes32 r, bytes32 s) public pure returns(address) {
