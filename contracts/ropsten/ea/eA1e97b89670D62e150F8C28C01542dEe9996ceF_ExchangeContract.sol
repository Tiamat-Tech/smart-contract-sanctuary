...
contract ExchangeContract is Sale{
    ...
    constructor() public{}
    function mint(address token ,string memory tokenuri, uint256 value, uint256 tokenId, uint256 royal, string memory _type, uint256 supply) public returns (bool){
       if(keccak256(abi.encodePacked((_type))) == keccak256(abi.encodePacked(("721")))){
         IERC721(token).mint(msg.sender, tokenId, tokenuri);
