...
contract FOB_StableCoin {
    ...
    bool private seller_paid_share;
    bool private buyer_paid_share;
    ...
    buyer_confirmed_shipment_done, seller_confirmed_shipment_done, disagreement, canceled, done }
...
//      ///////////////////////////  End of  Modifiers  ///////////////////
    ...
    constructor(address _token_address, string[5] memory _all_strings, uint[7] memory _all_uints, address _buyer, address _seller, address _vessel_validator,
        ...
        PERCENTAGE_RATE = 100*(10**token_decimals);
        ...
        seller_paid_share = false;
        buyer_paid_share = false;
    ...
    function Deposit_to_contract() public inState(State.supplied) {
        ...
        if(msg.sender == buyer && !buyer_paid_share)
        {
            require(Token.balanceOf(buyer) >= buyer_share, "Deposit_to_contract: Buyer Insufficient Funds!");
            require(Token.allowance(msg.sender, address(this))>= buyer_share, "Deposit_to_contract: Failed on Buyer allowance!");
            ...
            buyer_paid_share = true;
        }
        else if(msg.sender == seller && !seller_paid_share)
        {
            require(Token.balanceOf(seller) >= seller_share, "Deposit_to_contract: Seller Insufficient Funds!");
            require(Token.allowance(msg.sender, address(this))>= seller_share, "Deposit_to_contract: Failed on Seller allowance!");
    ...
    {
        uint total_price = unit_price * demand ;
        uint seller_share = ((total_price * penalty_percent) / PERCENTAGE_RATE)+total_price;

        require(seller_share>=Token.balanceOf(address(this)),"Check: Insufficient fund!");
        bool send = Token.transfer(seller, seller_share);
        require(send, "Checkout: Failed to checkout");
    }
