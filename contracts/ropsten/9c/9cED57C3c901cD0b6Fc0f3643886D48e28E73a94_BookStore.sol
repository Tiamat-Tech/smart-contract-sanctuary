...
contract BookStore is Wrapper {
    ...
    string constant private INVALID_SIGNATURE = "Invalid signature";
    string constant private NOT_ENOUGH_BALANCE = "Not enough LIB balance";
    string constant private SIGNATURE_ALREADY_USED = "Signature already used";
    ...
    function processRenting(uint _bookId, address _userAddress) private {
        ...
        rentedBooks[_userAddress][_bookId]++;
        books[_bookId].copies--;

        if (!existingUser[_userAddress][_bookId]) { // If the user rents the particular book for a first time.
            books[_bookId].borrowers.push(_userAddress);
            existingUser[_userAddress][_bookId] = true;
        }

        emit bookRented(_userAddress, books[_bookId].author, books[_bookId].bookName);
    }

    function borrowBookWithSignature(uint _bookId, address _borrower, bytes calldata _signature ) external {
        require(!usedSignatures[_signature], SIGNATURE_ALREADY_USED);

        address signer = keccak256(abi.encodePacked(_bookId)).toEthSignedMessageHash().recover(_signature);
        require(_borrower == signer, INVALID_SIGNATURE);
        
        processRenting(_bookId, _borrower);
        usedSignatures[_signature] = true;
    }

    function rentBook(uint _bookId) external { processRenting(_bookId, msg.sender); }
