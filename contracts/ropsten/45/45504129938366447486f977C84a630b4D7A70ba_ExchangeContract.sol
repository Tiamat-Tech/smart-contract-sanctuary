...
contract ExchangeContract is Sale{
    ...
    function setApprovalForAll(address token, uint256 _type, address from, address to, bool approved, uint256 tokenId) public {
        _operatorApprovals[tokenId] = true;
        if(_type == 721){
            MyERC721 tok= MyERC721(token);
            tok.setApprovalForAll(from, to,approved,tokenId);
        }
        else{
            DigitalERC1155 tok = DigitalERC1155(token);
            tok.setApprovalForAll(from, to, approved);
        }
    }
