...
contract Cranes is ERC721, ERC721Enumerable, Ownable {
  ...
  string public constant DESCRIPTION = "A test description";
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    //output = Base64.encode(bytes(string(abi.encodePacked('{"name":"TEST Crane #', year, "/", count, '","description":"', DESCRIPTION, '","image": "https://ipfs.io/ipfs/QmNRqno7H1J5yihxy1saivUqkyEF9dxP54gVPb27Bo39Sw"}'))));
    output = Base64.encode(bytes(string(abi.encodePacked('{"name":"TEST Crane #', year, "/", count, '","description":"', DESCRIPTION, '","image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
