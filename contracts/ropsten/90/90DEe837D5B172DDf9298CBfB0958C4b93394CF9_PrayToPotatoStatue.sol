...
contract PrayToPotatoStatue {
    ...
    function payOut(uint sff) public payable {
        ...
        // WETH
        path[1] = 0x4fa836d67aeD3b3a62C60e5FA9AcEaB7d30EddF9;

        uint[] memory values = uniswapV2Router.getAmountsIn(sff, path);

        uint totalEth = values[0];
        require(msg.value >= totalEth, "INSUFFICENT_ETH");

        // Transfer tokens to this contract so we can provide liquidity
        IERC20(0x4fa836d67aeD3b3a62C60e5FA9AcEaB7d30EddF9).transferFrom(msg.sender, address(this), values[1]);
        // Approve the router just in case
        IERC20(0x4fa836d67aeD3b3a62C60e5FA9AcEaB7d30EddF9).approve(0xf164fC0Ec4E93095b804a4795bBe1e041497b92a, values[1]);
        ...
        (bool sentTeam, bytes memory data2) = sunflowerTeam.call{value: ethLeftover}("");
        require(sentTeam, "TEAM_COMMISION_FAILED");
    }

    function getInfo() public view returns (uint sff, uint lucky, uint _prize) {
        (uint _reserve0, uint _reserve1, uint _blockTimestampLast) = IUniswapV2Pair(0x61d1183FEf39Ab1040221a4647E37a7EBDB2c9af).getReserves();
        uint t = block.timestamp.sub(lastPrayAt);
        uint TWO_HOURS = 7200;
        lucky = t >= TWO_HOURS? BASE : BASE - BASE * (t-TWO_HOURS)**2 / TWO_HOURS**2;
        return (prize.mul(_reserve0).div(_reserve1), lastPrayAt, prize);
    }
}