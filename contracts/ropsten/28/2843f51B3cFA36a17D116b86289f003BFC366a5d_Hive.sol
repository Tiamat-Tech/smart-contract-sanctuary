...
contract Hive is IHive, Ownable, IERC721Receiver, Pausable {
    ...
    function calculateOwed(uint256 beeAge, uint256 hiveAge) internal view returns (uint256 owed) {
        return (calculateAccumulation(hiveAge, block.timestamp) - calculateAccumulation(hiveAge, beeAge));
    }
    ...
    function calculateBeeOwed(uint256 hiveId, uint256 tokenId) public view returns (uint256 owed) {
        ...
        if (since >= hives[hiveId].startedTimestamp) {
            owed = calculateOwed(since, hives[hiveId].startedTimestamp);
