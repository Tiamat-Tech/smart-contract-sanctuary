...
contract FengRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. FENGs to distribute in the pool.
        uint256 lastRewardTime; // Last time that FENGs distribution occurred.
        uint256 accFengPerShare; // Accumulated FENGs per share, times 1e18. See below.
    ...
    uint256[3] public epochFengPerSecond;
    ...
    constructor(address _feng, uint256 _poolStartTime) public {
        ...
        epochFengPerSecond[0] = epochTotalRewards[0].div(4 days);
        epochFengPerSecond[1] = epochTotalRewards[1].div(5 days);

        epochFengPerSecond[2] = 0;
        operator = msg.sender;
    }

    modifier onlyOperator() {
        require(operator == msg.sender, "FengRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "FengRewardPool: existing pool?");
        }
    }
        ...
        for (uint8 epochId = 2; epochId >= 1; --epochId) {
            if (_toTime >= epochEndTimes[epochId - 1]) {
                if (_fromTime >= epochEndTimes[epochId - 1]) {
                    return _toTime.sub(_fromTime).mul(epochFengPerSecond[epochId]);
                }

                uint256 _generatedReward = _toTime.sub(epochEndTimes[epochId - 1]).mul(epochFengPerSecond[epochId]);
                if (epochId == 1) {
                    return _generatedReward.add(epochEndTimes[0].sub(_fromTime).mul(epochFengPerSecond[0]));
                }
                for (epochId = epochId - 1; epochId >= 1; --epochId) {
                    if (_fromTime >= epochEndTimes[epochId - 1]) {
                        return _generatedReward.add(epochEndTimes[epochId].sub(_fromTime).mul(epochFengPerSecond[epochId]));
                    }
                    _generatedReward = _generatedReward.add(epochEndTimes[epochId].sub(epochEndTimes[epochId - 1]).mul(epochFengPerSecond[epochId]));
                }
                return _generatedReward.add(epochEndTimes[0].sub(_fromTime).mul(epochFengPerSecond[0]));
            }
        }
        return _toTime.sub(_fromTime).mul(epochFengPerSecond[0]);
    }

    // View function to see pending FENGs on frontend.
    ...
    function safeFengTransfer(address _to, uint256 _amount) internal {
        uint256 _fengBal = feng.balanceOf(address(this));
        if (_fengBal > 0) {
            if (_amount > _fengBal) {
                feng.safeTransfer(_to, _fengBal);
            } else {
