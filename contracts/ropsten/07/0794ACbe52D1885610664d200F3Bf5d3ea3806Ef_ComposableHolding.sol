    ...
    PausableUpgradeable
{
    ...
    function approve(
        ...
        address token,
        uint256 amount
    ) external override whenNotPaused validAddress(spender) onlyVaultOrAdmin {
    ...
    function invest(
        IInvestmentStrategy.Investment[] calldata investments,
        address investmentStrategy,
        bytes calldata data
    ) external onlyAdmin validAddress(investmentStrategy) {
        ...
        uint256 mintedTokens = IInvestmentStrategy(investmentStrategy)
            .makeInvestment(investments, data);
    ...
    function withdrawInvestment(
        ...
        require(investmentStrategies[investmentStrategy], "Invalid strategy");
        IInvestmentStrategy(investmentStrategy).withdrawInvestment(
            investments,
            data
        );
    ...
    {
        require(investmentStrategies[investmentStrategy], "Invalid strategy");
        address rewardTokenAddress = IInvestmentStrategy(investmentStrategy)
            .claimTokens(data);
