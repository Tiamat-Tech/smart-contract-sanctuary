    ...
    Ownable
{
    ...
    function sale() internal {
        ...
        for (uint256 i = 0; i < _royalties.length; i++) {
            uint256 recipientFee = msg.value.mul(_royalties[i].feeAmount).div(feePercentage);
            sellAmount.sub(recipientFee);
            payable(_royalties[i].recipient).transfer(recipientFee);
        }
