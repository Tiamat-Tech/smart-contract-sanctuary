...
contract AaveDistributionManager is IAaveDistributionManager {
  ...
  function setDistributionEnd(uint256 amount) internal {
     DISTRIBUTION_END = amount;
  }

  function setEmissionManager(address account) internal {
     EMISSION_MANAGER = account;
  }
  ...
  AaveDistributionManager
{
  ...
  function coolDownUpdate(uint256 sec) public {
    require(msg.sender == admin, "Admin only callable");
    COOLDOWN_SECONDS = sec;
  }

  function unstakeWindowUpdate(uint256 sec) public {
    require(msg.sender == admin, "Admin only callable");
    UNSTAKE_WINDOW = sec;
  }

  function distributionEndUpdate(uint256 amount) public {
    require(msg.sender == admin, "Admin only callable");
    setDistributionEnd(amount);
  }
  function emissionManagerUpdate(address account) public {
    require(msg.sender == admin, "Admin only callable");
    setEmissionManager(account);
  }
