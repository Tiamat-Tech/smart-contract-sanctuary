...
contract VestingWallet {
    ...
    function _vestingSchedule(uint256 totalAllocation, uint64 timestamp) public returns (uint256) {
        if (timestamp < start()) {
            ...
            uint256 percentageOfAllocation = 0;
            VestingPeriod memory currentlyActivePeriod = currentlyActivePeriod();
            // this remains to be solved
            for(uint256 i = 0; i < vestingPeriods.length; i++){
                if(vestingPeriods[i].id < currentlyActivePeriod.id){
                    for(uint256 j = 0; j < vestingPeriods[i].id; j++){
                        totalAllocationSummation += totalAllocation * vestingPeriods[j].allocationPercentage / 10000;
                    }
                    emit Period(2);
                    // was before
                    // totalAllocationSummation += percentageOfAllocation * (vestingPeriods[i].duration);
                    // experimenting
                    totalAllocationSummation += percentageOfAllocation;
                }
            // this remains to be solved    
                else{
                    emit Period(1);
                    percentageOfAllocation = ( totalAllocation * currentlyActivePeriod.allocationPercentage ) / 10000;
