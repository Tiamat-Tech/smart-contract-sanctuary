...
contract Zap is Ownable {
    ...
    ITangleseaRouter02 private ROUTER;  // = ITangleseaRouter02(0xC0788A3aD43d79aa53B09c2EaCc313A787d1d607);
    ...
    function zapInToken(
        ...
        if (isFlip(_to)) {
            ITangleseaPair pair = ITangleseaPair(_to);
    ...
    function zapOut(address _from, uint256 amount) external {
        ...
        if (!isFlip(_from)) {
            ...
            ITangleseaPair pair = ITangleseaPair(_from);
            address token0 = pair.token0();
            address token1 = pair.token1();
    ...
    function _swapBNBToFlip(
        ...
        if (!isFlip(flip)) {
            ...
            // flip
            ITangleseaPair pair = ITangleseaPair(flip);
            address token0 = pair.token0();
            address token1 = pair.token1();
            if (token0 == WETH || token1 == WETH) {
