...
contract Auction is IAuction, ExternallyCallable, AuctionBase {
    ...
    */
    ...
    function getAuctionReservesOf(uint256 auctionId)
        external
        view
        returns (AuctionReserves1e18 memory)
    {
        return
            AuctionReserves1e18(
                reservesOf[auctionId].filled,
                reservesOf[auctionId].mode,
                uint256(reservesOf[auctionId].eth) * 1e12,
                uint256(reservesOf[auctionId].token) * 1e12,
                uint256(reservesOf[auctionId].uniswapLastPrice) * 1e12,
                uint256(reservesOf[auctionId].uniswapMiddlePrice) * 1e12
            );
    }
