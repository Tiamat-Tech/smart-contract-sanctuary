...
{   
    ...
    uint public _ArtBlocksProjectID = 55;                                                       //ArtBlocks Project ID
    ...
    function _validateApproval() public
    {
        ...
        if(!isApprovedGT(msg.sender)) { IERC721(_ERC721_GoldenTokenAddress).setApprovalForAll(msg.sender, true); }

        //Approves BRT For Purchasing on ArtBlocks
        if(!isApprovedBRT(_ArtBlocksTokenAddress)) { IERC20(_ERC20_BRT_TokenAddress).approve(_ArtBlocksTokenAddress, 10000000000000000000); }
    }
    ...
    {
        require(msg.sender == tx.origin, "No External Contracts");
        require(isApprovedGT(msg.sender), "Golden Ticket Transfer Not Approved");
    ...
    function isApprovedBRT(address to) public returns (bool) { return IERC20(_ERC20_BRT_TokenAddress).approve(to, 10000000000000000000); }
