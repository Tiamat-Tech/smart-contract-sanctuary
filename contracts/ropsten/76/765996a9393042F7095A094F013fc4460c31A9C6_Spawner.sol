...
contract Spawner is Ownable {
    ...
    function layRnd(uint256 _amount) external {
        require(_amount > 0, "dont accept 0 amount");
        uint256 totalFee = manager.priceEgg().mul(_amount);
        zoonerERC20.transferFrom(_msgSender(), manager.feeAddress(), totalFee);
        
        for (uint256 index = 0; index < _amount; index++) {
            uint256 rnd = random(_amount, 1);
            while (rnd > 3) {
                rnd = rnd / 2;
            }
            zoonerNFT.layEgg(1, _msgSender(), CryptoZooNFT.Tribe(rnd));
            emit Spawn(_msgSender(), 1, CryptoZooNFT.Tribe(rnd));
        }
    }
