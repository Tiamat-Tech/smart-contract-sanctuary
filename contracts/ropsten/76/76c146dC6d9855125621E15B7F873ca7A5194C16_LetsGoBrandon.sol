...
contract LetsGoBrandon is Context, IERC20, Ownable {
	...
	function _processFeesTEST(uint256 amount, uint256 balanceEth) public view returns(uint256,uint256,uint256,uint256) {
		uint256 feeSum = fees.marketingFee + fees.liquidityFee + fees.developmentFee;
		if (feeSum == 0) return(0,0,0,0);
		...
		////Send marketing funds
		uint256 marketingInEth = balanceEth * fees.marketingFee / (fees.marketingFee + fees.developmentFee);
		return (amountForMarketing,amountForLiquidity, marketingInEth,balanceEth-marketingInEth);
	}
