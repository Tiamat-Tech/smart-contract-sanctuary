...
*/
    ...
    {
        uint24 result;
        uint randomHash = uint(keccak256(abi.encodePacked(_tokenId, block.timestamp)));
        result = result | getTraitFromUint8(uint8(randomHash));
        result = result | getTraitFromUint8(uint8(randomHash >> 8)) << 5;
        result = result | getTraitFromUint8(uint8(randomHash >> 16)) << 10;
        result = result | getTraitFromUint8(uint8(randomHash >> 24)) << 15;
        return result;
    }
